...
const App = {
  ...
  setupLicenseModals() {
    ...
    // Inicializar subida y sincronización directa de apuntes
    this.setupAdminNotesUpload();
  },

  // SUBIDA Y SINCRONIZACIÓN DE APUNTES DESDE EL PANEL DE ADMINISTRADOR
  pendingAdminNoteFile: null,
  pendingAdminNoteContent: null,

  setupAdminNotesUpload() {
    const dropZone = document.getElementById("drop-zone-admin-notes");
    const fileInput = document.getElementById("input-admin-notes-file");
    const btnUpload = document.getElementById("btn-upload-admin-notes");
    const btnAdvanced = document.getElementById("btn-admin-open-advanced-notes");
    ...
    if (dropZone && fileInput) {
      ...
      dropZone.addEventListener("drop", (e) => {
        ...
        if (file) this.handleAdminNoteFileSelection(file);
      });
    ...
    if (btnUpload) {
      btnUpload.addEventListener("click", async () => {
        await this.submitAdminNoteUpload();
      });
    }
  },

  handleAdminNoteFileSelection(file) {
    const validExts = [".md", ".markdown", ".txt"];
    const fileName = file.name || "";
    const ext = fileName.slice(fileName.lastIndexOf(".")).toLowerCase();
    const resultBox = document.getElementById("admin-notes-result");
    const dropLabel = document.getElementById("drop-zone-admin-label");
    const dropSublabel = document.getElementById("drop-zone-admin-sublabel");

    if (!validExts.includes(ext)) {
      this.showToast("Solo se admiten archivos .md, .markdown o .txt", "error");
      if (resultBox) {
        resultBox.style.color = "var(--danger)";
        resultBox.textContent = "Extensión no permitida. Selecciona un archivo Markdown (.md, .markdown) o texto (.txt).";
      }
      return;
    }

    this.pendingAdminNoteFile = file;
    const sizeKb = Math.round(file.size / 1024);
    if (dropLabel) dropLabel.textContent = `📄 ${fileName} (${sizeKb} KB)`;
    if (dropSublabel) dropSublabel.textContent = "Archivo cargado. Revisa los metadatos abajo y presiona 'Subir y Sincronizar'";
    if (resultBox) {
      resultBox.style.color = "var(--text-muted)";
      resultBox.textContent = `Archivo seleccionado: ${fileName}`;
    }

    // Leer contenido con FileReader
    const reader = new FileReader();
    reader.onload = (e) => {
      const text = e.target.result;
      this.pendingAdminNoteContent = text;
      this.inferAdminNoteMetadata(fileName, text);
    };
    reader.onerror = () => {
      this.showToast("Error al leer el archivo seleccionado.", "error");
    };
    reader.readAsText(file, "UTF-8");
  },

  inferAdminNoteMetadata(fileName, content) {
    const combined = (fileName + " " + content.slice(0, 3000)).toLowerCase();
    const subjectSelect = document.getElementById("admin-notes-subject");
    const chapterInput = document.getElementById("admin-notes-chapter");
    const chapterTitleInput = document.getElementById("admin-notes-chapter-title");
    const categoryInput = document.getElementById("admin-notes-category");

    // Inferencia de materia
    if (subjectSelect) {
      if (combined.includes("constitucional") || combined.includes("cpr") || combined.includes("garantía") || combined.includes("derechos fundamentales")) {
        subjectSelect.value = "constitucional";
      } else if (combined.includes("procesal") || combined.includes("cpc") || combined.includes("cpp") || combined.includes("juicio") || combined.includes("recurso")) {
        subjectSelect.value = "procesal";
      } else if (combined.includes("civil") || combined.includes("contrato") || combined.includes("obligacion") || combined.includes("bienes") || combined.includes("acto juridico")) {
        subjectSelect.value = "civil";
      }
    }

    // Inferencia de título desde primer heading #
    const firstHeadingMatch = content.match(/^#+\s+(.+)$/m);
    const cleanStem = fileName.replace(/\.[^/.]+$/, "").replace(/_/g, " ").trim();
    const inferredTitle = firstHeadingMatch ? firstHeadingMatch[1].replace(/[*_#]/g, "").trim() : cleanStem;

    if (chapterTitleInput && !chapterTitleInput.value.trim()) {
      chapterTitleInput.value = inferredTitle;
    }
    if (categoryInput && !categoryInput.value.trim()) {
      categoryInput.value = inferredTitle;
    }

    // Inferencia de capítulo si contiene 'capítulo N'
    const chapMatch = combined.match(/cap[ií]tulo\s*(\d+)/i);
    if (chapMatch && chapterInput) {
      chapterInput.value = parseInt(chapMatch[1], 10) || 1;
    }
  },

  async submitAdminNoteUpload() {
    if (!LicenseService.isAdminMode()) {
      this.showToast("Acceso restringido: Se requiere sesión activa de Administrador.", "warning");
      return;
    }

    if (!this.pendingAdminNoteFile || !this.pendingAdminNoteContent) {
      this.showToast("Por favor arrastra o selecciona un archivo de apuntes primero.", "warning");
      const resultBox = document.getElementById("admin-notes-result");
      if (resultBox) {
        resultBox.style.color = "var(--warning)";
        resultBox.textContent = "Por favor selecciona un archivo (.md, .markdown, .txt) antes de subir.";
      }
      return;
    }

    const btnUpload = document.getElementById("btn-upload-admin-notes");
    const btnText = document.getElementById("btn-upload-admin-text");
    const resultBox = document.getElementById("admin-notes-result");

    const subject = document.getElementById("admin-notes-subject")?.value || "civil";
    const chapterNumber = parseInt(document.getElementById("admin-notes-chapter")?.value, 10) || 1;
    const chapterTitle = document.getElementById("admin-notes-chapter-title")?.value.trim() || "";
    const category = document.getElementById("admin-notes-category")?.value.trim() || chapterTitle || "General";
    const adminPin = LicenseService.getAdminPin();

    const payload = {
      pin: adminPin,
      filename: this.pendingAdminNoteFile.name,
      subject: subject,
      chapterNumber: chapterNumber,
      chapterTitle: chapterTitle || category,
      category: category,
      content: this.pendingAdminNoteContent,
      replace: true
    };
    ...
    try {
      ...
      if (resultBox) {
        resultBox.style.color = "var(--text-muted)";
        resultBox.textContent = "Persistiendo apunte en el servidor y regenerando índice temático...";
      }

      const res = await fetch("/api/admin/upload-notes", {
        method: "POST",
        headers: {
      ...
      if (!res.ok || !data.ok) {
        const errorMsg = data?.error || "Error al subir y sincronizar el apunte.";
      ...
      // Éxito: Sincronización inmediata con el servidor
      const sectionsCount = data.sectionsCreated || (data.topics ? data.topics.length : 1);
      this.showToast(`¡Apuntes sincronizados: ${sectionsCount} sección(es) creadas y vinculadas al índice!`, "success");

      if (resultBox) {
        resultBox.style.color = "var(--success)";
        resultBox.textContent = `✅ "${escapeHTML(data.filename)}" subido y sincronizado exitosamente (${sectionsCount} sección(es) creadas).`;
      }

      // Limpiar archivo seleccionado
      this.pendingAdminNoteFile = null;
      this.pendingAdminNoteContent = null;
      const fileInput = document.getElementById("input-admin-notes-file");
      if (fileInput) fileInput.value = "";
      const dropLabel = document.getElementById("drop-zone-admin-label");
      if (dropLabel) dropLabel.textContent = "Arrastra aquí otro apunte (.md, .markdown, .txt) o haz clic para seleccionarlo";
      const dropSublabel = document.getElementById("drop-zone-admin-sublabel");
      if (dropSublabel) dropSublabel.textContent = "Detecta automáticamente secciones (§ N.N) o genera una sección completa";

      // Re-sincronizar y renderizar al instante
      await this.syncWithServer();
      this.renderSidebar();
      if (this.currentView === "topics") {
        if (data.topics && data.topics.length > 0) {
      ...
      console.error("Error en submitAdminNoteUpload:", e);
      this.showToast("Error de conexión al servidor al subir el apunte.", "error");
      if (resultBox) {
        resultBox.style.color = "var(--danger)";
        resultBox.textContent = "❌ Error de conexión al sincronizar con el servidor.";
      }
    } finally {
      if (btnUpload) btnUpload.disabled = false;
      if (btnText) btnText.textContent = "Subir y Sincronizar";
      if (window.lucide) window.lucide.createIcons();
    }
  },
