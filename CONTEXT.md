...
* **Auto-Descubrimiento en Runtime y Auto-Seccionado de Fuentes (v7.9):**
  ...
  * **Invariante Canónica de 53 Tópicos:** `discover_fuentes_files('fuentes')` retorna `[]` en el estado base del repositorio, asegurando que los 6 archivos canónicos de `FILES_CONFIG` permanezcan 100% byte-estables e idénticos (34 Civil / 9 Procesal / 10 Constitucional).
...
| `js/app.js` | Orquestador | Controlador principal de GRADOMANIACOS, navegación, gateo estricto del sidebar `#app-sidebar` y `#btn-toggle-sidebar` a la vista `topics`, sincronización autoritativa con `/api/sync-topics` sin acumulación zombi (`syncWithServer`), renderizado deduplicado y debounced con `replaceChildren` (`renderSidebar`), tooltips y breadcrumbs sanitizados, listado dinámico y reactivo de fuentes en `renderSourcesList()` (v7.9, con consulta a `/api/fuentes` o derivación defensiva de `INITIAL_DATA.topics`) y auto-sincronizador de fuentes. Los métodos de subida admin fueron eliminados en v7.8; se conserva el importador local `#btn-admin-manage-notes` (NotebookLM → `localStorage`). |
...
| `js/case-generator-agent.js` | Agente IA | Síntesis dogmática de casos inéditos, catálogo de 37 instituciones en 13 arquetipos (10 existentes + 3 nuevos interdisciplinarios), 4 contra-instituciones doctrinales, preguntas graduadas con 5 alternativas y distractores seductores, pauta docente, error fatal de grado, rúbrica AIME de 4 dimensiones, soluciones modelo $\ge 400$ chars, nutrición dinámica desde apuntes (`syncApuntesFromServer`, `APUNTES_INDEX`), `FUENTES_CORPUS` canónico completo (incluyendo `CLASE_9_11.md`), tolerancia a fuentes desconocidas mediante `resolveLinkedFuente()` (v7.9), validación estricta y poda FIFO. |
...
| `server.py` | Backend | Servidor HTTP Python multi-hilo, resolución dinámica de `DB_PATH`, deduplicación canónica en `get_all_synced_topics()` con base canónica `fuentes/`, libro mayor de usos auditables en `/api/admin/codes`, daemon de purga con doble inmunidad (`run_auto_purge_daemon`), ordenamiento canónico de `indexCode` en `/api/sync-topics`, endpoint `/api/fuentes` con escritura correcta de encabezados y cuerpo, endpoints de subida de apuntes eliminados (HTTP 404), Zero Exposure y rate limiting. |
| `generate_clean_notes_data.py` | Parser / Sync | **Base canónica de apuntes con auto-descubrimiento (v7.9):** `discover_fuentes_files()` escanea deterministamente `fuentes/` en runtime con deduplicación insensible a mayúsculas contra `FILES_CONFIG`. `infer_file_config()` deduce materias por heurística ponderada y categoría. `extract_sections_from_file()` auto-secciona módulos (`N.1, N.2...`), headings Markdown (`##`) o bloques monolíticos con renumeración consecutiva sin huecos. Deduplicación canónica con `deduplicate_sections()`, numeración ordinal única por disciplina mediante `assign_index_codes()` (`indexCode`: Civil `1.1..1.34`, Procesal `2.1..2.9`, Constitucional `3.1..3.10`). Preserva 100% byte-idéntica la base canónica de 53 tópicos y regenera `all_afg_topics.json` y `js/data.js`. |
| `fuentes/` | Base Canónica | **Carpeta canónica del temario.** Contiene los 6 apuntes oficiales (`ACTO JURIDICO.md`, `LOS BIENES.md`, `LAS OBLIGACIONES.md`, `CLASE_9_11.md`, `PROCESAL.md`, `CONSTITUCIONAL.md`) y admite nuevos archivos Markdown que son auto-descubiertos y auto-seccionados en runtime. Cualquier cambio aquí + `git push` regenera `all_afg_topics.json` y `js/data.js` en CI e implementa la página (GitHub Pages). |
...
| `test_e2e_case_flow.cjs` | Test E2E | Suite integral de 671 pruebas (100% PASS) que audita casos IA, seguridad, rúbrica AIME, confidencialidad, multi-dispositivo, limitación en Modo Demo vs. Pase Activo, canonización de fuentes, ordenamiento canónico `indexCode`, los 13 arquetipos con nutrición viva (Sección 23) y la Sección 24 con contrato de integridad de códigos temáticos (`code`/`indexCode` `N.M`, `chapterNumber` entero positivo) y tolerancia a fuentes en el agente (v7.9). |
...
| `test_deduplication_flow.cjs` | Test Integración | Suite de 8 pruebas (100% PASS) que verifica deduplicación canónica en `all_afg_topics.json`, unicidad de `indexCode`, auto-curación de 106 a 53 tópicos en `StorageService.getData()`, invariante de `discover_fuentes_files('fuentes') == []` y auto-seccionado modular/headings dinámico con fixture temporal (v7.9). |
| `PROMPTS/` | Documentación Operativa | **Puente humano ↔ Agente Antigravity IDE.** Carpeta canónica de prompts de ingeniería listos para ejecutar (índice `README.md`, plantilla `_TEMPLATE.md`, prompts numerados: 001 subida de apuntes ⚠️ **superseded por 007** (v7.8), 002 agente de casos ✅ v7.3, 003 índice unificado ✅ v7.2, 004 registro persistente de códigos de acceso usados ✅ v7.4, 005 header móvil ✅ v7.5, 006 diagnóstico Render ✅ v7.6, 007 fuentes/ canónica + eliminación subida admin + regen CI ✅ v7.8, 008 índice automático desde `fuentes/` ✅ v7.9, 009 nutrición viva del agente 📝 **por ejecutar**). Cada prompt exige actualización correlativa de `CONTEXT.md` y 100 % PASS de las suites al implementarse. |
...
* **v7.9 (Índice Automático desde fuentes/, Auto-Descubrimiento en Runtime y Auto-Seccionado Modular/Headings):**
  - **Misión de Ingeniería:** Eliminar cualquier necesidad de editar código (`FILES_CONFIG` en `generate_clean_notes_data.py`) al incorporar nuevos apuntes. Soltar un archivo `.md` en la carpeta `fuentes/` es el único acto necesario para publicar y organizar temarios de grado.
  - **Auto-Descubrimiento Determinista (`discover_fuentes_files`):** Escaneo en runtime de `fuentes/` con ordenamiento alfabético determinista. Implementa un guardrail de comparación insensible a mayúsculas/minúsculas contra los 6 archivos fijos de `FILES_CONFIG` para prevenir duplicaciones en Windows (`PROCESAL.md` vs `procesal.md`). Excluye archivos de documentación (`README.md`), archivos ocultos (`.`), temporales (`~`) o archivos insignificantes (< 50 caracteres).
  - **Inferencia Heurística de Metadatos (`infer_file_config`):** Detección automática de materia (`subject`) mediante conteo ponderado de palabras clave con orden de precedencia: `constitucional` $\to$ `procesal` $\to$ `civil` (fallback). Extracción de categoría y título legible a partir del primer encabezado `#` del documento. Detección del capítulo/módulo por defecto (`defaultChapterNum`).
  - **Auto-Seccionado Jerárquico Inteligente (`extract_sections_from_auto_discovered_file`):**
    1. Si detecta menciones de módulo/capítulo/unidad (`Módulo N`, `Capítulo N`, `UNIDAD N`), extrae las cédulas bajo cada módulo.
    2. Si no detecta módulos formales pero contiene subtítulos Markdown (`##`, `###`), los convierte en cédulas independientes bajo el capítulo por defecto.
    3. Si carece de subtítulos, genera una sección monolítica con código `N.1`.
    4. Renumera consecutivamente las secciones sin huecos (`N.1, N.2...`) para cada capítulo.
  - **Invariante Canónica de 53 Tópicos:** `discover_fuentes_files('fuentes')` retorna `[]` en el estado base del repositorio, asegurando que los 6 archivos canónicos de `FILES_CONFIG` permanezcan 100% byte-estables e idénticos (34 Civil / 9 Procesal / 10 Constitucional).
  - **Frontend Dinámico (`js/app.js`):** `renderSourcesList()` consulta dinámicamente `/api/fuentes` o deriva defensivamente del conjunto de `sourceFile` en `StorageService` / `INITIAL_DATA.topics`, enriqueciendo títulos y descripciones y garantizando soporte offline.
  - **Tolerancia a Fuentes en Agente IA (`js/case-generator-agent.js`):** Inclusión de `CLASE_9_11.md` en `FUENTES_CORPUS` e implementación de `resolveLinkedFuente(fuenteRef)` que resuelve archivos canónicos exactos, por stem insensible a mayúsculas/extensiones, y tolera archivos desconocidos omitiendo el vínculo con advertencia sin interrumpir la nutrición ni la síntesis de casos.
  - **Aprobación del 100% de Pruebas Automatizadas (804 Pruebas en Total / 100% PASS):**
    - 8/8 pruebas aprobadas en `test_deduplication_flow.cjs` (incluye invariante de fuentes canónicas y auto-descubrimiento dinámico con fixture temporal).
    - 671/671 pruebas aprobadas en `test_e2e_case_flow.cjs` (incluye Sección 24 con contrato de integridad de códigos temáticos `N.M`, `chapterNumber` entero positivo y tolerancia de fuentes).
...
* **Hito documental en `PROMPTS/009_agente_ia_nutricion_apuntes_dinamicos.md` (v1.0, propone Bitácora §9 v7.10 — incremental, el PROMPT 008 ya se implementó en v7.9):** Nuevo prompt de ingeniería del puente **(pendiente de ejecución)** que garantiza que el **Agente de IA se nutra en vivo de los apuntes dinámicos** de PROMPT 008: `FUENTES_CORPUS` pasa a ser semilla canónica más `DYNAMIC_CORPUS` derivado del contenido real de las cédulas vía nuevo helper `extractCitations(text)`; `populateApuntesIndex()` preserva `content`/`code`/`chapterNumber` y deriva `rules` verdaderas; `resolveLinkedFuente()` gana un paso de resolución contra el corpus dinámico (archivos no canónicos) y `getLinkedApuntesForArchetype()` resuelve por clave secundaria `(subject, indexCode|code)` cuando el id exacto no existe; `assertCitationIntegrity()` valida en 3 capas (semilla → dinámico → whitelist `validNorms` con índice O(1) pre-computado), legitimando citas de módulos nuevos y rechazando citas inventadas; `validateGeneratedCase()` verifica `linkedTopics` contra el índice vivo (soft-warning en entornos sin índice) y sanea `linkedFuentes.file`; re-nutrición confirmada en el ciclo `sync-check` → `syncWithServer()` → `invalidateApuntes()` (≤ 3.5 s) y nutrición estática vía `INITIAL_DATA.topics` regenerado por CI; Sección 23 de `test_e2e_case_flow.cjs` ampliada con aserciones de nutrición dinámica y citas corpus-driven (invariante: 53 tópicos y 671 aserciones previas intactas).
