...
* **Vault de Conocimiento con Índice Semántico Estático (v7.22 → v7.25, PROMPTS 017 y 020):**
  ...
  **En v7.25 (PROMPT 020 — Búsqueda que Encuentra):** El motor incorpora:
  ...
  En la UI, se aloja en `#vault-search-box` con `#vault-search-input`, `#vault-search-clear`, `#vault-search-filters`, `#vault-search-history` y contenedor reactivo `#vault-search-results` posicionado sobre `#topics-tree-container` en el sidebar.
...
* **Q&A Extractivo Puro: Pregunta → Citas Verbatim (v7.27, PROMPT 021):**
  ...
  - **Integración en UI:** Contenedor `#qa-container` integrado en `#vault-search-box`, selector `#qa-mode-pills` (pills para `definicion`, `panorama`, `comparativa`), input `#qa-ask-input`, botón `#qa-ask-button`, panel reactivo `#qa-answer`, y panel de huecos `#qa-gaps-panel` (`#qa-gaps-list`, `#qa-gaps-clear`).
...
| `index.html` | Estructura | Workbench jurídico, visor de cédulas, visor de casos, inyección de `js/exam-structure-grammar.js`, `js/question-developer.js`, `js/sinonimos.js` (v7.25), `js/vault-index.js`, `js/vault-search.js` y `js/qa-composer.js` (v7.27), header móvil optimizado solo-íconos con atributos de accesibilidad ARIA (`aria-label="GRADOMANIACOS"`, `aria-label="Cerrar sesión"`), sin selector de tema (Dark Academy inmutable), **buscador global unificado `#global-search-container` con selector segmentador de modos `#search-mode-selector` (`role="radiogroup"`), píldoras `#search-mode-semantic` y `#search-mode-qa`, campo de entrada `#global-search-input` (`maxlength="200"`) y dropdown de resultados `#search-results-dropdown` (`aria-live="polite"`) (v7.29, PROMPT 023)**, caja de búsqueda semántica del Vault `#vault-search-box` con `#vault-search-input`, `#vault-search-clear`, bloque de filtros `#vault-search-filters` (`#vault-filter-pills`, `#vault-filter-chapter`), contenedor de historial `#vault-search-history` (`#vault-history-clear`, `#vault-history-items`), contenedor reactivo de resultados `#vault-search-results` posicionado sobre `#topics-tree-container` (v7.22 / v7.25), módulo de Q&A extractivo puro `#qa-container` con selector de modos `#qa-mode-pills` (`.qa-mode-pill`), input `#qa-ask-input`, botón `#qa-ask-button`, panel de respuesta `#qa-answer` y ranking local de temas pendientes `#qa-gaps-panel` (`#qa-gaps-list`, `#qa-gaps-clear`) (v7.27, PROMPT 021), y modal unificado de acceso `#unlock-modal` con aviso de 48h `#demo-expiry-notice`. Bloque de respaldo manual `.sidebar-progress-actions` retirado definitivamente por seguridad (v7.28, PROMPT 022). **Cuenta de Presentación Docente (v7.16):** el panel de administración envuelve el formulario de generación en `#admin-license-form` y el historial en `#admin-codes-panel` (ambos ocultados en modo docente) e incluye el banner aclaratorio `#docente-mode-banner`. |
...
| `css/main.css` | Estilos | Sistema de diseño *Dark Academy* inmutable (`color-scheme: dark;` en `:root`), variables tipográficas (`Playfair Display`, `Plus Jakarta Sans`), tokens WCAG AA de modo claro preservados como código muerto documentado, header responsive móvil (`@media (max-width: 768px)`) con touch targets $\ge 44 \times 44\text{ px}$, **estilos del buscador global y segmentador de modos (`.search-mode-selector`, `.search-mode-pill`, `.search-input-wrapper`, `.search-mode-badge`, tokens de modo azul `--info-text` para Semántico y dorado `--gold-primary` para Q&A, ocultamiento de `.search-mode-label` en móvil para touch targets $\ge 44 \times 44\text{ px}$ sin desborde a 360 px, v7.29, PROMPT 023)**, componentes del cuestionario de grado (`#section-quiz-developer`, `.quiz-question-card`, `.quiz-q-statement` con `white-space: pre-line` para enunciados multilínea I-IV v7.17, `.quiz-option`, `.quiz-solution`, `#quiz-completed-banner`, `.quiz-progress-bar`) y la insignia de laboratorio `.quiz-admin-lab-badge` (v7.18) en ámbar/dorado con alto contraste accesible. |
| `css/sidebar.css` | Estilos | Índice lateral (`#app-sidebar`), barra de progreso de dominio (`.progress-bar`), estilos depurados sin reglas huérfanas de respaldo manual (v7.28), estilos para el Vault de Conocimiento (`.vault-search-box`, `.vault-search-input`, `.vault-search-clear-btn`, `.vault-search-filters`, `.vault-filter-pill`, `.vault-filter-chapter`, `.vault-search-history`, `.vault-history-chip`, `.vault-search-results`, `.vault-search-result-item`, `.vault-highlight`, `.vault-result-code`, `.vault-search-empty`, v7.22 / v7.25) y **estilos para el motor de Q&A extractivo (`.qa-container`, `.qa-mode-pill`, `.qa-input-wrapper`, `.qa-ask-input`, `.qa-ask-button`, `.qa-answer`, `.qa-citation-card`, `.qa-block-perspective-badge`, `.qa-citation-code`, `.qa-citation-snippet`, `.qa-action-jump-btn`, `.qa-uncovered-box`, `.qa-gaps-panel`, `.qa-gap-chip`, v7.27, PROMPT 021)**. |
...
| `js/app.js` | Orquestador | Controlador principal de GRADOMANIACOS, navegación, gateo estricto del sidebar `#app-sidebar` y `#btn-toggle-sidebar` a la vista `topics`, sincronización autoritativa con `/api/sync-topics` sin acumulación zombi (`syncWithServer`), renderizado deduplicado y debounced con `replaceChildren` (`renderSidebar`), **orden de capítulos por min `indexCode` para respetar el temario canónico Discusión→Prueba→Sentencia (v7.19.1)**, **controlador unificado del buscador global del header (`setupGlobalSearch`, `initGlobalSearchMode`, `setGlobalSearchMode`, `updateGlobalSearchModeUI`, `executeGlobalSearch`, `searchGlobal`, `sanitizeSearchQuery` con purga de nulos/zero-width y recorte a 200 caracteres, persistencia en `localStorage['global_search_mode']`, delegación transparente a `VaultSearch.searchVault` y `QAComposer.composeAnswer`, atajos `Ctrl+K`, `Enter`, `Esc`, y auto-conmutación a vista `topics` fuera de ella) (v7.29, PROMPT 023)**, integración y debounce de búsqueda en el Vault de Conocimiento en `setupVaultSearch()` con navegación a la cédula mediante `App.openTopic()` (v7.22), soporte de filtros por materia (`#vault-filter-pills`) y capítulos ordenados por min `indexCode` (`#vault-filter-chapter`), gestión de historial de búsqueda en `localStorage['vault_recent_searches']` con tope de 10 entradas, chips interactivos sanitizados con `SecurityShield.escapeHtml` y botón de limpieza rápida (v7.25, PROMPT 020), motor de Q&A extractivo integrado en `setupQAExtractor()` con selección de modos, composición verbatim instantánea y renderizado reactivo en `#qa-answer`, extensión de `App.openTopic(topicId, { highlight })` con resaltado del fragmento en `#topic-markdown-body` mediante `highlightInTopicViewer(snippet)` y `<mark class="vault-highlight">`, y gestión del ranking local de temas pendientes (v7.27, PROMPT 021), tooltips y breadcrumbs sanitizados, listado dinámico y reactivo de fuentes en `renderSourcesList()`, inyección del desarrollador de preguntas (`QuestionDeveloper`) en `renderTopicViewer()`, actualización reactiva de progreso y dominio en caliente, y auto-sincronizador de fuentes. **Gateo de Visibilidad del Cuestionario por Rol Admin (v7.18, PROMPT 015):** inyección de `#section-quiz-developer` condicionada a `isAdmin && isUnlocked` con insignia `.quiz-admin-lab-badge` en la cabecera del cuestionario, manteniendo el botón `#btn-bottom-mastery` / `#btn-toggle-mastery` visible y 100% operativo para todos los perfiles de usuario. **Sincronización de avance de cédulas (v7.12):** orquestador `pullMasteryProgress()` con guard de re-entrancia `_isPullingMastery` (hooks en `startLiveSync`, `syncWithServer` y `onAuthStateChanged`). Eliminado el cableado obsoleto de respaldo manual (`setupProgressBackup`) en v7.28. **Cuenta de Presentación Docente (v7.16):** login admin por rol (toasts diferenciados), `applyDocenteRestrictions()` que oculta `#admin-license-form`, `#admin-codes-panel` y `#btn-admin-manage-notes` mostrando `#docente-mode-banner`, y guards de click en `btnGenerate`, revocación e `openImportModal`. Los métodos de subida admin fueron eliminados en v7.8; se conserva el importador local `#btn-admin-manage-notes`. |
