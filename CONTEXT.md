...
* **Hito documental en `PROMPTS/008_indice_automatico_fuentes_auto_seccionado.md` (v1.0, propone Bitácora §9 v7.9):** Nuevo prompt de ingeniería del puente **(pendiente de ejecución)** que instruye implementar el **índice automático desde `fuentes/`**: `build_files_config()` debe escanear la carpeta en runtime (`discover_fuentes_files()`, orden alfabético determinista) sin exigir editar `FILES_CONFIG`; `extract_sections_from_file()` debe auto-seccionar en cédulas `N.1, N.2…` según módulo/capítulo detectado (regex `Módulo|Capítulo|UNIDAD`), con fallback a headings Markdown `##`/`###` y finalmente a sección monolítica con módulo detectado (o `1.1`); re-numeración consecutiva sin huecos; inferencia de `subject` por heurística de keywords (constitucional/procesal/civil) con orden de precedencia documentado; `App.renderSourcesList()` dinámico; `FUENTES_CORPUS`/`linkedFuentes` tolerantes a archivos desconocidos; invariante de que `discover_fuentes_files()` sobre el repo actual retorna `[]` (los 6 canónicos no se alteran → 53 tópicos intactos); suites extendidas y 100 % PASS. El humano solo debe **soltar el `.md` en `fuentes/`** y publicar con `git push`.
