...
| `js/qa-composer.js` | Motor Q&A Extractivo | Motor client-side puro y sin estado `QAComposer` (`composeAnswer(query, options)`) que compone respuestas extractivas sin LLM a partir de citas textuales de las 103 cédulas con procedencia (`§ indexCode`, archivo, disciplina). Soporta modos `definicion` (top-1 directo), `panorama` (representativos por materia en orden canónico) y `comparativa` (yuxtapone 2 instituciones de distinta disciplina/capítulo sin contraste subjetivo propio). Gate de salida estricto con falla cerrada ante citas no canónicas, sugerencia dogmática mediante `inferSuggestedSubject()` y persistencia local de huecos en `vault_uncovered_queries` (v7.27, PROMPT 021). |
...
| `.github/workflows/deploy-pages.yml` | CI / CD | Workflow de despliegue en GitHub Pages. **Regenera cruces dogmáticos, índice temático y Vault en CI** (`python3 scripts/build_cross_connections.py`, `python3 generate_clean_notes_data.py` y `python3 build_vault_index.py`) a partir de `fuentes/` antes de publicar el artefacto estático, garantizando que `git push` mantenga al día las 103 cédulas, los cruces dogmáticos y el motor de búsqueda en el despliegue estático. |
