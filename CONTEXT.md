...
* **Verificación de Cédula y Desarrollador de Preguntas del Agente (v7.11 → v7.15):**
  Al término de la lectura de cada cédula (canónica o módulo dinámico auto-descubierto), el componente `QuestionDeveloper` (`js/question-developer.js`) formula exactamente **4 preguntas de verificación de grado** en formato A-E con solución dogmática oficial revelable y cierre obligatorio `"Conclusión:"`. La naturaleza de las preguntas se adapta automáticamente a la materia de la cédula mediante la **Regla Maestra de Mix** (taxonomía y arquetipos en v7.15):
  * `dogmatic` (default): 4 preguntas dogmáticas $\to$ 1) concepto y requisitos de existencia/validez; 2) clasificación y elementos; 3) efectos y consecuencias jurídicas; 4) diferencias dogmáticas o causales de ineficacia.
  * `procedencia`: 4 preguntas adjetivas $\to$ 1) procedencia y agravio en el supuesto base; 2) requisitos de admisibilidad formal y oportunidad; 3) efectos y alcance procesal (efecto suspensivo vs. devolutivo, ONI); 4) dogmática y ratio iuris de la vía procesal.
  * `competencia`: 4 preguntas orgánicas $\to$ 1) tribunal naturalmente competente (fuero, materia, cuantía); 2) prórroga de la competencia relativa; 3) radicación de la causa e invariabilidad; 4) vías adjetivas de reclamo (declinatoria e inhibitoria).
  * `plazos`: 4 preguntas temporales $\to$ 1) régimen de cómputo (días hábiles CPC vs. corridos CC); 2) caso práctico de cómputo con feriados e inhábiles; 3) fatalidad legal y caducidad de facultades; 4) caso de entorpecimiento procesal por fuerza mayor.
  * `case`: 4 preguntas prácticas $\to$ 1) caso práctico de subsunción directa; 2) caso con alteración fáctica (dolo o vicio); 3) colisión de derechos concurrentes y protección a terceros; 4) síntesis dogmática de clausura.
  * **Perfil «Manejo» Contenido-Conductor (v7.15):** Desde v7.15 el motor no usa plantillas: extrae del contenido real del apunte oraciones sustantivas (definiciones, rasgos, condiciones de procedencia, reglas de cómputo, tribunales) y construye preguntas de arquetipo por naturaleza — `_specConcepto`/`_specConceptoAlt` (concepto mejor desarrollado: la correcta reproduce íntegramente la definición más extensa del apunte; distractores que omiten cláusulas, mutan la hipótesis o presentan una institución afín), `_specCaracteristicas` (trait joins con rasgos verdaderos y mutaciones), `_specProcedencia`, `_specPlazosCalculo` (cómputo aritmético real: p. ej. 18 días hábiles notificados un lunes → vence el **jueves de la cuarta semana**), `_specPlazosRegla`, `_specCompetencia`/`_specCompetenciaAlt`, `_specCaso` y `_specDefinicionDirecta`. La alternativa correcta siempre comparte contenido con el apunte (anclaje por bigramas), los distractores se normalizan hacia ~0.85×L de la correcta con tope duro `[0.72·L, 0.98·L]` (la correcta es estrictamente la más extensa; ratio máx/mín ≤ 1.6), y `validateSectionQuestions()` devuelve `{ valid, errors, warnings }` con warnings no bloqueantes (tokens de absurdos prohibidos, ausencia de anclaje, heterogeneidad > 1.6).
...
* **Diferenciación Canónica entre `code` e `indexCode` (v7.2):**
  ...
  * `indexCode`: Secuencia ordinal continua e irrepetible generada por disciplina mediante `assign_index_codes()` en `generate_clean_notes_data.py` y `server.py`. Utiliza el formato `N.M`, donde `N` identifica la disciplina (1 = Civil, 2 = Procesal, 3 = Constitucional) y `M` es una secuencia continua ordenada por `(chapterNumber asc, parse_code_tuple(code) asc, orig_idx asc)`:
    ...
    * **Procesal (59 tópicos):** `2.1` a `2.59` (consolidado en v7.14: canon 9 + La Prueba 14 + Mayor Cuantía 36).
...
* **Suite de Regresión e Integración (`test_e2e_case_flow.cjs`):** Ejecuta 810 pruebas integrales al 100% PASS (incluyendo la Sección 9 reescrita en v7.8; Sección 10 con ordenamiento canónico de `indexCode`; Sección 23 con los 13 arquetipos dogmáticos, pautas oficiales y la Subsección 23.6 (v7.10) con nutrición dinámica viva desde `DYNAMIC_CORPUS`, resolución secundaria y citas corpus-driven; Sección 24 con contrato de integridad de códigos temáticos `N.M`; la **Subsección 24.4 (v7.11)** que audita exhaustivamente el `QuestionDeveloper`: clasificación de las 5 naturalezas con precedencia estricta, 4 preguntas con 5 alternativas A-E, soluciones dogmáticas $\ge 150-200$ chars terminadas en `"Conclusión:"`, pauta docente, citas legales no alucinadas como subconjunto estricto de las fuentes reales, determinismo PRNG `fnv1a` + `mulberry32`, tolerancia a cédulas sin citas, auto-cierre como completada al alcanzar 4/4 en `StorageService` y no-regresión de `toggleTopicMastery`; la **Subsección 24.5 (v7.15)** que audita el perfil «manejo» contenido-conductor en las 103 cédulas (anclaje por bigramas de la alternativa correcta, concepto mejor desarrollado en `civil-losbienes-1-4` con la correcta reproduciendo íntegramente la definición más extensa y distractores que omiten cláusulas o mutan la institución, cero tokens de absurdos prohibidos, plausibilidad ≥ 1 distractor que comparte vocabulario, homogeneidad ≤ 1.6 con la correcta como opción más desarrollada, cómputo aritmético real en `procesal-procemayor-5-2` — 18 días hábiles notificados un lunes vencen el jueves de la cuarta semana — y regresión determinista de la 24.4); y la **nueva Sección 25 (v7.12)** con los 57 tests de sincronización multi-dispositivo del avance de cédulas: endpoints `GET/POST /api/user/topic-mastery` con `401` sin sesión, alta real de usuarios con código, multi-dispositivo (el Dispositivo B refleja el avance del A), LWW secuencial (ts 1000 → 2000 → 1500 no sobrescribe), lote de 3 cédulas, validaciones `400`/`413`, aislamiento total entre usuarios, purga de 48h sin huérfanos de `user_topic_mastery`, y contrato de cliente con mocks de sesión/red: degradación elegante con cero llamadas `fetch` sin Pase Activo, pull remoto aplicado con LWW, conservación de la marca local más nueva con reconciliación `POST`, sello de timestamps en `toggleTopicMastery`, merge aditivo de respaldos sin perder marcas recientes, validación estricta de `user-progress-backup` y forma del payload de exportación) que verifican la confidencialidad de modelos docentes, casos IA, rúbrica AIME 2026-20, ciberseguridad, sincronización multi-dispositivo y la regla de limitación de resolución de casos en Modo Demo vs. Pase Activo.
...
| `index.html` | Estructura | Workbench jurídico, visor de cédulas, visor de casos, script de `QuestionDeveloper`, header móvil optimizado solo-íconos con atributos de accesibilidad ARIA (`aria-label="GRADOMANIACOS"`, `aria-label="Cerrar sesión"`), sin selector de tema (Dark Academy inmutable), modal unificado de acceso `#unlock-modal` con aviso de 48h `#demo-expiry-notice`, y bloque de respaldo de avance `.sidebar-progress-actions` (v7.12: `#btn-export-progress`, `#btn-import-progress`, `#input-import-progress` oculto). **Cuenta de Presentación Docente (v7.16):** el panel de administración envuelve el formulario de generación en `#admin-license-form` y el historial en `#admin-codes-panel` (ambos ocultados en modo docente) e incluye el banner aclaratorio `#docente-mode-banner`. |
...
| `css/main.css` | Estilos | Sistema de diseño *Dark Academy* inmutable (`color-scheme: dark;` en `:root`), variables tipográficas (`Playfair Display`, `Plus Jakarta Sans`), tokens WCAG AA de modo claro preservados como código muerto documentado, header responsive móvil (`@media (max-width: 768px)`) con touch targets $\ge 44 \times 44\text{ px}$, y componentes del cuestionario de grado (`#section-quiz-developer`, `.quiz-question-card`, `.quiz-option`, `.quiz-solution`, `#quiz-completed-banner`, `.quiz-progress-bar`). |
| `css/sidebar.css` | Estilos | Índice lateral (`#app-sidebar`), barra de progreso de dominio (`.progress-bar`) y bloque de respaldo de avance multi-dispositivo (`.sidebar-progress-actions`, `.btn-progress-backup`) con hover dorado sobre tokens `--gold-*` (v7.12). |
...
| `js/question-developer.js` | Agente IA | Motor de desarrollo de preguntas de verificación de grado para cédulas (canónicas y dinámicas). Taxonomía de 5 naturalezas (`dogmatic`, `procedencia`, `competencia`, `plazos`, `case`), generador determinista PRNG (`fnv1a` + `mulberry32`), formulación estricta de 4 preguntas con 5 alternativas (A-E), solución dogmática oficial con cierre obligatorio `"Conclusión:"`, pauta docente, citas legales verificadas no alucinadas y memoización. Desde v7.15 es un motor **contenido-conductor de perfil «manejo»**: arquetipos por naturaleza (`_specConcepto`/`_specConceptoAlt`, `_specCaracteristicas`, `_specProcedencia`, `_specPlazosCalculo`, `_specPlazosRegla`, `_specCompetencia`/`_specCompetenciaAlt`, `_specCaso`, `_specDefinicionDirecta`) construidos con oraciones reales del apunte, alternativa correcta siempre anclada por bigramas y más desarrollada que los distractores (normalización a ~0.85×L con tope duro `[0.72·L, 0.98·L]`), y `validateSectionQuestions()` con `{ valid, errors, warnings }` (warnings no bloqueantes: absurdos, anclaje, heterogeneidad > 1.6). |
...
| `test_e2e_case_flow.cjs` | Test E2E | Suite integral de 810 pruebas (100% PASS) que audita casos IA, seguridad, rúbrica AIME, confidencialidad, multi-dispositivo, limitación en Modo Demo vs. Pase Activo, canonización de fuentes, ordenamiento canónico `indexCode`, los 13 arquetipos con nutrición viva, la Subsección 23.6 con nutrición dinámica y citas corpus-driven (v7.10), la Subsección 24.4 con el Desarrollador de Preguntas de Verificación del Agente (`QuestionDeveloper`, v7.11), la **Subsección 24.5** con el perfil «manejo» contenido-conductor del `QuestionDeveloper` (anclaje de la correcta por bigramas, concepto mejor desarrollado, cero absurdos, plausibilidad, homogeneidad ≤ 1.6 y cómputo aritmético de plazos, v7.15), la **Sección 25** con la sincronización multi-dispositivo del avance de cédulas (57 tests: `401`/`400`/`413`, LWW por cédula, multi-dispositivo, aislamiento, purga sin huérfanos y contrato de cliente con mocks, v7.12), y el canon de 103 tópicos / 59 procesales / 50 cédulas aportadas por las fuentes registradas (v7.14). |
