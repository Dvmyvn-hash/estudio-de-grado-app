...
* **Diferenciación Canónica entre `code` e `indexCode` y Orden Temario Canónico (v7.2 / v7.19):**
  ...
  * `indexCode`: Secuencia ordinal continua e irrepetible generada por disciplina mediante `assign_index_codes()` en `generate_clean_notes_data.py`. Utiliza el formato `N.M`, donde `N` identifica la disciplina (1 = Civil, 2 = Procesal, 3 = Constitucional) y `M` es una secuencia continua ordenada estrictamente por el **Temario Canónico Oficial del Examen de Grado** mediante la tupla de ordenamiento `(_temario_order asc, chapterNumber asc, parse_code_tuple(code) asc, _orig_idx asc)` con re-ordenamiento in-place de la lista global de secciones:
    ...
    * **Procesal (59 tópicos):** `2.1` a `2.59` ordenado de forma oficial según la estructura del Examen de Grado (**Juicio Ordinario de Mayor Cuantía ANTES que La Prueba**):
      ...
      * `2.10` a `2.45`: Juicio Ordinario de Mayor Cuantía (`PROCEMAYORCUANTIA-pulido.md`, cap 6–13, 36 cédulas): Medidas Prejudiciales (`2.10`-`2.15`), La Demanda (`2.16`-`2.19`), Control de Admisibilidad (`2.20`-`2.25`), Retiro/Desistimiento/Modificación (`2.26`-`2.31`), El Emplazamiento (`2.32`-`2.35`), Actitudes del Demandado (`2.36`-`2.39`), Réplica y Dúplica (`2.40`-`2.41`), Etapa de Sentencia y Modos Anormales de Término (`2.42`-`2.45`).
      * `2.46` a `2.59`: La Prueba (`DERECHO_PROCESAL_LA_PRUEBA.md`, cap 3–5, 14 cédulas): Teoría General de la Prueba (`2.46`-`2.49`), Etapa Probatoria en el Juicio Ordinario (`2.50`-`2.53`), Medios de Prueba en Particular Art. 341 CPC (`2.54`-`2.59`).
...
* **Auto-Ordenamiento por Temario Canónico del Examen de Grado (v7.19, PROMPT 016):**
