...
* **Nutrición Dinámica desde Apuntes de Grado (`APUNTES_INDEX`):**
  El agente se nutre en tiempo real de los 68 apuntes unificados mediante `CaseGeneratorAgent.syncApuntesFromServer()` e invalida caché ante sincronizaciones con el servidor (`invalidateApuntes()`). Cada caso sintetizado ancla `linkedApuntes` con código, título, archivo de origen sanitizado (`sourceFile`) y extracto doctrinal.
...
* **Diferenciación Canónica entre `code` e `indexCode` (v7.2):**
  ...
  * `indexCode`: Secuencia ordinal continua e irrepetible generada por disciplina mediante `assign_index_codes()` en `generate_clean_notes_data.py` y `server.py`. Utiliza el formato `N.M`, donde `N` identifica la disciplina (1 = Civil, 2 = Procesal, 3 = Constitucional) y `M` es una secuencia continua ordenada por `(chapterNumber asc, parse_code_tuple(code) asc, orig_idx asc)`:
    ...
    * **Procesal (24 tópicos):** `2.1` a `2.24`.
...
* **Auto-Descubrimiento en Runtime y Auto-Seccionado de Fuentes (v7.9):**
  ...
  * **Invariante Canónica de 68 Tópicos (v7.13):** `discover_fuentes_files('fuentes')` retorna `[]` en el estado base del repositorio, asegurando que los 8 archivos registrados de `FILES_CONFIG` permanezcan 100% byte-estables e idénticos (34 Civil / 24 Procesal / 10 Constitucional). En v7.13 se incorporaron `DERECHO_PROCESAL_LA_PRUEBA.md` (14 cédulas, capítulos 3-5) y `PROCE ORDINARIO DE MAYOR CUANTIA.md` (cédula monolítica, capítulo 6); ambos se registran como fijos en `FILES_CONFIG` con capítulos propios para evitar colisiones de la tupla natural `(subject, chapterNumber, code)` con el canon procesal (capítulos 1-2), que la auto-detección v7.9 sí provocaría (validado empíricamente: 63 tópicos mutilados por la regla de prevalencia por `charCount`).
...
| `generate_clean_notes_data.py` | Parser / Sync | **Base canónica de apuntes con auto-descubrimiento (v7.9):** `discover_fuentes_files()` escanea deterministamente `fuentes/` en runtime con deduplicación insensible a mayúsculas contra `FILES_CONFIG`. `infer_file_config()` deduce materias por heurística ponderada y categoría. `extract_sections_from_file()` auto-secciona módulos (`N.1, N.2...`), headings Markdown (`##`) o bloques monolíticos con renumeración consecutiva sin huecos. Deduplicación canónica con `deduplicate_sections()`, numeración ordinal única por disciplina mediante `assign_index_codes()` (`indexCode`: Civil `1.1..1.34`, Procesal `2.1..2.24`, Constitucional `3.1..3.10`). Preserva 100% byte-idéntica la base canónica de 68 tópicos (34 Civil / 24 Procesal / 10 Constitucional) y regenera `all_afg_topics.json` y `js/data.js`. En v7.13 los archivos `DERECHO_PROCESAL_LA_PRUEBA.md` y `PROCE ORDINARIO DE MAYOR CUANTIA.md` se registran en `FILES_CONFIG` (capítulos 3-5 y 6 respectivamente) por estar pensados para evitar colisiones de capítulo con el canon procesal. |
| `fuentes/` | Base Canónica | **Carpeta canónica del temario.** Contiene los 8 apuntes oficiales (`ACTO JURIDICO.md`, `LOS BIENES.md`, `LAS OBLIGACIONES.md`, `CLASE_9_11.md`, `PROCESAL.md`, `CONSTITUCIONAL.md`, `DERECHO_PROCESAL_LA_PRUEBA.md`, `PROCE ORDINARIO DE MAYOR CUANTIA.md`) y admite nuevos archivos Markdown que son auto-descubiertos y auto-seccionados en runtime (registrados en `FILES_CONFIG` cuando sus capítulos requerirían no colisionar con el canon). Cualquier cambio aquí + `git push` regenera `all_afg_topics.json` y `js/data.js` en CI e implementa la página (GitHub Pages). |
...
| `test_e2e_case_flow.cjs` | Test E2E | Suite integral de 753 pruebas (100% PASS) que audita casos IA, seguridad, rúbrica AIME, confidencialidad, multi-dispositivo, limitación en Modo Demo vs. Pase Activo, canonización de fuentes, ordenamiento canónico `indexCode`, los 13 arquetipos con nutrición viva, la Subsección 23.6 con nutrición dinámica y citas corpus-driven (v7.10), la Subsección 24.4 con el Desarrollador de Preguntas de Verificación del Agente (`QuestionDeveloper`, v7.11), y la **Sección 25** con la sincronización multi-dispositivo del avance de cédulas (57 tests: `401`/`400`/`413`, LWW por cédula, multi-dispositivo, aislamiento, purga sin huérfanos y contrato de cliente con mocks, v7.12). |
