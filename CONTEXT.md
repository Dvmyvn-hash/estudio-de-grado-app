...
| `js/app.js` | Orquestador | Controlador principal de GRADOMANIACOS, navegación, gateo estricto del sidebar `#app-sidebar` y `#btn-toggle-sidebar` a la vista `topics`, sincronización autoritativa con `/api/sync-topics` sin acumulación zombi (`syncWithServer`), renderizado deduplicado y debounced con `replaceChildren` (`renderSidebar`), **orden de capítulos por min `indexCode` para respetar el temario canónico Discusión→Prueba→Sentencia (v7.19.1)**, **controlador del Buscador Único Centralizado en el Header (`setupGlobalSearch`, `initGlobalSearchMode`, `setGlobalSearchMode`, `updateGlobalSearchModeUI`, `executeGlobalSearch`, `renderSearchResults`, `renderRecentSearches`, `renderQAGaps`, `updateChapterOptions`, atajos `Ctrl+K`, `Enter`, `Esc`, conmutación automática a vista `topics`, soporte de filtros por materia `#vault-filter-pills` y capítulos `#vault-filter-chapter` en dropdown, historial en `localStorage['vault_recent_searches']`, motor extractivo Q&A y ranking local de temas pendientes en `#search-results-dropdown`, y stubs de retrocompatibilidad `setupVaultSearch` y `setupQAExtractor`) (v7.34, PROMPT 027)**, extensión de `App.openTopic(topicId, { highlight })` con resaltado del fragmento en `#topic-markdown-body` mediante `highlightInTopicViewer(snippet)` y `<mark class="vault-highlight">`, tooltips y breadcrumbs sanitizados, listado dinámico y reactivo de fuentes en `renderSourcesList()`, inyección del desarrollador de preguntas (`QuestionDeveloper`) en `renderTopicViewer()`, actualización reactiva de progreso y dominio en caliente, y auto-sincronizador de fuentes. **Gateo de Visibilidad del Cuestionario por Rol Admin (v7.18, PROMPT 015):** inyección de `#section-quiz-developer` condicionada a `isAdmin && isUnlocked` con insignia `.quiz-admin-lab-badge` en la cabecera del cuestionario, manteniendo el botón `#btn-bottom-mastery` / `#btn-toggle-mastery` visible y 100% operativo para todos los perfiles de usuario. **Sincronización de avance de cédulas (v7.12):** orquestador `pullMasteryProgress()` con guard de re-entrancia `_isPullingMastery` (hooks en `startLiveSync`, `syncWithServer` y `onAuthStateChanged`). Eliminado el cableado obsoleto de respaldo manual (`setupProgressBackup`) en v7.28. **Cuenta de Presentación Docente (v7.16):** login admin por rol (toasts diferenciados), `applyDocenteRestrictions()` que oculta `#admin-license-form`, `#admin-codes-panel` y `#btn-admin-manage-notes` mostrando `#docente-mode-banner`, y guards de click en `btnGenerate`, revocación e `openImportModal`. Los métodos de subida admin fueron eliminados en v7.8; se conserva el importador local `#btn-admin-manage-notes`. |
...
| `server.py` | Backend | Servidor HTTP Python multi-hilo, resolución dinámica de `DB_PATH`, deduplicación canónica en `get_all_synced_topics()` con base canónica `fuentes/`, libro mayor de usos auditables en `/api/admin/codes`, daemon de purga con doble inmunidad (`run_auto_purge_daemon`), ordenamiento canónico de `indexCode` en `/api/sync-topics`, endpoint `/api/fuentes` con escritura correcta de encabezados y cuerpo, endpoints de subida de apuntes eliminados (HTTP 404), Zero Exposure y rate limiting. **Persistencia de sesión, cookies dual-mode y preflight CORS (v7.28, PROMPT 022):** `make_session_cookie` y `make_logout_cookie` con contrato dual-mode (sin `Secure` en HTTP local, `Secure` obligatorio con `X-Forwarded-Proto: https`, `SameSite=None; Secure` para cross-site en allowlist), preflight `do_OPTIONS` (HTTP 204) con ACAO, ACAC y cabeceras permitidas en `/api/*`, e inyección automática de CORS en `send_json_response`. **Cuenta de Presentación Docente (v7.16):** mapa de roles `ADMIN_PIN_HASHES`, `verify_admin_pin()` en tiempo constante, `_require_admin_manage()` con `ADMIN_LIMITER` (429), 401 clave inválida y 403 modo docente. **Sincronización de avance de cédulas (v7.12):** endpoints `GET/POST /api/user/topic-mastery` con sesión obligatoria (401), lote máx. 500 y resolución LWW. |
