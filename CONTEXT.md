...
* **Vault de Conocimiento con Índice Semántico Estático (v7.22 → v7.25, PROMPTS 017 y 020):**
  Búsqueda semántica instantánea sobre las 103 cédulas canónicas sin base vectorial externa (sin ChromaDB, Qdrant ni LangChain), operativa idénticamente en servidor local (`server.py`) y en GitHub Pages estático. Se compone de un índice estático determinista (`build_vault_index.py` → `vault_index.json` de 535.42 KB ≤ 600 KB y `js/vault-index.js`) y un buscador cliente (`js/vault-search.js`) con scoring BM25-lite (`k1=1.2, b=0.75`), boosts temáticos (×1.3 palabras clave del temario canónico, ×1.2 títulos) y extracción de snippets verbatim (±140 caracteres) sanitizados mediante `SecurityShield.escapeHtml` y resaltados con `<mark class="vault-highlight">`.
  **En v7.25 (PROMPT 020 — Búsqueda que Encuentra):** El motor incorpora:
  - **Normalización Contractada Python ↔ JS:** Paridad estricta entre `normalize_text()` y `normalizeText()` manejando ligaduras (`ﬁ`, `ﬂ`, `ﬀ`, `ﬃ`, `ﬄ`, `æ`, `œ`), eliminación de guiones blandos (`\u00ad`) y espacios de ancho cero (`\u200b`), unificación de comillas y guiones tipográficos, remoción de tildes preservando siempre la letra `ñ`, y colapso de espacios múltiples.
  - **Sinónimos Jurídicos Curados:** Glosario de 33 grupos canónicos (`sinonimos.json` compilado deterministamente a `js/sinonimos.js` para carga estática) con expansión de consulta atenuada (factor ×0.8 para sinónimos y ×1.0 para literales), garantizando recall del 100% en top-5 sin alterar la precisión del top-1 literal.
  - **Filtros por Materia y Bloque Temático:** Pills de materia (`#vault-filter-pills`) y selector de capítulos (`#vault-filter-chapter`) ordenados por min `indexCode` según el temario canónico (v7.19.1), garantizando que el filtrado solo descarta y jamás reordena el ranking relativo.
  - **Historial Reciente Local:** Persistencia en `localStorage['vault_recent_searches']` (tope 10, FIFO, deduplicación case-insensitive, botón `#vault-history-clear`), chips interactivos sanitizados con `SecurityShield.escapeHtml` y aislamiento estricto (cero fuga en la exportación de avance de `StorageService`).
* **Vault de Conocimiento con Índice Semántico Estático (v7.22 → v7.25, PROMPTS 017 y 020):**
  Búsqueda semántica instantánea sobre las 103 cédulas canónicas sin base vectorial externa (sin ChromaDB, Qdrant ni LangChain), operativa idénticamente en servidor local (`server.py`) y en GitHub Pages estático. Se compone de un índice estático determinista (`build_vault_index.py` → `vault_index.json` de 535.42 KB ≤ 600 KB y `js/vault-index.js`) y un buscador cliente (`js/vault-search.js`) con scoring BM25-lite (`k1=1.2, b=0.75`), boosts temáticos (×1.3 palabras clave del temario canónico, ×1.2 títulos) y extracción de snippets verbatim (±140 caracteres) sanitizados mediante `SecurityShield.escapeHtml` y resaltados con `<mark class="vault-highlight">`.
  **En v7.25 (PROMPT 020 — Búsqueda que Encuentra):** El motor incorpora:
  - **Normalización Contractada Python ↔ JS:** Paridad estricta entre `normalize_text()` y `normalizeText()` manejando ligaduras (`ﬁ`, `ﬂ`, `ﬀ`, `ﬃ`, `ﬄ`, `æ`, `œ`), eliminación de guiones blandos (`\u00ad`) y espacios de ancho cero (`\u200b`), unificación de comillas y guiones tipográficos, remoción de tildes preservando siempre la letra `ñ`, y colapso de espacios múltiples.
  ...
  - **Filtros por Materia y Bloque Temático:** Pills de materia (`#vault-filter-pills`) y selector de capítulos (`#vault-filter-chapter`) ordenados por min `indexCode` según el temario canónico (v7.19.1), garantizando que el filtrado solo descarta y jamás reordena el ranking relativo.
  - **Historial Reciente Local:** Persistencia en `localStorage['vault_recent_searches']` (tope 10, FIFO, deduplicación case-insensitive, botón `#vault-history-clear`), chips interactivos sanitizados con `SecurityShield.escapeHtml` y aislamiento estricto (cero fuga en la exportación de avance de `StorageService`).
  En la UI hasta v7.28 se alojaba en el sidebar; a partir de **v7.34 (PROMPT 027)**, todo el motor de filtros (`#vault-search-filters`), selector de capítulos (`#vault-filter-chapter`), chips de historial (`#vault-search-history`), botón de limpieza (`#vault-search-clear`) y contenedor reactivo de resultados (`#vault-search-results`) se encuentra **100% unificado en el dropdown del header (`#search-results-dropdown`)**, habiéndose retirado definitivamente `#vault-search-box` y `#vault-search-input` del sidebar para mantener exactamente **un único input de búsqueda en todo el DOM** (`#global-search-input`).

* **Q&A Extractivo Puro: Pregunta → Citas Verbatim (v7.27, PROMPT 021):**
  Módulo client-side sin estado (`js/qa-composer.js`) que responde preguntas formuladas en lenguaje natural mediante **composición de citas verbatim (textuales) estrictas** de las 103 cédulas canónicas, sin generación generativa de texto (cero LLM, cero alucinaciones y latencia < 100 ms).
  - **Tres Modos de Composición Extractiva:**
    1. `definicion`: Selecciona la mejor cita puntual (top-1 de BM25) con su metadato canónico (`§ indexCode`, título y archivo de procedencia).
    2. `panorama`: Agrupa citas representativas ordenadas canónicamente por disciplina (`civil` $\to$ `procesal` $\to$ `constitucional`), ofreciendo una visión transversal sin repetición de materias.
    3. `comparativa`: Yuxtapone dos cédulas de distinta disciplina o bloque temático (etiquetadas *"Perspectiva A"* y *"Perspectiva B"*), permitiendo cotejar instituciones afines (ej. nulidad civil vs. nulidad procesal) **sin redactar conclusiones subjetivas ni texto propio**.
  - **Salto Interactivo con Resaltado en Destino:** Cada tarjeta `.qa-citation-card` incluye el botón `.qa-action-jump-btn` (`App.openTopic(id, { highlight: snippet })`). Al hacer clic, navega a la cédula, busca el fragmento textual en `#topic-markdown-body`, lo envuelve en `<mark class="vault-highlight">` sanitizado y realiza scroll suave centrado. Sin `highlight`, `App.openTopic()` preserva el scroll a `top: 0` (cero regresiones).
  - **Manejo Honesto de Falta de Cobertura y Sugerencia Dogmática:** Si una consulta no encuentra coincidencias (`empty: true`), despliega un mensaje transparente (*"Tus apuntes no cubren esto aún"*), sugiere la disciplina canónica más cercana según vocabulario temático (`inferSuggestedSubject()`) y la registra automáticamente en el ranking local de huecos.
  - **Ranking Local de Huecos (`localStorage['vault_uncovered_queries']`):** Registro persistente en cliente (máx. 30 elementos, deduplicado case-insensitive, incremento de frecuencia y botón de limpieza `#qa-gaps-clear`), para informar qué cédulas o apuntes incorporar a futuro. **Aislamiento estricto:** excluido al 100% de la exportación de avance de `StorageService.buildUserProgressExportPayload()`.
  - **Guardrails de Seguridad y Gate de Salida:** Toda cita, snippet y query pasa por `SecurityShield.escapeHtml`. Dispone de un **gate de salida estricto con falla cerrada**: si alguna cita no resuelve en el índice canónico vivo, la respuesta completa se degrada a estado vacío (`empty: true, reason: 'gate_failed'`). Prohibido redactar conclusiones o conectar LLMs.
  - **Integración en UI:** Hasta v7.28 en `#qa-container` del sidebar; a partir de **v7.34 (PROMPT 027)**, el selector de modo extractivo (`#qa-mode-pills` para `definicion`, `panorama`, `comparativa`), el panel reactivo de respuestas (`#qa-answer`) y el ranking de temas pendientes (`#qa-gaps-panel` con `#qa-gaps-list`, `#qa-gaps-clear`) quedan **integrados en el dropdown del header (`#search-results-dropdown`)**, eliminando del DOM `#qa-container`, `#qa-ask-input` y `#qa-ask-button`. El sidebar `aside#app-sidebar` queda exclusivamente reservado al índice canónico del temario (`#topics-tree-container`).

* **Buscador Global del Header con Modos Semántico y Q&A (v7.29, PROMPT 023):**
  Unificación del buscador superior (`#global-search-container`, `#global-search-input`, `#search-results-dropdown`, atajo `Ctrl+K`) incorporando un selector segmentador de modo (`#search-mode-selector`, `role="radiogroup"`) compuesto por píldoras conmutables:
  - **Modo Semántico (`#search-mode-semantic`):** Icono de lupa y color azul info (`--info-text: #0284c7`). Delega en el motor client-side `VaultSearch.searchVault()` retornando un listado priorizado de cédulas relevantes con título, disciplina, `indexCode` y snippets verbatim resaltados con `<mark class="vault-highlight">`.
  - **Modo Q&A (`#search-mode-qa`):** Icono de pregunta (`help-circle`) y color dorado de examen (`--gold-primary: #92400e`). Delega en el motor client-side `QAComposer.composeAnswer()` (modo `definicion`) retornando tarjetas de cita textual `.qa-citation-card` con metadatos de procedencia, botón de salto interactivo `.qa-action-jump-btn` y gate de salida estricto.
  - **Persistencia y Re-ejecución Reactiva:** El modo activo se persiste en `localStorage['global_search_mode']` (default: `'semantic'`). Al alternar de modo mediante clic, si el campo contiene $\ge 3$ caracteres, la consulta se re-ejecuta de inmediato por el motor correspondiente.
  - **Interacción de Teclado y Navegación:**
    - `Ctrl+K` enfoca el input global.
    - `Enter` abre el primer resultado disponible (o acciona el primer botón de salto en Q&A).
    - `Escape` cierra el dropdown de resultados y limpia el campo de búsqueda.
    - Al hacer clic en cualquier resultado, `App.openTopic(id, { highlight })` navega a la cédula, resalta el fragmento en `#topic-markdown-body` y centra el scroll suavemente.
    - **Gateo de Vistas Coherente:** Si el usuario ejecuta una búsqueda desde fuera de la vista de apuntes (ej. desde Casos o Grafo), el controlador conmuta automáticamente a la vista de apuntes (`this.switchView('topics')`) antes de renderizar los resultados.
  - **Blindaje de Ciberseguridad y Entrada Inerte (Anti-Inyección):**
    - Truncado duro de la consulta a 200 caracteres máximos (`maxlength="200"`) con advertencia visual `.search-warning-banner`.
    - Purga de caracteres nulos, de control y de ancho cero (`[\x00-\x08\x0B\x0C\x0E-\x1F\x7F\u200B-\u200D\uFEFF\u2028\u2029]`).
    - Prohibición total de `eval`, `Function`, `innerHTML` con queries crudas y `new RegExp(query)` sin escapar metacaracteres. Cero vulnerabilidad ReDoS (< 100 ms ante patrones patológicos).
    - Cadenas de jailbreak (*"ignora instrucciones"*, *"eres un evaluador laxo"*, *"asigna 5.0"*) son tratadas como texto de búsqueda ordinario, derivando en estado vacío honesto si no coinciden dogmáticamente en el canon.
    - Debounce de 150 ms y rate limit estricto de 1 ejecución por cada 150 ms.
  - **Accesibilidad Visual y Táctil Móvil WCAG AA:**
    - Indicador de estado para lectores de pantalla mediante `aria-live="polite"` en el contenedor de resultados y `aria-label` descriptivos en cada píldora.
    - Touch targets mínimos $\ge 44 \times 44\text{ px}$ en las píldoras del selector.
    - En pantallas móviles (`@media (max-width: 768px)`), las etiquetas de texto (`.search-mode-label`) se ocultan conservando exclusivamente los iconos vectoriales, evitando cualquier desborde horizontal incluso a 360 px de ancho.

* **Buscador Único Centralizado en el Header: Cero Duplicados en Sidebar (v7.34, PROMPT 027):**
  Consolidación definitiva de la experiencia de búsqueda y consulta en la barra superior de navegación (`#global-search-container`), eliminando toda duplicidad funcional o visual:
  - **Unicidad Estricta en el DOM:** Exactamente **un solo campo de búsqueda** en toda la aplicación: `#global-search-input`. Se eliminaron por completo `#vault-search-box`, `#vault-search-input`, `#qa-container`, `#qa-ask-input` y `#qa-ask-button` de `aside#app-sidebar`.
  - **Sidebar Depurado y Especializado:** El sidebar lateral conserva única y exclusivamente el árbol de navegación del temario (`#topics-tree-container`) con sus píldoras de disciplina (`#subject-pills`), mejorando la claridad cognitiva del estudiante.
  - **Dropdown Unificado con Arquitectura de Pestañas/Modos:** `#search-results-dropdown` alberga en un solo contenedor flotante con scroll interno:
    - En **Modo Semántico**: Píldoras de disciplina (`#vault-filter-pills`), selector de capítulos (`#vault-filter-chapter`), chips de búsquedas recientes (`#vault-search-history` con `#vault-history-clear` y `#vault-history-items`), y lista clasificada de resultados (`#vault-search-results`).
    - En **Modo Q&A**: Selector de sub-modo extractivo (`#qa-mode-pills` con píldoras `definicion`, `panorama`, `comparativa`), contenedor reactivo de respuestas textuales (`#qa-answer`), y panel de huecos no cubiertos (`#qa-gaps-panel` con `#qa-gaps-list` y `#qa-gaps-clear`).
  - **Responsive Móvil Impecable (360 px):**
    - En pantallas $\le 768\text{ px}$, el header wrap permite que `.search-bar` ocupe una fila dedicada al 100% de ancho (`order: 4; margin-top: 6px;`).
    - El dropdown de resultados `.search-dropdown` se posiciona como overlay fijo de viewport completo (`position: fixed; top: 104px; left: 8px; right: 8px; max-width: calc(100vw - 16px); max-height: calc(100vh - 120px);`), eliminando scrolls horizontales accidentales.
    - Touch targets mínimos $\ge 44 \times 44\text{ px}$ en botones de limpieza, chips, píldoras y enlaces de navegación.
  - **Preservación Total de Capacidades:** Búsqueda instantánea BM25-lite, snippets verbatim, saltos a cédula con resaltado `<mark class="vault-highlight">`, conmutación automática de vista hacia `topics` (`this.switchView('topics')`), atajos `Ctrl+K`, `Enter`, `Escape`, sanitización anti-XSS y cuota de 200 caracteres intactos al 100%.
...
* **Diferenciación Canónica entre `code` e `indexCode`, Orden Temario Canónico y Crecimiento Dinámico del Canon (v7.2 / v7.19 / v7.19.1 / v7.30):**
  ...
  * **Rangos Base y Protocolo de Crecimiento del Canon (v7.30, PROMPT 024; crecimiento v7.40):**
    Los conteos y rangos por materia no son verdades estáticas sino derivados dinámicamente de `all_afg_topics.json` y del temario canónico (`CANON_ESPERADO = all_afg_topics.length`):
    * **Civil (`CANON_CIVIL`, base 34 → 136 tópicos desde v7.40):** `1.1` a `1.M` (`1.136` actual) siguiendo la dogmática canónica:
      ...
      * `1.35` a `1.39`: Contrato de Arrendamiento (`CONTRATOARRENDAMIENTO-pulido.md`, cap 12, 5 cédulas, v7.40).
      * `1.40` a `1.66`: Contrato de Comodato (`CONTRATOCOMODATO-pulido.md`, cap 13, 27 cédulas, v7.40).
      * `1.67` a `1.90`: Contrato de Mutuo (`CONTRATOMUTUO-pulido.md`, cap 14, 24 cédulas, v7.40).
      * `1.91` a `1.118`: Contrato de Mandato (`CONTRATOMANDATO-pulido.md`, cap 15, 28 cédulas, v7.40).
      * `1.119` a `1.136`: Derechos Reales de Garantía — Hipoteca (`CONTRATOHIPOTECA-pulido.md`, cap 11, 18 cédulas, v7.40).
      * Nuevos apuntes civiles (ej. Sucesiones cap 9, Familia cap 10) se intercalan según su orden en `TEMARIO_CANONICO['civil']` expandiendo la secuencia `1.1..1.M` de forma continua y sin huecos.
    * **Procesal (`CANON_PROCESAL`, base 59 tópicos):** `2.1` a `2.M` (`2.59` base) ordenado dogmáticamente según la secuela del Juicio Ordinario de Mayor Cuantía (**Discusión → Prueba → Sentencia**, v7.19.1):
    ...
    * **Constitucional (`CANON_CONST`, base 10 tópicos):** `3.1` a `3.M` (`3.10` base) (Acciones y Garantías Constitucionales `3.1`-`3.4`, Derechos Fundamentales Art. 19 CPR `3.5`-`3.10`).
  * **Invariantes Duros de Crecimiento del Canon:**
    1. **Unicidad absoluta de `indexCode`:** cero duplicados de `indexCode` dentro de una misma disciplina o entre cédulas.
    2. **Continuidad estricta `N.M` sin huecos:** desde `N.1` hasta `N.total` sin saltos secuenciales para toda materia.
    3. **Orden relativo dogmático inmutable:** en Procesal, Discusión siempre precede a La Prueba, y La Prueba siempre precede a Sentencia.
    4. **Resiliencia de suites ante crecimiento:** todas las pruebas automatizadas calculan `CANON_ESPERADO` a partir de `all_afg_topics.json`, eliminando aserciones con números mágicos (`103`, `34`, `59`, `10`) y pasando al 100% tanto en el estado base como al agregar apuntes nuevos.
...
* **Vault de Conocimiento con Índice Semántico Estático (`vault_index.json` / `VAULT_INDEX`) (v7.22; presupuestos escalables v7.40):**
  * **Estructura del Índice:** Objeto estático generado por `build_vault_index.py` (828.23 KB con canon 205; presupuesto efectivo `max(600 KB, totalDocs × 6 KB)` = 1230 KB) con claves:
    * `totalDocs`: 205 (total de cédulas canónicas; 136 civil + 59 procesal + 10 constitucional).
...
* **Auto-Ordenamiento por Temario Canónico del Examen de Grado (v7.19, PROMPT 016; corrección dogmática + sidebar v7.19.1):**
  ...
  * **Protocolo de Crecimiento del Canon y Resolución de Choques (`colision-canon`) (v7.30, PROMPT 024):**
    * **Regla Inmutable de Respeto al Canon:** El depósito de un apunte `.md` en la carpeta `fuentes/` amplía el canon temático dinámicamente sin requerir modificaciones en apuntes preexistentes ni romper suites de pruebas. Si `scripts/lint_fuentes.py` detecta un error de tipo `colision-canon` (colisión de la tupla natural `(subject, chapterNumber, code)` con el canon existente), **la solución NUNCA es mutilar o modificar los apuntes canónicos consolidados**. La solución canónica es registrar el nuevo archivo en `FILES_CONFIG` dentro de `generate_clean_notes_data.py` con un capítulo propio o mapeo de categorías (`categoryMap`), preservando la integridad histórica y dogmática de todas las cédulas (precedente v7.14: `PROCEMAYORCUANTIA-pulido.md` y `DERECHO_PROCESAL_LA_PRUEBA.md`).
    * **Resiliencia de Pruebas ante Crecimiento:** Se eliminan definitivamente aserciones con números mágicos (`103`, `34`, `59`, `10`) en todas las suites de prueba (`test_deduplication_flow.cjs`, `test_e2e_case_flow.cjs`, `test_mocks_factory.cjs`). Los conteos esperados se derivan dinámicamente de `all_afg_topics.json` (`CANON_ESPERADO`, `CANON_CIVIL`, `CANON_PROCESAL`, `CANON_CONST`) verificando simultáneamente los invariantes duros: unicidad absoluta de `indexCode`, continuidad estricta `N.1..N.M` sin huecos secuenciales, y orden relativo dogmático inmutable (en Procesal: Discusión < Prueba < Sentencia).
...
1. **Modo Servidor Local / Producción (`server.py` y `db.py`):**
   ...
   * **Sesiones Seguras y Cookies Dual-Mode (v7.28, PROMPT 022):**
      - Emisión de tokens firmados con **HMAC-SHA256** persistidos en cookies `session_token`.
      - **Contrato Dual-Mode de Cookies:**
        * En conexiones HTTP locales (`localhost`, `127.0.0.1`), la cookie se emite como `SameSite=Lax; HttpOnly; Path=/` **sin** la bandera `Secure`, evitando que los navegadores modernos la descarten por falta de cifrado.
        * En conexiones HTTPS (detectadas nativamente o vía cabecera `X-Forwarded-Proto: https` en Render), la cookie incluye imperativamente la bandera `Secure`.
        * En peticiones cross-site (Pages → Render) cuyo `Origin` pertenezca a la lista autorizada (`ALLOWED_ORIGINS` / `auth_config.json` o subdominios `*.github.io`), la cookie se emite con `SameSite=None; Secure`.
        * Orígenes no autorizados o peticiones HTTP sin HTTPS degradan de forma segura a `SameSite=Lax` sin `Secure`.
      - **Preflight CORS y Encabezados en `/api/*`:** El servidor responde preflight `OPTIONS` con **HTTP 204 No Content**, inyectando `Access-Control-Allow-Origin: <origin>` eco, `Vary: Origin`, `Access-Control-Allow-Credentials: true`, `Access-Control-Allow-Methods: GET, POST, OPTIONS` y cabeceras permitidas incluyendo `X-Admin-PIN` y `Content-Type`. Toda respuesta JSON de `/api/*` inyecta automáticamente estas cabeceras CORS si el origen está permitido.
      - **Erradicación Definitiva de la "Cuenta Fantasma":** Ante una respuesta HTTP 401 explícita del servidor en `/api/auth/me`, `checkSession()` elimina de inmediato `grado_auth_user` de `localStorage`, revoca licencias activas y retorna `null`, garantizando una transición limpia a Modo Demo sin residuos visuales ni estados inconsistentes. El fallback a caché local queda estrictamente condicionado a excepciones de red (`networkError === true`).
      - **Inclusión Universal de Credenciales:** Todos los fetch de la capa de autenticación (`js/auth-service.js`) y de sincronización de usuario (`js/storage.js`) incorporan explícitamente `credentials: "include"`.
      - **Retiro de UI de Respaldo Manual y Blindaje Prototype-Pollution:** Se suprime el puente manual `.sidebar-progress-actions` (`#btn-export-progress`, `#btn-import-progress`, `#input-import-progress`) y su cableado en `js/app.js`, conservando intacta la capa de servicio `StorageService.mergeUserProgressFromJsonString` con validación estricta anti-prototype-pollution (`__proto__`, `constructor`, `prototype`).
...
| `/api/auth/logout` | `POST` | Cookie opcional | `200` | **Cierre de Sesión:** Invalida y expira la cookie `session_token` con encabezado `Set-Cookie: session_token=; Max-Age=0` respetando el contrato dual-mode (v7.28). |
| `/api/*` (Preflight) | `OPTIONS` | Pública | `204` | **Preflight CORS:** Responde **HTTP 204 No Content** con `Access-Control-Allow-Origin: <origin>` eco, `Vary: Origin`, `Access-Control-Allow-Credentials: true`, métodos `GET, POST, OPTIONS` y cabeceras permitidas incluyendo `X-Admin-PIN` y `Content-Type` para orígenes autorizados (v7.28). |
...
| `/api/admin/create-code-batch` | `POST` | PIN Admin Requerido (`X-Admin-PIN` o body `pin`) — rol pleno `admin` | `201`, `400`, `401`, `403`, `429`, `500` | **Crear Lote de Códigos de Acceso (Admin, v7.35):** Generación masiva atómica all-or-nothing de 1 a 50 códigos (`1 <= count <= 50`). Valida clave y rol pleno `admin` vía `_require_admin_manage` (401 PIN inválido, 403 rol `docente`, 429 limiter). Acepta `count`, `prefix` (sanitizado a `^[A-Z0-9_\-]{1,16}$`), `label`, `assigned_email` (o `email`), `max_uses` y `days`. Ejecuta en una sola transacción SQLite con reintentos anti-colisión usando `[A-Z2-9]` no ambiguos. Retorna `{ ok: true, count, codes: [...], items: [...] }`. |
| `/api/admin/purge-codes` | `POST` | PIN Admin Requerido (`X-Admin-PIN` o body `pin`) — rol pleno `admin` | `200`, `400`, `401`, `403`, `429`, `500` | **Purgar Stock Ocioso de Códigos (Admin, v7.35):** Limpieza segura de códigos que no representen derechos adquiridos ni auditoría histórica. Valida clave y rol pleno `admin` vía `_require_admin_manage`. Soporta `dry_run` (booleano, por defecto `true`), `older_than_days` y `prefix`. Elimina **exclusivamente** códigos activos (`active = 1`) que cumplan copulativamente: `times_used = 0`, `assigned_email IS NULL OR ''`, sin usuario vinculado en `users` y sin registros en el libro mayor `access_code_usages`. Preserva estrictamente códigos revocados (`active = 0`) para auditoría y códigos pre-asignados a compradores. Retorna `{ ok: true, dry_run, count, codes: [...], purged: [...] }`. |
| `/api/admin/update-code-label` | `POST` | PIN Admin Requerido (`X-Admin-PIN` o body `pin`) — rol pleno `admin` | `200`, `400`, `401`, `403`, `404`, `429`, `500` | **Actualizar Etiqueta / Estado de Código (Admin, v7.35):** Permite alternar ágilmente el estado operativo de un código (`[PENDIENTE]` $\leftrightarrow$ `[ENTREGADO]`) o actualizar notas del alumno sin tocar la estructura de la base de datos. Valida clave y rol pleno `admin` vía `_require_admin_manage`. Acepta `code` y `label` (string $\le 200$ caracteres). Actualiza en SQLite y retorna `{ ok: true, code, label }`. |
...
| `#admin-batch-count` | Input Cantidad (v7.35) | Selector de cantidad para emisión en lote (1 a 50 códigos). |
| `#admin-custom-code` | Input Código | Código personalizado (o autogenerado si se deja vacío; deshabilitado cuando batch count > 1). |
...
| `#btn-generate-code` | Botón Submit | Crea la licencia unitaria o en lote vía `POST /api/admin/create-code` o `/api/admin/create-code-batch`. |
| `#admin-codes-filter-status` | Selector Filtro (v7.35) | Filtro client-side de códigos por estado operativo (`all`, `pending`, `delivered`, `unused`). |
| `#btn-admin-purge-codes` | Botón Acción (v7.35) | Dispara la purga de stock ocioso con previsualización dry-run interactiva y confirmación explícita. |
| `.admin-codes-table` / `#admin-codes-tbody` | Tabla | Historial de códigos emitidos activos con detalle de usos, estado operativo (`.code-status-badge`) y auditoría `usages`. |
| `.code-status-badge` | Badge Interactivo (v7.35) | Insignia de estado operativo (`[PENDIENTE]` en `.badge-pendiente`, `[ENTREGADO]` en `.badge-entregado`, o `.badge-neutral`) con conmutación en 1 clic. |
...
### 7.3. Arquitectura de Venta Manual Ágil: Lotes, Pre-asignación y Estados Operativos (v7.35, PROMPT 028)

Con el fin de agilizar las ventas directas por canales manuales (transferencia bancaria vía Instagram DM con las plantillas de `VENTAS/plantillas_dm.md`) reduciendo el tiempo operativo a menos de 5 minutos, la plataforma introduce cuatro pilares fundamentales de administración sin requerir pasarelas de pago, sin relajar el control humano y con **cero migraciones de base de datos** (reutilizando los campos canónicos `assigned_email` y `label` de `access_codes`):

1. **Generación en Lote Atómica y Alfanumérica Segura (`create_access_code_batch`):**
   * **Transacción All-or-Nothing:** El administrador puede generar entre 1 y 50 códigos en una única invocación (`POST /api/admin/create-code-batch`), ejecutada bajo una transacción SQLite (`with self.conn:`) donde la falla de inserción o validación en cualquier ítem aborta completamente el lote, previniendo estados inconsistentes o códigos huérfanos.
   * **Entropía y Caracteres No Ambiguos:** Los códigos son generados mediante `generate_random_access_code(prefix)` utilizando el alfabeto seguro `[A-Z2-9]` (excluyendo caracteres visualmente confusos como `0`, `O`, `1`, `I`), con un loop de reintentos anti-colisión garantizando unicidad en SQLite.
   * **Parámetros Homogéneos:** Todo el lote hereda el prefijo configurado (sanitizado a `^[A-Z0-9_\-]{1,16}$`, ej. `LOTE-PROCESAL-`), materias liberadas (`scope`), días de vigencia (`days`), límite de usos (`max_uses`) y etiqueta base con prefijo de estado.

2. **Pre-asignación Estricta de Correo (`assigned_email`) como Escudo Anti-Préstamo:**
   * Tanto en generación unitaria como en lote, se puede asociar preventivamente el correo electrónico del comprador en `access_codes.assigned_email`.
   * **Enforcement en Registro y Convalidación:** Durante `POST /api/auth/link-code` (y `create_user` si se registra con código previo), `db.link_user_code` y `db.create_user` verifican si `assigned_email` está presente. Si el correo de la cuenta del postulante no coincide de forma insensible a mayúsculas con `assigned_email`, la operación es rechazada inmediatamente con **HTTP 400 Bad Request** y el mensaje pedagógico *"Este código de acceso está asignado exclusivamente al correo <assigned_email>"*.
   * **Compatibilidad con Stock Genérico:** Si `assigned_email` es nulo o vacío, el código opera con normalidad permitiendo su convalidación por cualquier usuario válido.

3. **Purga Segura y Defensiva de Stock Ocioso (`purge_unused_codes`):**
   * **Dry-Run por Defecto:** El endpoint `POST /api/admin/purge-codes` opera con `dry_run = True` por defecto, permitiendo al frontend calcular y previsualizar exactamente cuántos y cuáles códigos califican para borrado antes de ejecutar cualquier mutación destructiva.
   * **Cuádruple Condición Excluyente:** Para ser purgado, un código debe cumplir simultáneamente:
     $$\text{times\_used} = 0 \quad\land\quad \text{active} = 1 \quad\land\quad \text{assigned\_email IS NULL OR ''} \quad\land\quad \text{code} \notin \text{users} \quad\land\quad \text{code} \notin \text{access\_code\_usages}$$
   * **Blindaje Inviolable:** Quedan estrictamente protegidos e inmunes ante la purga: (a) códigos con usos registrados ($\ge 1$), (b) códigos vinculados a cuentas de usuario, (c) códigos pre-asignados a alumnos compradores, y (d) códigos revocados (`active = 0`), los cuales deben preservarse permanentemente con fines de auditoría histórica e investigación de disputas.

4. **Convención de Estados Operativos en `label` y UI Ágil:**
   * **Convención Semántica:** Los códigos creados para venta adoptan el prefijo `[PENDIENTE]` (generado, pendiente de entrega al comprador) o `[ENTREGADO]` (enviado por DM tras confirmación de transferencia).
   * **Conmutación en 1 Clic (`update_access_code_label`):** En la tabla de licencias, la columna *Estado* despliega insignias táctiles interactivas (`.code-status-badge` en verde `.badge-entregado`, naranja `.badge-pendiente` o neutro `.badge-neutral`). Un clic sobre la insignia alterna atómicamente el estado entre `[PENDIENTE]` y `[ENTREGADO]` vía `POST /api/admin/update-code-label`, sin necesidad de recargar la página ni abrir diálogos modales complejos.
   * **Filtro Client-Side en Tiempo Real:** El selector `#admin-codes-filter-status` permite filtrar instantáneamente la tabla entre *Todos los estados*, *Pendientes de entrega* (`[PENDIENTE]`), *Entregados* (`[ENTREGADO]`) y *Sin usar / Stock* (`times_used = 0`), facilitando la gestión de inventario sin sobrecargar el servidor.

---

### 7.4. Arquitectura del Visor de Apuntes: Tablas Comparativas y Escala Tipográfica Canónica (v7.36, PROMPT 029)

A raíz de auditorías visuales sobre dispositivos de escritorio (1440 px) y móviles (360 px), se diagnosticaron y resolvieron fallos estructurales en la visualización de apuntes jurídicos:
1. **Recuperación Total de Tablas Comparativas (`MarkdownParser.parseTables`):**
   * **Causa Raíz:** En la extracción de apuntes desde fuentes PDF/Markdown, se insertaban líneas en blanco y números de página intermedios entre las filas de las tablas. La versión previa de `MarkdownParser` abortaba el parseo tabular ante cualquier línea vacía, degradando todas las 40 tablas del canon a párrafos rotos (`| col 1 | col 2 |`).
   * **Algoritmo Look-Ahead Resiliente:** El parser ahora detecta bloques de tabla e inspecciona líneas subsiguientes omitiendo líneas vacías intermedias si la siguiente línea no vacía continúa con formato de celdas (`|...|`), cerrando la tabla únicamente cuando finaliza efectivamente la estructura tabular. Las 40 tablas comparativas del canon ahora renderizan como etiquetas `<table>` semánticas reales.
   * **Accesibilidad WCAG AA y Sanitización de Saltos:** La envoltura `.table-responsive` se genera con atributos accesibles `tabindex="0"`, `role="region"` y `aria-label="Tabla comparativa de estudio"`, permitiendo su navegación mediante foco por teclado y lectores de pantalla. Las entidades escapadas (`&lt;br&gt;` y `&amp;lt;br&amp;gt;`) se restituyen a etiquetas `<br>` válidas para saltos de línea legibles dentro de cada celda.

2. **Desplazamiento Horizontal Autónomo y Eliminación de Recortes Ancestrales (`css/topic-viewer.css`):**
   * **Desplazamiento Independiente:** `.table-responsive` cuenta con `overflow-x: auto` y `-webkit-overflow-scrolling: touch`, garantizando que tablas densas con múltiples columnas comparativas se desplacen fluidamente sin alterar el ancho del documento ni desbordar la ventana.
   * **Eliminación de Recorte Ancestral:** Se eliminó la regla `overflow-x: hidden` en `.topic-reading-col` dentro de la media query móvil (`@media (max-width: 1024px)`), la cual impedía el scroll natural de los contenedores secundarios.
   * **Anchos Mínimos Defensivos y Prevención de Aplastamiento Silábico:** Se definió `min-width: 580px` en PC y `min-width: 520px` en móvil para `.reading-table`, con anchos mínimos por columna (`th`: 140 px, primera columna: 150 px, celdas de datos: 180 px). Se fijó `word-break: normal;` y `hyphens: auto;`, eliminando de raíz la rotura vertical de sílabas ("pro-ce-den-cia" a una letra por línea) y suprimiendo la regla lesiva `table { word-break: break-word }`.
   * **Cero Desborde Horizontal Global:** Auditorías automatizadas con Chrome headless confirman que en viewport de 360 px, `document.body.scrollWidth === window.innerWidth === 360`, asegurando cero desborde horizontal a nivel de página (`bodyOverflow = false`).

3. **Escala Tipográfica Canónica de 5 Pasos (`css/main.css`):**
   Para garantizar confort visual sostenido y jerarquía armónica en jornadas extensas de estudio, se consolidó en `:root` una escala estricta de interlineado proporcional:
   * `--lh-title: 1.28;`: Títulos mayores, H1 de cédula (`.topic-h1-title`) y encabezados H2 (`.reading-h2`).
   * `--lh-heading: 1.35;`: Subtítulos y apartados H3 (`.reading-h3`), H4 y H5.
   * `--lh-body: 1.70;`: Cuerpo de lectura principal (`.reading-p`, listas ordenadas/desordenadas y bloques explicativos).
   * `--lh-table: 1.60;`: Celdas y encabezados de tablas comparativas (`.reading-table th, td`), optimizando densidad y legibilidad.
   * `--lh-dense: 1.50;`: Notas al pie, metadatos, citas en bloque (`blockquote`) y tarjetas compactas (`.callout-card`).

---

### 7.5. Arquitectura del Shell Óptimo, Jerarquía de Capas y Accesibilidad Universal WCAG AA (v7.37, PROMPT 030)

Tras una auditoría visual y de métricas forenses comparativa entre escritorio (1440 px) y teléfono móvil (360 px), se diagnosticaron y resolvieron 12 defectos visuales y de accesibilidad estructurales, implementándose una refactorización de alto rendimiento sin mutaciones al diseño identitario *Obsidian & Gold*:

1. **Tokens Canónicos de Espaciado 8pt y Z-Index Unificado en `:root` (`css/main.css`):**
   * **Escala 8pt:** `--space-1: 4px;` (micro espaciado), `--space-2: 8px;` (chips y badges compactos), `--space-3: 12px;` (relleno de controles), `--space-4: 16px;` (separación base estándar), `--space-5: 24px;` (secciones medianas), `--space-6: 32px;` (márgenes de tarjetas), `--space-7: 48px;` (separación mayor), `--space-8: 64px;` (cabeceras y pies amplios).
   * **Jerarquía de Capas Inviolable (header < dropdown < modal < toast):**
     $$\text{--z-base (1)} < \text{--z-surface (10)} < \text{--z-sidebar (40)} < \text{--z-header (50)} < \text{--z-dropdown (100)} < \text{--z-overlay (200)} < \text{--z-sidebar-mobile (250)} < \text{--z-modal (500)} < \text{--z-toast (1000)}$$
   * **Resolución de Inversión Dropdown vs. Modal Backdrop:** El dropdown `#search-results-dropdown` (anteriormente fijado en `1000` en PC y `1200` en móvil) fue reposicionado en `var(--z-dropdown)` (100), mientras que el backdrop de modales `.modal-backdrop` (anteriormente en `200`) fue elevado a `var(--z-modal)` (500). Esto garantiza que cualquier diálogo modal (convalidación, desbloqueo o importación) quede permanentemente sobre dropdowns y cabeceras sin superposiciones erróneas ni conflictos de clic.

2. **Contención de Desplazamiento (*Scroll Containment*) y Centrado de Lectura:**
   * **Aislamiento de Scroll:** Se implementó `overscroll-behavior: contain;` en `.app-sidebar`, `.sidebar-tree` y `.topic-reading-col`, eliminando el encadenamiento de desplazamiento (*scroll chaining*) donde desplazarse hasta el final del árbol temático o del apunte transfería el movimiento al cuerpo de la ventana.
   * **Columna de Lectura Centrada:** En pantallas anchas (PC), `.topic-reading-col` fija `margin: 0 auto; max-width: 940px;`, garantizando una columna de lectura ergonómica y perfectamente centrada.

3. **Optimización de Rendimiento de Renderizado en Árbol Temático (`css/sidebar.css`):**
   * Se aplicó `content-visibility: auto; contain-intrinsic-size: 0 44px;` en `.category-group`. El navegador evita pintar y calcular layout para los capítulos y cédulas que se encuentran fuera de la vista en el desplazamiento del sidebar, acelerando drásticamente el tiempo de rendering y manteniendo 60 FPS en el scroll de los 103 temas del temario.

4. **Prevención de Saltos de Diseño (*Zero Layout Shifts*):**
   * **Insignia de Contador de Casos (`#case-count-badge`):** Se definió `min-width: 20px; display: inline-flex; align-items: center; justify-content: center; box-sizing: border-box;` (y `min-width: 18px` en móvil), evitando el desplazamiento o salto horizontal de la pestaña adyacente `Grafo` cuando el número de casos muta de 0 a un dígito o a dos dígitos.
   * **Columna Derecha de Cédulas (`.topic-item-right`):** Se fijó `min-width: 24px; justify-content: flex-end;`, asegurando que la presencia o ausencia de insignias de cobertura o candados no altere la longitud de los títulos ni provoque vibración en hover.

5. **Accesibilidad Semántica ARIA y Sincronización Reactiva:**
   * **Navegación Superior:** `.header-nav` adopta `role="tablist"`, y cada botón de vista `.nav-tab` adopta `role="tab"` con `aria-selected="true"` en la pestaña activa y `"false"` en las inactivas, sincronizado reactivamente en `App.switchView()`.
   * **Botón de Menú Lateral:** `#btn-toggle-sidebar` incorpora el atributo dinámico `aria-expanded="false"` / `"true"`, actualizado de forma instantánea al abrir, colapsar o restaurar el drawer lateral en `App.setupNavigation()`.
   * **Cédula Activa:** El elemento correspondiente a la cédula en estudio dentro del árbol temático (`.topic-tree-item.active`) recibe el atributo estándar `aria-current="true"`.
   * **Iconografía:** Todos los iconos decorativos (`<i>` de Lucide) incorporan `aria-hidden="true"`.

6. **Accesibilidad Universal WCAG AA y Feedback Unificado:**
   * **Focus Visible Universal:** `:focus-visible { outline: 2px solid var(--gold-primary, #d4af37); outline-offset: 2px; }` asegura un indicador de enfoque visible y de alto contraste en navegación por teclado (Tab) sin alterar la interacción con ratón (`:focus:not(:focus-visible) { outline: none; }`).
   * **Preferencia de Movimiento Reducido:** `@media (prefers-reduced-motion: reduce)` inhabilita transiciones y animaciones complejas (`animation-duration: 0.01ms !important; transition-duration: 0.01ms !important; scroll-behavior: auto !important;`) para usuarios sensibles a efectos vestibulares o cinetosis.
   * **Componentes de Estado Unificados:** Creación de `.ui-skeleton` (efecto shimmer con gradiente animado en CSS puro), `.ui-state-empty` (con icono, título y descripción amigable), `.sidebar-empty-state` y `.ui-state-error` (con borde de alerta y soporte de reintento).
   * **Restauración de Foco en Búsqueda:** Al presionar `Escape` con el dropdown de búsqueda abierto, el menú se oculta de inmediato y el foco se restituye limpiamente al campo `#global-search-input`.

---

### 7.6. Arquitectura de Cruces Dogmáticos Enriquecidos, Citas Textuales del Canon y Salto con Highlight (v7.38, PROMPT 031)

Tras una auditoría forense de la Fase 0 (`scripts/audit_crossovers_phase0.py`), se descubrió la causa raíz por la cual el panel lateral de Cruces Dogmáticos y el acordeón inline mostraban un estado vacío o plantillas genéricas: los 80 cruces manuales en `build_dogmatic_connections.py` empleaban prefijos de ID acortados o con espacios (`civil-acto-1-1`, `civil-clas-2-1`, `civil-las -1-1`, `civil-los -1-1`, `procesal-proc-1-1`) en lugar de los identificadores canónicos de `all_afg_topics.json` (`civil-actojuridi-1-1`, `civil-clase911-2-1`, `civil-lasobligac-1-1`, `civil-losbienes-1-1`, `procesal-derechopro-1-1`). Al no coincidir los IDs, el 100% de los temas quedaba con `connections: []`, activando en runtime plantillas estáticas con texto genérico y la categoría vacía "Materia Afín".

Para resolver esto con rigor dogmático y sin dependencias de LLM ni llamadas a APIs externas en tiempo de ejecución, se implementó una arquitectura determinista en dos capas:

1. **Motor Determinista Offline (`scripts/build_cross_connections.py` y `build_dogmatic_connections.py`):**
   * **Resolución de Identificadores Heredados:** Función `resolve_legacy_id()` que mapea de forma determinista los 80 cruces manuales originales contra los IDs reales del canon de 103 temas, preservándolos como *authoritative overrides*.
   * **Descubrimiento Guiado por Vault BM25 y Compuerta de Bigramas ($\ge 3$):** Para completar la cobertura temática al 100%, el motor indexa las 103 cédulas canónicas y calcula similitud textual ponderando el cruce interdisciplinario (Civil $\leftrightarrow$ Procesal, Civil $\leftrightarrow$ Constitucional). Solo se admiten conexiones que superen estrictamente la compuerta de al menos 3 bigramas compartidos (`shared_bigrams >= 3`, con un promedio real de 125 bigramas compartidos).
   * **Taxonomía Cerrada de 6 Categorías Dogmáticas:** Se erradicó la taxonomía abierta y libre. Toda conexión pertenece a uno de 6 tipos jurídicos precisos: `Genero-Especie`, `Sustantivo-Procesal`, `Fundamento-Constitucional`, `Excepcion-Procesal`, `Efecto-Patrimonial`, `Materia-Afin`.
   * **Citas Textuales Verbatim del Canon ($\le 140$ caracteres):** Cada cruce extrae obligatoriamente un fragmento textual exacto (`quote`) del contenido real de la cédula destino (`targetTopic.content`). Cero citas alucinadas; el 100% de las citas existe literalmente en el apunte.
   * **Presupuesto y Métricas de Generación:** Genera `dogmatic_connections.json` (274.01 KB $\le 300$ KB) y `cruces_report.json` con 103/103 temas cubiertos (100%), exactamente 3 cruces por tema (309 en total), 0 auto-enlaces, 0 destinos duplicados y 100% de targets válidos en el canon.

2. **Renderizado en Frontend y Contrato de Salto Interactivo (`js/app.js`, `css/topic-viewer.css`):**
   * **Helper Unificado `renderConnectionCardsHtml(connsList)`:** Elimina la duplicación de plantillas entre el acordeón inline (`.inline-connections-box`) y el panel lateral derecho (`#topic-aside-panel`), asegurando coherencia visual absoluta.
   * **Erradicación Total de Plantillas Genéricas:** Se eliminó cualquier texto residual del tipo `"Institución directamente relacionada con el estudio y profundización de..."`. Si no hay conexiones validadas, se despliega un estado vacío honesto.
   * **Componentes de Interfaz Enriquecidos:**
     - `.connection-target-meta`: Despliega `§ indexCode · Disciplina` en la cabecera de la tarjeta.
     - `.connection-quote-box`: Despliega la cita textual entre comillas con `<mark class="vault-highlight">` y la referencia al código de sección.
     - `.btn-jump-connection`: Botón de acción con dimensiones táctiles accesibles ($\ge 44 \times 44\text{ px}$ en móvil) y atributos `data-target-id` y `data-snippet`.
   * **Navegación Interactiva con Resaltado y Blindaje Anti Open-Redirect:**
     - Al pulsar el botón de salto o la tarjeta de cruce, se invoca `App.openTopic(targetId, { highlight: snippet })`.
     - `App.highlightInTopicViewer(snippet)` localiza mediante `TreeWalker` el nodo de texto en `#topic-markdown-body`, lo envuelve en `<mark class="vault-highlight">` y realiza un desplazamiento suave centrado hacia el texto resaltado.
     - Compuerta de seguridad: Antes de navegar, se valida estrictamente que `targetId` exista en el allowlist de las 103 cédulas canónicas del `StorageService.getData().topics`, neutralizando cualquier intento de inyección de rutas o redirección externa.
   * **Empty State Honesto con Sugerencias del Vault:**
     - Si una cédula personalizada o sin cruces validados tuviera 0 conexiones, se despliega `.connections-empty-state` con el aviso `"Sin cruces validados aún"` y tarjetas `.unvalidated-suggestion-card` provenientes del Vault con la insignia explícita `"Sugerencia no validada"`.

---

### 7.7. Contratos de Seguridad y Pruebas Automatizadas
* **Suite de Autenticación Autónoma (`test_unlock_auth_flow.cjs`):** Ejecuta 207 pruebas automatizadas al 100% PASS cubriendo estado inicial, validación reactiva de política de contraseñas, rechazo por discrepancia, registro directo con emisión inmediata de cookie de sesión (`session_token`), rechazo de duplicados con HTTP 409 Conflict, transición automática a Versión Demo en Paso 2, convalidación atómica de licencias en SQLite, desbloqueo integral de materias en `LicenseService`, cierre de sesión e invalidación de cookie, inicio de sesión exitoso con hash PBKDF2, account lockout (15 min tras 5 intentos fallidos), creación, consulta y revocación de licencias mediante PIN de administrador (`almabaltoamial2020`), simulación de purga automática de cuentas demo con más de 48 horas de antigüedad preservando intactas las cuentas convalidadas, la **Sección 17 (24 nuevas aserciones v7.4)** que valida exhaustivamente: escritura atómica en el libro mayor `access_code_usages`, convalidación idempotente sin re-descuento de cupos (`times_used`), inicio de sesión multi-dispositivo con restauración automática de Pase Activo en cuentas con `max_uses = 1`, re-validación de sesión en `/api/auth/me` con auto-reparación, supremacía absoluta del estado del servidor sobre registros de `localStorage` expirados o vacíos, doble inmunidad ante la purga de 48 horas para usuarios con registros en `access_code_usages`, exposición del array de auditoría `usages: [{ email, consumed_at }]` en `GET /api/admin/codes`, la **Sección 16.10 (27 aserciones v7.16)** que valida la Cuenta de Administrador de Presentación Docente: contrato por rol en el cliente (`verifyAdminPin` → `"docente"`/`"admin"`/`null`, `getAdminRole`, `isFullAdmin`, `isDocente`, `canManageNotes` false en docente, `isDemoMode` false), defensa en profundidad sin red (0 fetch en `generateCode`/`fetchAdminCodes`/`revokeCode` en modo docente), cierre de sesión limpio que borra rol y PIN de `sessionStorage`, HTTP **403** del rol docente en los 3 endpoints admin con mensaje «Modo Presentación Docente», prueba de no-creación (sin rastro de códigos intentados), y regresión del rol pleno admin (200) y PIN inválido (401), y la **Sección 19 (34 aserciones v7.35, PROMPT 028)** que valida exhaustivamente la Venta Manual Ágil: generación atómica de lotes de hasta 50 códigos en `POST /api/admin/create-code-batch` con reversión total all-or-nothing ante errores, pre-asignación estricta de correo en `POST /api/auth/link-code` y `POST /api/auth/register` con rechazo HTTP 400 ante discrepancia de titularidad y compatibilidad con stock genérico, purga de stock ocioso en `POST /api/admin/purge-codes` con previsualización `dry_run=True`, cuádruple exclusión defensiva (preservación de códigos consumidos, pre-asignados y revocados), actualización en vivo de etiquetas de estado en `POST /api/admin/update-code-label`, y bloqueo estricto con HTTP 403 para la cuenta docente en los 3 nuevos endpoints admin.
* **Suite de Regresión e Integración (`test_e2e_case_flow.cjs`):** Ejecuta 1338 pruebas integrales al 100% PASS (incluyendo la Sección 9 reescrita en v7.8; Sección 10 con ordenamiento canónico de `indexCode`; Sección 23 con los 13 arquetipos dogmáticos, pautas oficiales y la Subsección 23.6 (v7.10) con nutrición dinámica viva desde `DYNAMIC_CORPUS`, resolución secundaria y citas corpus-driven; Sección 24 con contrato de integridad de códigos temáticos `N.M`; la **Subsección 24.4 (v7.11)** que audita exhaustivamente el `QuestionDeveloper`: clasificación de las 5 naturalezas con precedencia estricta, 4 preguntas con 5 alternativas A-E, soluciones dogmáticas $\ge 150-200$ chars terminadas en `"Conclusión:"`, pauta docente, citas legales no alucinadas como subconjunto estricto de las fuentes reales, determinismo PRNG `fnv1a` + `mulberry32`, tolerancia a cédulas sin citas, auto-cierre como completada al alcanzar 4/4 en `StorageService` y no-regresión de `toggleTopicMastery`; la **Subsección 24.5 (v7.17)** que audita el formato simple y solo dogmático en las 103 cédulas (anclaje por bigramas de las 3 proposiciones verdaderas de cada combinación + anti-verbatim de la falsa, contrato de formato I–IV con 4 proposiciones y la correcta como set exacto de las verdaderas, distractores que incluyen la falsa u omiten una verdadera sin duplicar la correcta, opciones ≤ 120 chars, cero absurdos, plausibilidad ≥ 1 distractor verosímil — proposición verdadera, vocabulario o cifra/unidad compartida según el formato —, sencillez con topes por formato, plazo numérico simple en `procesal-procemayor-5-2` con correcta literal «18 días hábiles» y distractores numéricos sin cómputo aritmético, casos aparte y regresión determinista de la 24.4); la **Sección 26 (v7.17)** con la compilación determinista de estructura (`extract_estructura_pruebas.py` → `js/exam-structure-grammar.js`: guardrail de skip de `README.md`/`_estructura_extraida.md`, fixture incorporado con `generatedFrom` y `hasCasoPractico`, regeneración byte-idéntica idempotente tras limpieza y consumo perezoso de la gramática por `_gparam` con fallback); la **Sección 25 (v7.12)** con los 57 tests de sincronización multi-dispositivo del avance de cédulas; la **Sección 27 (v7.18, PROMPT 015)** con 8 aserciones de gateo de visibilidad por rol admin; la **Sección 28 (v7.22, PROMPT 017)** con 11 aserciones del Vault semántico estático; la **Sección 29 (v7.25, PROMPT 020)** con 11 bloques de aserciones dedicadas a la búsqueda con normalización, sinónimos, filtros e historial; la **Sección 30 (v7.27, PROMPT 021)** con 11 bloques de aserciones del motor de Q&A extractivo puro: definición puntual con top-1 en cap 10 de Procesal para "emplazamiento", comparativa yuxtapuesta sin texto propio entre 2 bloques o materias distintas, panorama por disciplina con 1 cita representativa por materia en orden canónico, manejo de falta de cobertura con mensaje honesto + sugerencia dogmática (`inferSuggestedSubject`), gate de salida estricto con falla cerrada ante citas no verificables en el canon, determinismo estricto JSON idéntico en doble corrida, latencia < 100 ms (~5-14 ms), contrato de `App.openTopic(id, { highlight })` con resaltado `<mark class="vault-highlight">` y no-regresión de scroll a 0 sin highlight, persistencia y aislamiento estricto de `localStorage['vault_uncovered_queries']` con límite de 30 y botón limpiar, sanitización XSS de consultas e inyección segura en DOM, e integración en `index.html` y `css/sidebar.css`; la **Sección 31 (v7.29, PROMPT 023)** con 44 aserciones del Buscador Global del Header con Modos Semántico y Q&A: verificación de identificadores DOM (`#global-search-container`, `#search-mode-selector` con `role="radiogroup"`, `#search-mode-semantic`, `#search-mode-qa`, `#global-search-input` con `maxlength="200"`, `#search-results-dropdown` con `aria-live="polite"`), tokens CSS y responsividad (`.search-mode-selector`, `.search-mode-pill`, `.search-mode-badge`, `.search-input-wrapper`, azul `--info-text` para Semántico, dorado `--gold-primary` para Q&A, touch targets $\ge 44 \times 44\text{ px}$ en `@media 768px` y ocultamiento de `.search-mode-label` para evitar desbordes móviles), conmutación de modo y persistencia en `localStorage['global_search_mode']`, delegación estricta a motores canónicos sin duplicación de lógica (Semántico lista cédulas y Q&A compone tarjetas de citas directas sobre las 103 cédulas canónicas), blindaje de entrada inerte anti-inyección (truncado de 500+ a 200 caracteres, purga de nulos/control/zero-width, escape de `<script>` e `<img onerror>`, inmunidad ReDoS < 100 ms ante regex patológicas, y neutralidad de jailbreaks como texto plano sin mutar modos ni gates terminando en estado vacío honesto), y navegación interactiva con highlight y conmutación automática a la vista `topics` al buscar desde fuera de ella; la **Sección 32 (v7.34, PROMPT 027)** con 13 aserciones del Buscador Único al Header y Cero Duplicados en Sidebar: validación de exactamente 1 input de búsqueda en todo el DOM (`#global-search-input`), retiro absoluto de `#vault-search-box` y `#qa-container` de `aside#app-sidebar` con 0 inputs en el sidebar, verificación funcional de los 3 motores en el header (Semántico lista resultados en `#vault-search-results`, Q&A compone tarjetas en `#qa-answer`, gaps no cubiertos en `#qa-gaps-panel`), persistencia y renderizado de filtros e historial en `#search-results-dropdown`, tokens y layout responsive móvil (`order: 4`, 100% width, touch targets $\ge 44 \times 44\text{ px}$, overlay fijo sin overflow), y conmutación automática de vista a `topics` al buscar desde fuera de apuntes); la **Sección 33 (v7.38, PROMPT 031)** con 28 aserciones de Cruces Dogmáticos Enriquecidos y Citas del Canon: validación de presupuesto de `dogmatic_connections.json` ($\le 300\text{ KB}$, 274.01 KB real), cobertura del 100% del canon (103/103 temas con 3 cruces cada uno, 309 totales), cero auto-enlaces, 100% de targets en el allowlist canónico, cero duplicados por tema, taxonomía cerrada de 6 tipos, 100% de citas no vacías $\le 140$ caracteres y verbatim en el contenido destino, cumplimiento de compuerta $\ge 3$ bigramas compartidos, sincronización en `all_afg_topics.json` y `js/data.js` con erradicación de plantillas genéricas, verificación de reglas CSS y touch targets $\ge 44\text{ px}$ en `css/topic-viewer.css`, contrato de `renderConnectionCardsHtml`, botón de salto con highlight y empty state honesto en `js/app.js`, y certificación de métricas en `cruces_report.json`; la **Sección 34 (v7.41, PROMPT 032)** con 25 aserciones del índice jerárquico visual (etiqueta `M.C.S`, unicidad 205/205, colapsabilidad y accesibilidad ARIA); y la **Sección 35 (v7.43, PROMPT 034)** con 10 aserciones del Grafo Vivo: verificación de `graph_data.json` y `js/graph-data.js` (`window.GRAPH_DATA`), correspondencia 1:1 de los 205 nodos con el canon de temas, 545 enlaces únicos no dirigidos sin colisiones ni duplicados recíprocos $A \leftrightarrow B$ (`duplicatesCollapsed: 70`), 0 huérfanos, 0 auto-enlaces, taxonomía cerrada de 6 tipos, cálculo de peso numérico `val`, presupuesto $\le 615.00$ KB (171.96 KB real), y certificación determinista idéntica en `graph_report.json` con SHA-256 (1338/1338 pruebas exitosas en la suite).
...
| `index.html` | Estructura | Workbench jurídico, visor de cédulas, visor de casos, inyección de `js/exam-structure-grammar.js`, `js/question-developer.js`, `js/sinonimos.js` (v7.25), `js/vault-index.js`, `js/vault-search.js` y `js/qa-composer.js` (v7.27), header móvil optimizado solo-íconos con atributos de accesibilidad ARIA (`aria-label="GRADOMANIACOS"`, `aria-label="Cerrar sesión"`), sin selector de tema (Dark Academy inmutable), **buscador único centralizado en el header `#global-search-container` con selector segmentador de modos `#search-mode-selector` (`role="radiogroup"`), píldoras `#search-mode-semantic` y `#search-mode-qa`, campo de entrada único en el DOM `#global-search-input` (`maxlength="200"`), botón de limpieza `#vault-search-clear` y dropdown unificado de resultados `#search-results-dropdown` (`aria-live="polite"`) que aloja los filtros de disciplina (`#vault-filter-pills`), selector de capítulos (`#vault-filter-chapter`), historial de búsqueda (`#vault-search-history`), resultados semánticos (`#vault-search-results`), sub-selector Q&A (`#qa-mode-pills`), respuestas extractivas (`#qa-answer`) y ranking de temas no cubiertos (`#qa-gaps-panel`) (v7.34, PROMPT 027)**. **Accesibilidad ARIA y Shell Óptimo (v7.37, PROMPT 030):** `.header-nav` con `role="tablist"`, botones `.nav-tab` con `role="tab"` y `aria-selected` dinámico, botón `#btn-toggle-sidebar` con `aria-expanded="false"` / `"true"` reactivo, insignias decorativas con `aria-hidden="true"`, y reserva de ancho mínimo en `#case-count-badge` para erradicar saltos de diseño. El sidebar `aside#app-sidebar` queda limpio de inputs y cajas de búsqueda, conteniendo exclusivamente `#topics-tree-container`. Modal unificado de acceso `#unlock-modal` con aviso de 48h `#demo-expiry-notice`. Bloque de respaldo manual `.sidebar-progress-actions` retirado definitivamente (v7.28). **Cuenta de Presentación Docente (v7.16) y Venta Manual Ágil (v7.35, PROMPT 028):** el panel de administración envuelve el formulario de generación en `#admin-license-form` (con selector de cantidad `#admin-batch-count` de 1 a 50 códigos) y el historial en `#admin-codes-panel` (con filtro de estado operativo `#admin-codes-filter-status`, botón de purga de stock ocioso `#btn-admin-purge-codes`, y columna Estado en la tabla de licencias con badges `.code-status-badge`); ambos ocultados en modo docente con banner `#docente-mode-banner`. |
| `css/paywall.css` | Estilos | Estilos del formulario de credenciales, hints de contraseña, aviso de expiración 48h `.demo-expiry-notice`, paywall, insignias de estado y responsive móvil. **Venta Manual Ágil (v7.35, PROMPT 028):** estilos interactivos para insignias de estado operativo en licencias admin (`.code-status-badge`, `.badge-pendiente`, `.badge-entregado`, `.badge-neutral`) con transiciones de color, cursor interactivo y micro-animaciones accesibles. |
| `css/main.css` | Estilos | Sistema de diseño *Dark Academy* inmutable (`color-scheme: dark;` en `:root`), variables tipográficas (`Playfair Display`, `Plus Jakarta Sans`), **escala canónica unificada de 5 pasos para interlineado (`--lh-title: 1.28`, `--lh-heading: 1.35`, `--lh-body: 1.70`, `--lh-table: 1.60`, `--lh-dense: 1.50`, v7.36, PROMPT 029)**, **tokens canónicos de espaciado 8pt (`--space-1` a `--space-8`) y jerarquía de z-index unificada (`--z-base: 1` a `--z-toast: 1000`) (v7.37, PROMPT 030)**, resolución de inversión de capas dropdown vs. modal backdrop, reserva de ancho mínimo anti-layout shift en `.badge-count` (`min-width: 20px`), regla universal `:focus-visible` con anillo dorado de alto contraste, soporte para `@media (prefers-reduced-motion: reduce)`, y componentes visuales unificados (`.ui-skeleton`, `.ui-state-empty`, `.ui-state-error`), tokens WCAG AA de modo claro preservados como código muerto documentado, header responsive móvil (`@media (max-width: 768px)`) con touch targets $\ge 44 \times 44\text{ px}$, **estilos del buscador único centralizado en el header (`.search-bar`, `.search-dropdown`, `.search-mode-selector`, `.search-mode-pill`, `.search-input-wrapper`, `.search-mode-badge`, tokens de modo azul `--info-text` para Semántico y dorado `--gold-primary` para Q&A, estilización completa del dropdown para filtros `.vault-search-filters`, selector de capítulos `.vault-filter-chapter`, chips de historial `.vault-history-chip`, tarjetas de resultados `.vault-search-result-item`, resaltado `.vault-highlight`, sub-píldoras Q&A `.qa-mode-pill`, tarjetas de cita `.qa-citation-card`, insignias de perspectiva `.qa-block-perspective-badge`, botones de salto `.qa-action-jump-btn`, caja de gaps `.qa-uncovered-box`, y layout responsive móvil con fila dedicada al 100% de ancho `order: 4; margin-top: 6px;` y overlay flotante fijo `left: 8px; right: 8px; max-width: calc(100vw - 16px)` sin desborde a 360 px, v7.34, PROMPT 027)**, componentes del cuestionario de grado (`#section-quiz-developer`, `.quiz-question-card`, `.quiz-q-statement` con `white-space: pre-line` para enunciados multilínea I-IV v7.17, `.quiz-option`, `.quiz-solution`, `#quiz-completed-banner`, `.quiz-progress-bar`) y la insignia de laboratorio `.quiz-admin-lab-badge` (v7.18) en ámbar/dorado con alto contraste accesible. |
| `css/topic-viewer.css` | Estilos | Visor de lectura dogmática y apuntes de grado. Tipografía unificada bajo la escala canónica (`--lh-body`, `--lh-title`, `--lh-heading`), espaciado de títulos (h1–h6), párrafos (`.reading-p`), bloques de cita y tarjetas. **Tablas comparativas fluidas y accesibles (v7.36, PROMPT 029):** contenedor `.table-responsive` con desplazamiento horizontal táctil fluido (`overflow-x: auto; -webkit-overflow-scrolling: touch;`), outline accesible por teclado (`focus-visible`), tabla `.reading-table` con ancho mínimo controlado (`min-width: 580px` en PC, `520px` en móvil), celdas con `word-break: normal;` y `hyphens: auto;` para evitar colapso o aplastamiento silábico vertical, anchos mínimos por columna, cebra estilizada Dark Academy y eliminación definitiva de `overflow-x: hidden` en `.topic-reading-col` en móvil. **Shell y scroll containment (v7.37, PROMPT 030):** `.topic-reading-col` centrado ergonómico en PC (`margin: 0 auto; max-width: 940px;`) y contención de scroll con `overscroll-behavior: contain;`. **Cruces Dogmáticos Enriquecidos (v7.38, PROMPT 031):** componentes `.connection-target-meta`, `.connection-quote-box` con `<mark class="vault-highlight">`, botón de salto interactivo `.btn-jump-connection` con touch target accesible ($\ge 44 \times 44\text{ px}$ en móvil), estado vacío honesto `.connections-empty-state` y tarjetas de sugerencia no validada `.unvalidated-suggestion-card`. |
| `css/sidebar.css` | Estilos | Índice lateral (`#app-sidebar`), barra de progreso de dominio (`.progress-bar`), estilos depurados sin reglas huérfanas de respaldo manual (v7.28). **Depuración y retiro de duplicados (v7.34, PROMPT 027):** eliminación definitiva de más de 730 líneas de estilos huérfanos de búsqueda y Q&A del sidebar, reemplazadas por una nota canónica de migración hacia `css/main.css`. El archivo conserva exclusivamente los estilos del árbol canónico de cédulas (`#topics-tree-container`, `.sidebar-subject-pills`, `.sidebar-topic-item`, `.sidebar-topic-title`, `.sidebar-badge-mastered`). **Optimización y contención de scroll (v7.37, PROMPT 030):** `overscroll-behavior: contain;` en `.app-sidebar` y `.sidebar-tree`, `content-visibility: auto; contain-intrinsic-size: 0 44px;` en `.category-group` para optimización de renderizado, `.topic-item-right` con `min-width: 24px; justify-content: flex-end;` anti-layout shift, y estado vacío `.sidebar-empty-state`. **Índice jerárquico visual (v7.41, PROMPT 032):** badge `.cedula-code-badge` con ancho fijo `min-width:52px` + `tabular-nums` anti layout-shift, conteo `.chapter-cedulas-count` (`N cédulas`), `:focus-visible` dorado en `.category-header`, `min-height:44px` en cabecera e ítem móvil (targets WCAG, cero overflow-x). |
| `css/modal.css` | Estilos | Diálogos modales, backdrop difuminado y estructura responsive para dispositivos móviles. **Jerarquía Z-Index (v7.37, PROMPT 030):** `.modal-backdrop` con `z-index: var(--z-modal);` (500) garantizando precedencia estricta sobre dropdowns de búsqueda. |
| `css/concept-graph.css` | Estilos | Lienzo de grafo HTML5 con `touch-action: none;` y controles flotantes adaptados a pantallas táctiles. **Inspector Cerebro: Ficha de Conocimiento y Mastery (v7.45, PROMPT 036):** estilos de la Ficha de Conocimiento por nodo (`.inspector-meta-row`, badges de disciplina `.inspector-meta-badge`, código jerárquico `.inspector-hier-badge`, fila de acciones `.inspector-actions-row`, botón de alternancia reactiva de dominio `.btn-mastery-toggle` con estado `.is-mastered` en esmeralda, sección Q&A extractivo `.inspector-qa-section`, tarjetas de cita `.inspector-qa-result`, y lista de cruces `.inspector-connections-list`), con touch targets interactivos accesibles ($\ge 44\text{ px}$) y contención de scroll estricta (`max-height: 70vh`, `overflow-x: hidden`, `box-sizing: border-box`) garantizando cero desbordes horizontales en pantallas móviles de 360 px. **Toolbar del Grafo, Modo Foco y Escala (v7.46, PROMPT 037):** estilos de la barra de herramientas flotante superior `.graph-toolbar` con glassmorphism, blur y sombras Dark Academy; componentes de búsqueda (`.graph-search-box`, `#graph-search-input` con `maxlength="200"`, botón `#graph-search-clear`, feedback `#graph-search-feedback`), píldoras de filtro por disciplina (`#graph-filter-pills`, `.graph-filter-pill` con estado `aria-pressed`), selector de capítulos `#graph-filter-chapter` ordenado por min `indexCode`, switch de overlay de dominio `#graph-toggle-mastery-overlay` con conteo reactivo `Dominadas X/N`; reubicación de controles de zoom flotantes (`.graph-floating-controls`) a `top: 76px; left: 16px;` (desktop) y `bottom: 80px; right: 10px;` (móvil) evitando colisiones; y diseño responsive móvil ($\le 768\text{px}$ y 360 px) con touch targets interactivos $\ge 44\text{ px}$ en pills, selector, switch e inputs, scroll horizontal contenido y altura máxima acotada (`max-height: 48vh`). |
...
| `js/app.js` | Orquestador | Controlador principal de GRADOMANIACOS, navegación, gateo estricto del sidebar `#app-sidebar` y `#btn-toggle-sidebar` a la vista `topics`, sincronización autoritativa con `/api/sync-topics` sin acumulación zombi (`syncWithServer`), renderizado deduplicado y debounced con `replaceChildren` (`renderSidebar`), **orden de capítulos por min `indexCode` para respetar el temario canónico Discusión→Prueba→Sentencia (v7.19.1)**, **controlador del Buscador Único Centralizado en el Header (`setupGlobalSearch`, etc.) (v7.34, PROMPT 027)**. **Accesibilidad y Shell Óptimo (v7.37, PROMPT 030):** sincronización reactiva de `aria-expanded` en `#btn-toggle-sidebar` y `aria-selected` en pestañas `.nav-tab` dentro de `switchView()`, atributo `aria-current="true"` en la cédula activa del árbol temático, contenedor vacío `.ui-state-empty`, y restauración limpia de foco en `#global-search-input` tras presionar `Escape`. **Índice Jerárquico Visual (v7.41, PROMPT 032):** helper solo-display `displayHierCode(topic)` → `M.C.S` (civil 1, procesal 2, constitucional 3; `S` posicional `idx-minIdx+1` con `_chapterMinIndexCache`, fallback a `code`/`indexCode`, 205/205 únicos — el `S=code` tal cual daría 24 duplicados por codes modulares 1.x/2.x/3.x), badge `.cedula-code-badge` con `data-hier-code`, tooltip con trazabilidad `Cédula ${indexCode} · Sección ${code}`, `aria-label` con ambas, capítulos colapsables con `data-chapter-key`/`Capítulo C · Título (N cédulas)`/`aria-expanded` sincronizado/teclado Enter-Espacio, estado en memoria `_openChapters` (aislado de `StorageService`), auto-expansión del capítulo activo + `scrollIntoView`. **Cruces Dogmáticos Enriquecidos y Salto con Highlight (v7.38, PROMPT 031):** orquestador de cruces con helper unificado `renderConnectionCardsHtml(connsList)` para el acordeón inline y el aside panel, eliminación total de plantillas genéricas, manejador de clic en `.btn-jump-connection` y `.linked-connection-card` con validación previa en el allowlist de 103 cédulas canónicas (escudo anti open-redirect) y navegación interactiva `this.openTopic(targetId, { highlight: snippet })` con centrado suave. **Venta Manual Ágil (v7.35, PROMPT 028):** integración de listener en `#admin-batch-count` (bloqueo reactivo de código manual y cambio dinámico a "Generar Lote (N)"), validación y diálogo de confirmación previa en `btnGenerate`, filtrado client-side en tiempo real con `#admin-codes-filter-status`, botón `#btn-admin-purge-codes` con cálculo interactivo dry-run y diálogo de confirmación detallado, y manejador de clic en `.code-status-badge` para alternar instantáneamente el estado (`[PENDIENTE]` $\leftrightarrow$ `[ENTREGADO]`) vía `LicenseService.updateCodeLabel`. **Gateo de Visibilidad del Cuestionario por Rol Admin (v7.18, PROMPT 015):** inyección de `#section-quiz-developer` condicionada a `isAdmin && isUnlocked` con insignia `.quiz-admin-lab-badge` en la cabecera del cuestionario, manteniendo el botón `#btn-bottom-mastery` / `#btn-toggle-mastery` visible y 100% operativo para todos los perfiles de usuario. **Sincronización de avance de cédulas (v7.12):** orquestador `pullMasteryProgress()` con guard de re-entrancia `_isPullingMastery`. **Cuenta de Presentación Docente (v7.16):** login admin por rol (toasts diferenciados), `applyDocenteRestrictions()` que oculta `#admin-license-form`, `#admin-codes-panel` y `#btn-admin-manage-notes` mostrando `#docente-mode-banner`. **Cruces Dogmáticos Desacoplados (v7.45, PROMPT 036):** helper unificado `renderConnectionCardsHtml(connsList, topicContext = null)` desacoplado como método de `App` y expuesto globalmente en `window.renderConnectionCardsHtml`, permitiendo su consumo idéntico tanto en el visor de lectura como en el Inspector Cerebro del grafo sin duplicación de plantillas. **Piloto 1 Laya: Autodetección de Modo Q&A con Override Manual (v7.47, PROMPT 038):** en `executeGlobalSearch()`, al consultar en modo Q&A se emite consulta asíncrona no bloqueante a `/api/laya/predict` (tarea `qa_mode`, timeout 600ms vía `AbortController`). Si el usuario no ha realizado elección manual (`qaModeManualOverride === false`), se preselecciona la píldora correspondiente (`definicion`, `panorama`, `comparativa`) con atributo `title="Sugerido por Laya"` y `aria-live="polite"`. Cualquier clic manual del postulante en las píldoras fija `qaModeManualOverride = true`, garantizando supremacía inmutable de la voluntad del usuario sobre cualquier sugerencia de Laya. |
| `js/markdown-parser.js` | Parser Markdown | Parser determinista de apuntes jurídicos (`MarkdownParser.render` / `MarkdownParser.parse`). Renderiza encabezados multinivel h1–h6 con clases semánticas, cajas de puntos críticos ("Red Flags"), esquemas dogmáticos, wikilinks (`[[...]]`) y listas. **Tablas comparativas multilínea y accesibilidad WCAG AA (v7.36, PROMPT 029):** método `parseTables()` con algoritmo look-ahead resiliente que ensambla correctamente tablas cuyos renglones o filas vienen separados por saltos de línea vacíos o números de página extraídos de apuntes en PDF (recuperando el 100% de las 40 tablas del canon que se caían a párrafos rotos); método `renderSingleTable()` que envuelve tablas en `<div class="table-responsive" tabindex="0" role="region" aria-label="Tabla comparativa de estudio">` para navegación accesible por teclado y lectores de pantalla, y sanitiza entidades `&lt;br&gt;` y `&amp;lt;br&amp;gt;` restituyendo etiquetas `<br>` válidas dentro de celdas. Soporta entornos Node.js (`module.exports = MarkdownParser`). |
| `js/storage.js` | Servicio | Gestor de persistencia en `localStorage` (`StorageService`), auto-curación y deduplicación transparente de tópicos legados preservando `mastered`, tracking de progreso por usuario, gestión de borradores de casos, y persistencia de progreso de cuestionarios en `topicQuizzes` con auto-marcado atómico de dominio al acertar 4/4 (`markTopicMastered`, `recordTopicAnswer`). **Sincronización multi-dispositivo y seguridad (v7.12 / v7.28):** `isServerSyncAvailable()`, `_pushMasteryChange()`, `pullMasteryFromServer()` (LWW por cédula + reconciliación), inclusión obligatoria de `credentials: "include"` en todos los fetch al servidor, y capa de servicio para respaldo manual `buildUserProgressExportPayload()` / `exportUserProgressFile()` / `mergeUserProgressFromJsonString()` blindada con guard estricto anti-prototype-pollution (`__proto__`, `constructor`, `prototype`). El historial reciente del Vault y el ranking de huecos (`vault_uncovered_queries`) quedan estrictamente aislados de esta exportación. |
...
| `js/auth-license.js` | Servicio | Lógica de licencias (`LicenseService`), cálculo de materias desbloqueadas y método `isDemoMode()` con **supremacía canónica del estado de servidor** (`AuthService.currentUser.access_code`) sobre `localStorage`. **Venta Manual Ágil (v7.35, PROMPT 028):** métodos `generateBatchCodes(options)` para emisión atómica masiva (1–50) con fallback client-side, `purgeUnusedCodes(options)` para limpieza segura de stock ocioso con sincronización en caché local, y `updateCodeLabel(code, label)` para alternar estados operativos; mapeo dual de `label` y `studentName` en `fetchAdminCodes()`; y guards de defensa en profundidad sin fetch cuando `isDocente()`. **Cuenta de Presentación Docente (v7.16):** división por rol — `DOCENTE_PIN_HASH` (`profesoresafg202602`), `STORAGE_ADMIN_ROLE_KEY`, `verifyAdminPin()` retorna `"admin"`/`"docente"`/`null`, accesores `getAdminRole()`/`isFullAdmin()`/`isDocente()`. |
| `js/auth-config.js` | Configuración | Configuración de cliente, códigos de invitación para modo estático y parámetros de sesión. |
| `js/concept-graph.js` | Visualizador | Grafo interactivo en HTML5 Canvas con física de fuerzas (repulsión Coulomb cuadrática con distancia de corte a 320 px y resortes Hooke), adaptabilidad a pantalla completa (100% viewport) sin dependencia del sidebar y soporte móvil completo (arrastre de nodos, pan y pinch-to-zoom con dos dedos). **Consumo de Grafo Vivo (v7.44, PROMPT 035):** renderiza el canon íntegro de 205 cédulas y 545 nexos dogmáticos desde `window.GRAPH_DATA` (con cadena de fallback hacia `StorageService` / `INITIAL_DATA` y advertencia de contingencia), radio proporcional al grado dogmático ($18 + \min(12, \text{grado} \times 2)$ px), truncamiento limpio de etiquetas a 28 caracteres, leyenda dinámica en vivo ("205 cédulas · 545 nexos"), optimización de ciclo de animación con auto-pausa en pestaña inactiva (`document.hidden`) o fuera de vista `graph`, navegación directa con sanitización anti-XSS y accesibilidad en controles (`role="toolbar"`, `aria-label`, `title`, `tabindex="0"`). **Inspector Cerebro: Ficha de Conocimiento, Mastery Reactivo y QA Extractivo (v7.45, PROMPT 036):** Ficha de conocimiento enriquecida en `selectNode(node)` con metadata dogmática (`§ indexCode`, capítulo temático, código jerárquico `M.C.S` con tooltip y título completo sin truncar); validación estricta de destinos en allowlist canónico `isValidTopicId(topicId)` como escudo anti open-redirect; botón `Ver Cédula Completa` (`App.openTopic`); botón de alternancia reactiva de dominio `Dominada ✓ / Marcar por repasar` (`StorageService.toggleTopicMastery`) con re-renderizado instantáneo de la ficha; cruces dogmáticos verbatim reutilizando el helper unificado de la aplicación `renderConnectionCardsHtml` (con fallback de aislamiento `_renderConnectionCardsFallback`) con resaltado `<mark class="vault-highlight">`, botón de salto interactivo `.btn-jump-connection`, navegación interactiva entre nodos del grafo al pulsar links vinculados y estado vacío honesto; motor de QA extractivo mínimo on-demand mediante botón `¿Qué dicen mis apuntes?` que invoca `QAComposer.composeAnswer(node.label, "definicion")` renderizando 1 tarjeta extractiva `.qa-citation-card` con salto o aviso de falta de cobertura; anillo/halo reactivo verde esmeralda (`#10b981`, stroke 2.5px) alrededor de nodos dominados en `draw()`; sanitización integral contra XSS con `SecurityShield.escapeHtml()`, semántica ARIA (`role="complementary"`, `aria-label`) y exportación universal CommonJS/Browser. **Toolbar del Grafo, Modo Foco Vecindario a 1 Salto, Cap de 400 Aristas y 60 FPS (v7.46, PROMPT 037):** motor de búsqueda canónica `searchNode(query)` con debounce (150ms), sanitización anti caracteres de control (`[\x00-\x1F\x7F-\x9F]`), delegación transparente en `VaultSearch.searchVault(cleanQuery, { limit: 5 })` y fallback local directo por coincidencia insensible a acentos/mayúsculas sobre `label`, `title` e `id` en `this.nodes`, centrado suave de cámara en el top-1 (`centerNode(node)`) y despliegue automático en el inspector (`selectNode(node)`), con retroalimentación accesible en `#graph-search-feedback` (`aria-live="polite"`); filtros atenuantes sin mutación de arrays (`setSubjectFilter` y `setChapterFilter`) que atenúan a opacidad 0.12 los nodos fuera de filtro preservando intacta la física y estructura del grafo; selector de capítulos ordenado canónicamente por disciplina y min `indexCode`; switch de overlay de dominio `toggleMasteryOverlay()` con contador en vivo sincronizado con `StorageService`; modo foco por vecindario a 1 salto al seleccionar un nodo (nodo y vecinos directos a opacidad 1.0, nodos no conectados atenuados a 0.10, aristas incidentes resaltadas en dorado `#d4af37` con stroke 2.5px y rótulo del tipo de nexo, y aristas no incidentes a opacidad 0.05); cap de 400 aristas visibles (`visibleLinksCap = 400`) con heurística de ordenación por prioridad (incidentes en selección $\rightarrow$ inter-materia transversales $\rightarrow$ hubs de alto grado) y leyenda reactiva en vivo; optimización crítica de rendimiento en `draw()` precomputando el conjunto de dominio (`masteredSet = new Set(StorageService.getUserMasteredTopics())`) una sola vez por frame antes del loop de nodos, reduciendo el frame time de ~85ms a **6.9ms - 8.1ms** garantizando 60 FPS sostenidos; y cierre definitivo del ciclo canónico de visualización del grafo (034–037). **Hotfix Estabilidad Numérica (v7.46.1):** signo Hooke corregido en resortes (`b.vx -= fx; b.vy -= fy` — el `+=` en ambos extremos explotaba a NaN y vaciaba el canvas), `MAX_VELOCITY = 12` con cap por nodo y guarda anti-NaN con recentrado en `updatePhysics()`. |
| `js/case-generator-agent.js` | Agente IA | Síntesis dogmática de casos inéditos, catálogo de 37 instituciones en 13 arquetipos (10 existentes + 3 nuevos interdisciplinarios), 4 contra-instituciones doctrinales, lote de repaso v7.33 (`generateReviewLot` determinista + reporte de razonamiento), gate de anclaje dogmático v7.32 (`ANCHOR_MIN_BIGRAMS=3`, `needsReview` visible, reorder estable por overlap), preguntas graduadas con 5 alternativas y distractores seductores, pauta docente, error fatal de grado, rúbrica AIME de 4 dimensiones, soluciones modelo $\ge 400$ chars. **Nutrición viva y corpus dinámico (v7.10):** corpus semilla canónico (`FUENTES_CORPUS` / `corpusSeed`), corpus dinámico (`DYNAMIC_CORPUS`) derivado de contenidos reales vía `extractCitations()`, pre-computación de índice $O(1)$ (`validCitationsIndex`), validación de citas en 3 capas (`assertCitationIntegrity`), resolución de fuentes en 5 pasos (`resolveLinkedFuente`), vinculación tolerante por clave secundaria `(subject, indexCode|code)` en `getLinkedApuntesForArchetype()`, validación de `linkedTopics` vivos y sanitización de rutas en `linkedFuentes.file` (`validateGeneratedCase`), y poda FIFO. |
...
| `sinonimos.json` | Glosario Semántico | Diccionario JSON curado de 33 grupos de sinónimos e instituciones equivalentes de examen de grado chileno (Civil, Procesal, Constitucional) con esquema canónico `{ terminos, subject, nota }` (v7.25, PROMPT 020). |
| `js/sinonimos.js` | Artefacto JS / Vault | Declaración global `window.VAULT_SINONIMOS` / `module.exports` generada deterministamente desde `sinonimos.json` por `build_vault_index.py` para búsqueda cliente offline y paridad en tests (v7.25, PROMPT 020). |
| `scripts/sweep_mixed_trios_ledger.cjs` | Auditoría / Barrido | Script determinista offline de barrido completo de los 4.785 tríos mixtos civil/procesal (2+1 ambos sentidos) con generación de casos con seed PRNG, validación estructural, integridad de citas y compuerta de anclaje dogmático ($\ge 3$ bigramas). Genera ledger completo en TEMP y `trios_ledger_report.json` en workspace con certificación del 100% de solidez y análisis de tríos (v7.32, PROMPT 026). |
| `scripts/ciclo_nutricion.cjs` | Comando repetible | **Comando del ciclo de nutrición (v7.42, PROMPT 033):** `node scripts/ciclo_nutricion.cjs` ejecuta medir + registrar (~60–90s): lee baseline de `trios_ledger_report.json`, corre el barrido determinista, compara métricas (delta fully pp, delta needsReview, overlap top-20, piloto 4/4), anexa una línea a `nutricion_historial.jsonl` y emite veredicto `BASELINE/MEJORA/ESTABLE/REGRESION` (exit 2 ante regresión). Frase de activación: `nutrir sistema` (OpenCode o Anti). Sin tocar canon, datos, Vault, QA ni cruces. |
| `nutricion_historial.jsonl` | Auditoría / Historial | **Registro auditable de ciclos (v7.42, PROMPT 033):** una línea JSON por corrida (`fecha`, `canon_total`, `total_trios`, `fully_pct`, `needs_review`, `piloto_ok`, `top20_overlap_vs_previo`, deltas, `veredicto`). Las mediciones puras se registran aquí sin inflar versiones de `CONTEXT.md` (§9 solo ante cambio de motor/código). |
| `scripts/build_cross_connections.py` | Compilador Offline | Motor determinista offline de enriquecimiento de cruces dogmáticos. Resuelve IDs legados con `resolve_legacy_id()` (80 cruces manuales autoritativos), calcula similitud Vault BM25 + compuerta estricta de bigramas compartidos ($\ge 3$, promedio 125), clasifica bajo taxonomía cerrada de 6 tipos, y extrae citas textuales verbatim ($\le 140$ chars) desde `targetTopic.content`. Genera `dogmatic_connections.json` (274.01 KB $\le 300$ KB) y `cruces_report.json` con 100% de cobertura (103/103 temas, 309 cruces, 0 huérfanos, 0 auto-enlaces) (v7.38, PROMPT 031). |
| `build_dogmatic_connections.py` | Script / Wrapper | Script raíz de cruces dogmáticos refactorizado en v7.38 para delegar directamente en `scripts/build_cross_connections.py`, manteniendo compatibilidad y garantizando la regeneración determinista de `dogmatic_connections.json`. |
| `dogmatic_connections.json` | Artefacto JSON | Archivo estático JSON (274.01 KB $\le 300$ KB) con las conexiones interdisciplinarias canónicas para las 103 cédulas del temario, citas textuales verbatim y clasificación dogmática estricta (v7.38, PROMPT 031). |
| `cruces_report.json` | Auditoría / Reporte | Reporte forense y estadístico generado por `scripts/build_cross_connections.py` certificando cobertura (103/103 temas), conteo exacto de 3 cruces por tema, 0 auto-enlaces, 0 duplicados, 100% citas verbatim y distribución por categoría (v7.38, PROMPT 031). |
| `scripts/build_graph_data.py` | Compilador Offline | Builder determinista offline del grafo vivo de conceptos a partir de `all_afg_topics.json` y `dogmatic_connections.json`. Extrae nodos canónicos por cédula (categoría por disciplina, peso por conexiones, indexCode, código jerárquico), colapsa enlaces bidireccionales recíprocos $A \leftrightarrow B$ (`duplicatesCollapsed: 70`), descarta huérfanos y auto-enlaces, valida taxonomía de 6 tipos, controla presupuesto $\le \max(600\text{ KB}, \text{totalNodes} \times 3\text{ KB}) = 615.00$ KB y genera `graph_data.json`, `js/graph-data.js` y `graph_report.json` con SHA-256 byte-idéntico (v7.43, PROMPT 034). |
| `graph_data.json` | Artefacto JSON | Archivo estático JSON del grafo vivo (205 nodos, 545 enlaces únicos, 171.96 KB $\le 615.00$ KB) con nodos normalizados y enlaces no dirigidos deduplicados (v7.43, PROMPT 034). |
| `js/graph-data.js` | Artefacto JS / Grafo | Declaración global `window.GRAPH_DATA` / `module.exports` / `globalThis` que transporta los datos del grafo al cliente y entornos de test de forma autónoma (v7.43, PROMPT 034). |
| `graph_report.json` | Auditoría / Reporte | Reporte forense y determinista de generación del grafo vivo: conteo de nodos (205), enlaces (545), enlaces recíprocos colapsados (70), huérfanos (0), auto-enlaces (0), presupuesto (615.0 KB), tamaño real (171.96 KB) y hash SHA-256 (v7.43, PROMPT 034). |
| `LAYA_CONTRACT.md` | Documentación / Contrato | **Contrato canónico de integración de Laya (`LAYA_HOME`, v7.47, PROMPT 038).** Consagra la arquitectura de capa de decisión compartida: aislamiento estricto de PyTorch y dependencias en `~/.laya/venvs/laya` (cero paquetes nuevos en `requirements.txt`), fallback determinista con exit code 0 ante ausencias o timeouts, línea roja dogmática inmutable (Laya sugiere, NUNCA decide rúbricas, auth, maestría ni notas), registro estricto en `LAYA_TASKS` ($\le 5$ opciones en choice), bitácora sin PII en `router.jsonl` (queries truncadas a $\le 80$ caracteres, con campo `reason` en cada línea) y especificación de tareas piloto (`qa_mode`, `shield_risk`). |
| `scripts/setup_laya.py` | Instalador / Verificador | **Instalador y verificador canónico versionado de Laya (v7.47, PROMPT 038).** Permite aprovisionar o verificar el entorno local `~/.laya` con flags `--check` (para CI o pre-vuelo sin tocar nada, exit 0 si está instalado o 1 si falta), `--smoke-only` (inferencia rápida de prueba) y `--checkpoint` (descarga opcional de pesos offline). Asegura que el entorno ejecutable de Laya y PyTorch quede confinado exclusivamente a la máquina local del postulante o servidor sin contaminar el repositorio. |
| `scripts/laya_router.py` | Router de Decisiones | **Capa compartida de decisiones sobre Laya local (v7.47, PROMPT 038).** Wrapper CLI y biblioteca Python que implementa el registro canónico `LAYA_TASKS` (`qa_mode`, `shield_risk`), auditoría de seguridad estricta con denylist de palabras prohibidas, subcomando `predict` con flag `--state-file` (compatible con codificación UTF-8 con BOM para evitar errores de escape en PowerShell/Windows) y `--timeout-ms`, servidor worker cálido multi-hilo en puerto 47888 (`run_worker_server`), cliente de baja latencia con sondeo a worker residente (`query_warm_worker`), fallback determinista garantizado ante desconexión o timeouts, y registro seguro sin PII en `~/.laya/logs/router.jsonl` con el campo trazable `reason` garantizado en cada línea. |
| `scripts/benchmark_laya_20.py` | Benchmark / Evaluación | **Evaluador determinista de 20 consultas del Vault contra Laya (v7.47, PROMPT 038).** Evalúa una batería representativa de 20 consultas reales (definiciones, panoramas y comparativas) contra el worker cálido de Laya midiendo precisión contra ground truth, tasa de fallbacks y latencias p50/p95 en milisegundos. |
| `benchmark_results_038.json` | Reporte / Auditoría | **Resultados del benchmark de 20 consultas de Laya (v7.47, PROMPT 038).** Certificación formal: 20 consultas evaluadas, **95.0% de precisión (19/20)** superando con holgura el umbral del 80%, **0/20 fallbacks**, latencia **p50 de 498.4 ms** y **p95 de 548.8 ms** en CPU local, con veredicto certificado: `VIABLE Y SUPERIOR AL DEFAULT`. |
| `build_vault_index.py` | Compilador / Índice | Script Python determinista e idempotente que lee `all_afg_topics.json`, tokeniza aplicando la normalización contractada de `normalize_text` (ligaduras, comillas/guiones tipográficos, guiones blandos, zero-width spaces, preservando `ñ`), generando el índice estático `vault_index.json` (535.42 KB ≤ 600 KB), su exportación para navegador `js/vault-index.js` (objeto `VAULT_INDEX`), y compila `sinonimos.json` a `js/sinonimos.js` (`VAULT_SINONIMOS`). Se ejecuta en CI antes de publicar a GitHub Pages (v7.22 / v7.25, PROMPTS 017 y 020). |
| `vault_index.json` | Índice Semántico | Archivo estático JSON (535.42 KB ≤ 600 KB) con postings compactos `[tf, [pos...]]`, `df`, `avgLen`, `docLen`, `meta` y `temarioKeywords` para las 103 cédulas canónicas del Examen de Grado (v7.22 / v7.25). |
...
| `js/vault-search.js` | Motor de Búsqueda | Buscador cliente vanilla y sin estado `searchVault(query, options)` con `normalizeText` contractado con paridad estricta Python ↔ JS, expansión de sinónimos ponderada (factor ×0.8 para sinónimos y ×1.0 para literales), filtrado por `subject` y `chapterNumber` sin alteración del ranking relativo, algoritmo BM25-lite (`k1=1.2, b=0.75`), boosts temáticos (×1.3 palabras clave del temario canónico, ×1.2 títulos) y extracción de snippets verbatim (±140 chars) con resaltado sanitizado mediante `SecurityShield.escapeHtml` y `<mark class="vault-highlight">`. Cero alucinaciones y latencia < 50 ms (v7.22 / v7.25, PROMPTS 017 y 020). |
| `js/qa-composer.js` | Motor Q&A Extractivo | Motor client-side puro y sin estado `QAComposer` (`composeAnswer(query, options)`) que compone respuestas extractivas sin LLM a partir de citas textuales de las 103 cédulas con procedencia (`§ indexCode`, archivo, disciplina). Soporta modos `definicion` (top-1 directo), `panorama` (representativos por materia en orden canónico) y `comparativa` (yuxtapone 2 instituciones de distinta disciplina/capítulo sin contraste subjetivo propio), tolerando `options` como string directo (ej. `'definicion'`) u objeto `{ mode: 'definicion' }` (v7.45, PROMPT 036). Gate de salida estricto con falla cerrada ante citas no canónicas, sugerencia dogmática mediante `inferSuggestedSubject()` y persistencia local de huecos en `vault_uncovered_queries` (v7.27, PROMPT 021). |
| `js/case-solver.js` | Workbench | Interrogación, compuertas excluyentes, rúbrica AIME 2026-20 en 4 dimensiones, despliegue de pauta de corrección oficial (`.question-pauta-card`) y error fatal de grado (`.question-fatal-error-card`), píldoras interactivas de apuntes nutridos (`.btn-linked-apunte`) con salto directo al temario (`App.openTopic`), limitación a pregunta 1 en Modo Demo sin justificación, visualización de ventajas en preguntas > 0, evaluación unidimensional y escape XSS integral con `SecurityShield.escapeHtml`. **Piloto 2 Laya: Escudo Aditivo en Evaluación (v7.47, PROMPT 038):** en `evaluateCurrentQuestion(activeCase)`, invoca `await SecurityShield.inspectPromptInjectionAsync(justificationText)` ejecutando la compuerta de ciberseguridad con registro de trazabilidad (`securityOrigin: 'regex'|'laya'|'ambos'|'ninguno'`, `layaScore`), bloqueando la evaluación si cualquiera de las capas detecta riesgo. |
| `js/security-shield.js` | Ciberseguridad | Detección de prompt injection, anti-XSS, escape seguro (`escapeHtml`) y cuotas de caracteres. **Piloto 2 Laya: Puntuación de Riesgo Aditiva con Matriz OR (v7.47, PROMPT 038):** define constante nombrada `LAYA_SHIELD_THRESHOLD = 0.85` y expone `inspectPromptInjectionAsync(text)` que ejecuta en paralelo la inspección por regex síncrona y la inferencia asíncrona de Laya (`task: "shield_risk"`, score continuo `noul` con timeout de 600 ms). Implementa la matriz lógica de 4 combinaciones: 1) Limpio (ambos dicen NO); 2) Bloqueado por regex (`origin: 'regex'`); 3) Bloqueado por Laya (`origin: 'laya'`); 4) Bloqueado por ambos (`origin: 'ambos'`). Si Laya está apagada o excede el timeout, degrada limpiamente al regex sin arrojar falsos positivos. |
| `server.py` | Backend | Servidor HTTP Python multi-hilo, resolución dinámica de `DB_PATH`, deduplicación canónica en `get_all_synced_topics()` con base canónica `fuentes/`, libro mayor de usos auditables en `/api/admin/codes`, daemon de purga con doble inmunidad (`run_auto_purge_daemon`), ordenamiento canónico de `indexCode` en `/api/sync-topics`, endpoint `/api/fuentes` con escritura correcta de encabezados y cuerpo, endpoints de subida de apuntes eliminados (HTTP 404), Zero Exposure y rate limiting. **Venta Manual Ágil (v7.35, PROMPT 028):** endpoints `POST /api/admin/create-code-batch`, `POST /api/admin/purge-codes` y `POST /api/admin/update-code-label` protegidos por `_require_admin_manage` (401 PIN inválido, 403 rol docente, 429 limiter). **Persistencia de sesión, cookies dual-mode y preflight CORS (v7.28, PROMPT 022):** `make_session_cookie` y `make_logout_cookie` con contrato dual-mode, preflight `do_OPTIONS` (HTTP 204), e inyección automática de CORS. **Cuenta de Presentación Docente (v7.16):** mapa de roles `ADMIN_PIN_HASHES`, `verify_admin_pin()` en tiempo constante, `_require_admin_manage()` con `ADMIN_LIMITER` (429), 401 clave inválida y 403 modo docente. **Sincronización de avance de cédulas (v7.12):** endpoints `GET/POST /api/user/topic-mastery` con sesión obligatoria (401), lote máx. 500 y resolución LWW. **Endpoint Laya Decision Router (v7.47, PROMPT 038):** monta `POST /api/laya/predict` con límite estricto de carga útil de 4 KB (HTTP 413 ante payload excesivo), rate-limiting en memoria por IP (1 petición cada 150 ms, HTTP 429 ante ráfagas), validación de tarea registrada en `LAYA_TASKS` (HTTP 400 ante tareas no autorizadas) y delegación directa y segura a `scripts.laya_router.predict()` con fallback determinista de salida transparente. |
...
| `scripts/lint_fuentes.py` | Validador | **Lint pre-ingesta (v7.24, PROMPT 019):** valida cada `.md` de `fuentes/` sin modificar nada (ERROR: encoding, > 512 KB, colisión de tupla natural con mensaje de registro; WARN: pocas secciones, `##` duplicados, capítulo ambiguo, descalce índice-vs-cuerpo como WARN `indice-descalce` no bloqueante (v7.41, PROMPT 032: el cuerpo manda al publicar, el índice es declarativo/visual)). Exit 1 ante ERROR; hook en CI antes de regenerar. |
...
| `scripts/make_mocks.py` | Fixtures | **Fábrica determinista de mocks solo-tests (v7.23, PROMPT 018):** corpus sintético por materia (civil/procesal/constitucional + edge: diminuto, vacío, latin-1, módulos duplicados, gigante > 64 KB) con manifest de salidas esperadas; escribe solo en temp dirs (exit 3 dentro del repo); stems `mock*` para aserción de aislamiento; modo `--benchmark N`. |
...
| `db.py` | Base de Datos | Conexión relacional SQLite dinámica (`DB_PATH`), tabla de auditoría indestructible `access_code_usages`, auto-migración y backfill en `init_db()`, funciones `restore_user_access_code()`, convalidación atómica e idempotente `link_user_code()`, hashing PBKDF2, lockout, y purga `purge_unvalidated_accounts` con doble exclusión inmunológica. **Venta Manual Ágil (v7.35, PROMPT 028):** generación criptográfica segura `generate_random_access_code(prefix)` sobre alfabeto `[A-Z2-9]`, emisión masiva atómica all-or-nothing `create_access_code_batch(...)` (1–50 códigos), purga segura de stock ocioso `purge_unused_codes(older_than_days, prefix, dry_run=True)` con cuádruple exclusión defensiva (preserva usados, vinculados, pre-asignados y revocados), actualización en vivo de etiquetas `update_access_code_label(code, label)` y enforcement estricto de `assigned_email` en `create_user` y `link_user_code` como escudo anti-préstamo (rechazo HTTP 400). **Tabla `user_topic_mastery` (v7.12):** registro de dominio de cédulas por usuario con upsert **LWW** y purga en cascada junto a `user_progress`. |
...
| `.github/workflows/deploy-pages.yml` | CI / CD | Workflow de despliegue en GitHub Pages. **Regenera cruces dogmáticos, datos del grafo vivo, índice temático y Vault en CI** (`python3 scripts/build_cross_connections.py`, `python3 scripts/build_graph_data.py`, `python3 generate_clean_notes_data.py` y `python3 build_vault_index.py`) a partir de `fuentes/` antes de publicar el artefacto estático, garantizando que `git push` mantenga al día las 205 cédulas, los cruces dogmáticos, el grafo vivo de conceptos y el motor de búsqueda en el despliegue estático. |
| `test_unlock_auth_flow.cjs` | Test E2E | Suite de 219 pruebas (100% PASS) que valida el flujo Candado -> Registro Directo -> Versión Demo -> Convalidación -> Pase Activo, endpoints de administración, consumo atómico, normalización, simulación de purga 48h, la Sección 17 con 24 aserciones de persistencia multi-dispositivo y auditoría en `access_code_usages`, la Sección 16.10 (v7.16) con 27 aserciones de la Cuenta de Administrador de Presentación Docente, la **Sección 20 (12 aserciones v7.48, Fase E1):** `GET /api/health` con forma `{ok, version, time, uptimeSec, db}`, rate-limit Laya intacto ante ráfaga concurrente (429), racha sin sesión toda 401 sin consumir cuota E1, y presencia en fuente de `/api/health`, `_check_e1_rate_limit` y `Retry-After`, y la **Sección 19 (34 aserciones v7.35, PROMPT 028)** que valida exhaustivamente la Venta Manual Ágil: generación atómica de lotes de hasta 50 códigos en `POST /api/admin/create-code-batch` con reversión total all-or-nothing ante errores, pre-asignación estricta de correo en `POST /api/auth/link-code` y `POST /api/auth/register` con rechazo HTTP 400 ante discrepancia de titularidad y compatibilidad con stock genérico, purga de stock ocioso en `POST /api/admin/purge-codes` con previsualización `dry_run=True`, cuádruple exclusión defensiva (preservación de códigos consumidos, pre-asignados y revocados), actualización en vivo de etiquetas de estado en `POST /api/admin/update-code-label`, y bloqueo estricto con HTTP 403 para la cuenta docente en los 3 nuevos endpoints admin. |
| `test_e2e_case_flow.cjs` | Test E2E | Suite integral de 1496 pruebas (100% PASS, medicion v7.48 sobre arbol final; ver discrepancia v7.47 en bitacora) que audita casos IA, seguridad, rúbrica AIME, confidencialidad, multi-dispositivo, limitación en Modo Demo vs. Pase Activo, canonización de fuentes, ordenamiento canónico `indexCode`, los 13 arquetipos con nutrición viva, la Subsección 23.6 con nutrición dinámica y citas corpus-driven (v7.10), la Subsección 24.4 con el Desarrollador de Preguntas de Verificación del Agente (`QuestionDeveloper`, v7.11), la **Subsección 24.5** con el formato simple y solo dogmático del `QuestionDeveloper` (combinación I–IV con 3 verdaderas ancladas y 1 falsa anti-verbatim, correcta como conjunto exacto de verdaderas, distractores que incluyen falsa u omiten verdadera, opciones ≤ 120 chars, cero absurdos, plausibilidad, sencillez sin exigir que la correcta sea la más desarrollada, plazo numérico literal «18 días hábiles» sin cómputo aritmético, casos aparte y regresión 24.4, v7.17), la **Sección 25** con la sincronización multi-dispositivo del avance de cédulas (57 tests, v7.12), la **Sección 26** con la compilación de estructura desde `MODELOS_DE_PRUEBA/` (6 tests, v7.17), la **Sección 27 (v7.18, PROMPT 015)** con el gateo de visibilidad del cuestionario por rol admin (8 tests: cero inyección en alumnos, 4 preguntas con badge `.quiz-admin-lab-badge` en admin/docente, `#btn-bottom-mastery` visible, reactividad tras logout y regresión estructural), la **Sección 28 (v7.22, PROMPT 017)** con el Vault de Conocimiento e índice semántico estático (11 tests: existencia y presupuesto ≤ 600 KB de `vault_index.json`, totalDocs derivado dinámicamente de `all_afg_topics.json`, determinismo SHA-256 de `build_vault_index.py`, contrato de `VaultSearch.searchVault`, ranking semántico prioritario para "emplazamiento" en cap 10, resolución exacta de "18 días hábiles", rechazo de términos inexistentes, filtro por materia civil, garantía anti-alucinaciones verbatim en todas las cédulas, sanitización XSS con `<mark>` y latencia < 50 ms), la **Sección 29 (v7.25, PROMPT 020)** con la búsqueda normalizada, sinónimos, filtros e historial (11 tests: paridad Python ↔ JS en 6 pares fixture, 33 grupos en `sinonimos.json` y `js/sinonimos.js`, recall 100% en top-5, precisión intacta en top-1, filtros por disciplina civil y capítulo 10 sin reordenar scoring relativo, persistencia y aislamiento de `localStorage['vault_recent_searches']` sin fuga en exportación de progreso, sanitización anti-XSS y selectores DOM en `index.html` y `sidebar.css`), la **Sección 30 (v7.27, PROMPT 021)** con el motor de Q&A extractivo puro (11 tests: definición puntual con top-1 en cap 10, comparativa yuxtapuesta sin texto propio entre 2 bloques o materias distintas, panorama por disciplina sin repetición, falta de cobertura con mensaje honesto y sugerencia dogmática por keywords, gate de salida estricto con falla cerrada, determinismo absoluto JSON idéntico, latencia < 100 ms, contrato de `App.openTopic(id, { highlight })` con resaltado `<mark class="vault-highlight">`, ranking local de huecos `vault_uncovered_queries` con tope de 30 y aislamiento de exportación de progreso, sanitización XSS e integración DOM), la **Sección 31 (v7.29, PROMPT 023)** con el Buscador Global del Header con Modos Semántico y Q&A (44 tests: selectores DOM, tokens CSS azul/dorado, touch targets móviles $\ge 44 \times 44\text{ px}$, conmutación y persistencia en `localStorage['global_search_mode']`, delegación transparente a `searchVault` y `composeAnswer`, blindaje anti-inyección: tope 200, nulos/zero-width purgados, escape HTML, ReDoS safe < 100 ms, jailbreaks inocuos en vacío honesto, navegación interactiva con highlight y auto-conmutación a vista `topics`), la **Refactorización de Conteos Dinámicos y Protocolo de Crecimiento del Canon (v7.30, PROMPT 024)** eliminando números mágicos en aserciones por variables derivadas de `all_afg_topics.json`, la **Sección 32 (v7.34, PROMPT 027)** con el Buscador Único al Header y Cero Duplicados en Sidebar (13 tests: unicidad estricta de 1 input de búsqueda `#global-search-input` en todo el DOM, ausencia de inputs en el sidebar, verificación funcional de los 3 motores en el header dropdown, filtros e historial unificados, tokens y overlay móvil responsive, y conmutación a `topics`), la **Sección 33 (v7.38, PROMPT 031)** con 28 aserciones de Cruces Dogmáticos Enriquecidos y Citas del Canon (presupuesto $\le 300\text{ KB}$, 103 temas con 3 cruces, 0 auto-enlaces, allowlist de targets, taxonomía de 6 tipos, citas verbatim $\le 140$ chars, compuerta $\ge 3$ bigramas, CSS $\ge 44\text{ px}$, `renderConnectionCardsHtml`, salto interactivo con highlight, empty state honesto y certificación de reporte forense), la **Sección 34 (v7.41, PROMPT 032)** con 25 aserciones del índice jerárquico visual (etiqueta `M.C.S`, unicidad 205/205, colapsabilidad y accesibilidad ARIA), la **Sección 35 (v7.43, PROMPT 034)** con 10 aserciones del Grafo Vivo: verificación de `graph_data.json` y `js/graph-data.js` (`window.GRAPH_DATA`), correspondencia 1:1 de los 205 nodos con el canon de temas, 545 enlaces únicos no dirigidos sin colisiones ni duplicados recíprocos $A \leftrightarrow B$ (`duplicatesCollapsed: 70`), 0 huérfanos, 0 auto-enlaces, taxonomía cerrada de 6 tipos, cálculo de peso numérico `val`, presupuesto $\le 615.00$ KB (171.96 KB real), y certificación determinista idéntica en `graph_report.json` con SHA-256; la **Sección 36 (v7.44, PROMPT 035)** con 10 aserciones del Grafo Vivo en Canvas: carga de `js/graph-data.js` previa a `js/concept-graph.js`, ingesta de 205 nodos y 545 enlaces dogmáticos, ausencia total de IDs legacy cuando `GRAPH_DATA` está presente, radio proporcional al grado dogmático [18, 30] px, fallback defensivo elegante hacia `INITIAL_DATA.graph` (14 nodos) con advertencia explícita en consola si `GRAPH_DATA` no existe, resolución de muestra representativa de 10 capítulos dogmáticos a cédulas reales, blindaje XSS mediante `SecurityShield.escapeHtml` en inspector, accesibilidad ARIA (`role="toolbar"`, `tabindex="0"`, `aria-label`), leyenda dinámica (`N cédulas · M nexos`), constantes físicas nombradas con corte de repulsión a 320 px y auto-pausa de animación en `document.hidden` / vista no-grafo, y zoom funcional; la **Sección 37 (v7.45, PROMPT 036)** con 10 aserciones del Inspector Cerebro (Ficha de Conocimiento, cruces dogmáticos, dominio reactivo y QA extractivo mínimo): renderizado de metadata dogmática (`§ indexCode`, capítulo, `displayHierCode` `M.C.S`), cruces dogmáticos con tarjeta unificada `.linked-connection-card`, citas verbatim con `<mark class="vault-highlight">` y botón `.btn-jump-connection`, navegación segura en allowlist anti open-redirect (`ConceptGraph.isValidTopicId`), navegación entre nodos vinculados, alternancia reactiva de dominio `Dominada ✓` (`StorageService.toggleTopicMastery`) con halo esmeralda `#10b981` (stroke 2.5px) verificado en canvas, QA extractivo on-demand con `QAComposer.composeAnswer(label, "definicion")`, accesibilidad ARIA (`role="complementary"`, `aria-label`) y touch targets $\ge 44\text{ px}$ en móviles de 360 px; la **Sección 38 (v7.46, PROMPT 037)** con 10 aserciones exhaustivas del Grafo Vivo a Escala (Toolbar, Filtros, Modo Foco y 60 FPS): búsqueda local delegada al Vault con match canónico en "cautelares", centrado de cámara `panX`/`panY` y selección de nodo; despliegue en inspector (`.inspector-open`, título y botón de estudio); búsqueda sin match con query no canónica (`"xyzqwk987zz"`), inmutabilidad de viewport y feedback honesto; filtro por materia civil con atenuación a 0.12 sin mutar `this.nodes`; selector de capítulos ordenado por disciplina y min `indexCode`; filtro por capítulo con atenuación a 0.12 sin borrar nodos; toggle de mastery overlay y reactividad de contador `Dominadas X/N`; modo foco por vecindario con nodo y vecinos directos a opacidad 1.0 vs 0.10 resto, aristas incidentes doradas (`#d4af37`), y deselección completa; cap de 400 aristas con >400 enlaces candidatos y leyenda reactiva; y simulación pausada con medición de tiempo de frame en `draw()` comprobado en **8.1ms < 16.6ms** (60 FPS); la **Sección 39 (v7.46.1)** con 6 aserciones de estabilidad numérica de la física del grafo (signo Hooke opuesto en resortes, constante `MAX_VELOCITY = 12`, 600 frames con 0 NaN, coordenadas acotadas < 20000 y velocidad controlada); y la **Sección 40 (v7.47, PROMPT 038)** con 8 aserciones integrales sobre el Laya Decision Router y Pilotos: verificación de `scripts/setup_laya.py` con `--check`, contrato formal `LAYA_CONTRACT.md`, auditoría estática y CLI `predict` con `--state-file` en `scripts/laya_router.py`, endpoint `POST /api/laya/predict` en `server.py` con rate limit (150ms) y límite de 4 KB (HTTP 413), Piloto 1 de autodetección de Q&A en `js/app.js` con override manual del alumno, Piloto 2 de puntuación de riesgo aditiva en `SecurityShield` con umbral `LAYA_SHIELD_THRESHOLD = 0.85` y matriz OR de 4 combinaciones, bitácora `router.jsonl` con el campo `reason` en cada línea y cero PII, y benchmark formal de 20 consultas del Vault con 95.0% de precisión y latencia p50 sub-500ms (1526/1526 pruebas exitosas en la suite). |
| `test_mobile_header_theme.cjs` | Test E2E / Visual | **Suite de 117 pruebas (100% PASS)** que verifica el layout móvil solo-íconos, touch targets $\ge 44 \times 44\text{ px}$, accesibilidad ARIA, inmutabilidad de Dark Academy, ausencia absoluta de selectores/botones de modo claro, la **Sección 5 (v7.36, PROMPT 029)** con 24 aserciones sobre la escala tipográfica canónica de 5 pasos en `:root` (`--lh-title`, `--lh-heading`, `--lh-body`, `--lh-table`, `--lh-dense`), `.table-responsive` con `overflow-x: auto` e iOS scrolling, la **Sección 6 (v7.37, PROMPT 030)** con 41 aserciones que auditan: escala canónica de espaciado 8pt (`--space-1` a `--space-8`), z-index unificado (`--z-base` a `--z-toast`), contención de scroll (`overscroll-behavior: contain`), `content-visibility: auto` en el sidebar, anchos mínimos anti-layout shift (`.badge-count` y `.topic-item-right`), roles y estados ARIA (`role="tablist"`, `role="tab"`, `aria-selected`, `aria-expanded`, `aria-current`), universal `:focus-visible` dorado, `@media (prefers-reduced-motion: reduce)`, y componentes `.ui-skeleton`, `.ui-state-empty`, `.ui-state-error`, la **Sección 7 (v7.41, PROMPT 032)** con 14 aserciones del índice jerárquico visual (badge `min-width:52px` + `tabular-nums`, `.chapter-cedulas-count`, `:focus-visible` en cabecera, `min-height:44px`, `displayHierCode`/`data-hier-code`/`aria-expanded`/teclado/auto-scroll, tooltip con trazabilidad y unicidad 205/205 `M.C.S`), la **Sección 8 (v7.45, PROMPT 036)** con 6 aserciones de estilos móviles del Inspector Cerebro: existencia de selectores `.inspector-meta-row`, `.inspector-meta-badge`, `.inspector-hier-badge`, `.inspector-actions-row`, `.btn-mastery-toggle`, `.inspector-qa-section`, contención de scroll móvil (`max-height: 70vh`, `overflow-x: hidden`, `box-sizing: border-box`), touch targets interactivos $\ge 44\text{ px}$ y estado dominado `.is-mastered` con borde esmeralda `#10b981`, y la **Sección 9 (v7.46, PROMPT 037)** con 12 aserciones de estilos móviles y accesibilidad del Toolbar del Grafo: existencia y posicionamiento de `.graph-toolbar`, controles de zoom `.graph-floating-controls` reubicados a `top: 76px; left: 16px;` en desktop y `bottom: 80px; right: 10px;` en móvil evitando colisión, campo `#graph-search-input` con `maxlength="200"`, botón `#graph-search-clear`, feedback `#graph-search-feedback` con `aria-live="polite"`, píldoras `#graph-filter-pills` con scroll horizontal y `aria-pressed`, selector `#graph-filter-chapter`, switch `#graph-toggle-mastery-overlay`, touch targets $\ge 44\text{ px}$ en todos los interactivos y contención de scroll a 360 px con `max-height: 48vh` (117/117 pruebas exitosas). |
| `test_deduplication_flow.cjs` | Test Integración | Suite de 14 pruebas (100% PASS) que verifica deduplicación canónica en `all_afg_topics.json`, unicidad de `indexCode`, auto-curación de `2 * CANON_ESPERADO` a `CANON_ESPERADO` tópicos en `StorageService.getData()`, invariante de `discover_fuentes_files('fuentes') == []` (8 registrados en `FILES_CONFIG`), auto-seccionado modular/headings dinámico con fixture temporal (v7.9), orden temario canónico de Procesal v7.19.1 (Discusión 2.10–2.41 → La Prueba 2.42–2.55 → Sentencia 2.56–2.59), auto-posicionamiento en temario de apuntes nuevos como `SUCESIONES.md` (v7.19, PROMPT 016), orden del sidebar por min `indexCode` sin reorden numérico por `chapterNumber` (v7.19.1), y **Test 10 (v7.30, PROMPT 024)** con el Protocolo de Crecimiento del Canon (crecimiento dinámico de 103 a 105 con fixture temporal, auto-descubrimiento, indexCode continuo `1.1..1.36` y limpieza defensiva sin huérfanos), y **Tests 11–12 (v7.41, PROMPT 032)** con etiqueta jerárquica `M.C.S` posicional única por capítulo (205/205, Comodato cap 13 → `1.13.1`) y WARN `indice-descalce` del lint ante fixture descalzado. |
| `PROMPTS/` | Documentación Operativa | **Puente humano ↔ Agente Antigravity IDE.** Carpeta canónica de prompts de ingeniería listos para ejecutar (índice `README.md`, plantilla `_TEMPLATE.md`, prompts numerados: 001 subida de apuntes ⚠️ **superseded por 007** (v7.8), 002 agente de casos ✅ v7.3, 003 índice unificado ✅ v7.2, 004 registro persistente de códigos de acceso usados ✅ v7.4, 005 header móvil ✅ v7.5, 006 diagnóstico Render ✅ v7.6, 007 fuentes/ canónica + eliminación subida admin + regen CI ✅ v7.8, 008 índice automático desde `fuentes/` ✅ v7.9, 009 nutrición viva del agente ✅ v7.10, 010 desarrollador de preguntas de grado ✅ v7.11, 011 sincronización multi-dispositivo del avance de cédulas ✅ v7.12, 012 perfeccionamiento de las preguntas de verificación (perfil «manejo», concepto mejor desarrollado, características, procedencia, plazos, definiciones — redacción en términos de los apuntes) ✅ Implementado (v7.15), 013 cuenta de administrador de presentación docente (clave `profesoresafg202602`, rol `docente`: visualización total con herramientas de prueba — casos, preguntas, grafo, cobertura — sin generar/revocar/listar códigos ni gestionar, subir, importar ni descargar archivos; rol `admin` intacto) ✅ Implementado (v7.16) con endurecimiento cibernético de claves admin: solo hashes SHA-256, comparación en tiempo constante, limiter anti fuerza bruta `ADMIN_LIMITER` por IP (429) y 403 por rol en servidor, 014 preguntas de verificación simples y claras (formato combinación I-IV, solo dogmáticas) + modelos de prueba estructurales ✅ Implementado (v7.17), 015 visibilidad admin de preguntas de verificación de cédula (modo laboratorio de perfeccionamiento) ✅ Implementado (v7.18), **016 temario canónico y ordenamiento automático del índice según Examen de Grado ✅ Implementado (v7.19; corrección dogmática Discusión→Prueba→Sentencia + sidebar por min indexCode v7.19.1)** — orden oficial del Examen de Grado establecido declarativamente en `TEMARIO_CANONICO` / `TEMARIO_ORDER_MAP`, reposicionamiento de Juicio Ordinario de Mayor Cuantía (`2.10`–`2.45`) ANTES que La Prueba (`2.46`–`2.59`), auto-inferencia de posición para apuntes nuevos en `fuentes/` con `infer_temario_position()`, re-ordenamiento in-place de `all_sections` y 100% de pruebas pasando, **017 vault de conocimiento con índice semántico estático ✅ Implementado (v7.22)** — búsqueda client-side BM25-lite sobre las 103 cédulas canónicas sin base vectorial (`build_vault_index.py` → `vault_index.json` de 535.42 KB ≤ 600 KB + `js/vault-search.js`), snippets verbatim y citación exacta en local y GitHub Pages). 018 fábrica de mocks solo-tests (inyección al Vault rechazada) ✅ Implementado (v7.23). 019 ingesta confiable lint+diff ✅ Implementado (v7.24). **020 búsqueda con normalización contractada, sinónimos jurídicos, filtros e historial reciente ✅ Implementado (v7.25)**. **021 Q&A extractivo pregunta→citas ✅ Implementado (v7.27, PROMPT 021)** — motor `js/qa-composer.js`, modos definición/panorama/comparativa, gate cerrado, salto interactivo con resaltado `<mark class="vault-highlight">`, sugerencia dogmática y ranking local de huecos. 022 persistencia de sesión sin cuenta nueva + sin Export/Import ✅ Implementado (v7.28). **023 buscador global header Semántico/Q&A ✅ Implementado (v7.29, PROMPT 023)** — selector segmentador en `#global-search-container`, pills Semántico azul/lupa y Q&A dorado/help-circle, persistencia `localStorage['global_search_mode']`, un solo controlador que delega en `searchVault()` / `composeAnswer()`, blindaje anti-inyección (tope 200, nulos/zero-width purgados, escape HTML, ReDoS safe, jailbreaks neutros) y navegación con highlight y auto-conmutación a `topics`. **024 protocolo crecimiento del canon ✅ Implementado (v7.30, Antigravity)** — eliminación de números mágicos (`103`, `34`, `59`, `10`) en suites y documentación; conteos derivados dinámicamente de `all_afg_topics.json`; preservación de invariantes duros (unicidad de `indexCode`, continuidad estricta `N.M` sin huecos, orden relativo dogmático); fixture de prueba de crecimiento a 105 con 100% PASS en 5 suites. 025 ciclo de mejora banco+ledger ✅ Ejecutado (Nexo: 655 pares, 596 válidos). 026 mejora generador con anclaje ✅ Implementado (v7.32, Nexo). **027 buscador único en header ✅ Implementado (v7.34, Antigravity)** — todo al header (`#global-search-container` / `#search-results-dropdown`), cero inputs en `aside#app-sidebar`, exactamente 1 input en el DOM (`#global-search-input`), filtros, historial y Q&A unificados en dropdown, y 100% de suites pasando. **028 venta manual ágil + plantillas DM en VENTAS/ + purga stock ocioso ✅ Implementado (v7.35, Antigravity)** — lotes de 1 a 50 códigos en 1 llamada atómica (`POST /api/admin/create-code-batch`), pre-asignación estricta de correo anti-préstamo (rechazo 400), purga de stock ocioso (`POST /api/admin/purge-codes`) con `dry_run=True` por defecto y blindaje de usados/asignados/revocados, estados operativos `[PENDIENTE]` / `[ENTREGADO]` en `label` con toggle en 1 clic y badges táctiles, filtro client-side en tiempo real, cero migraciones de BD y 207 tests PASS. **029 repaso visual tablas+interlineado ✅ Implementado (v7.36, Antigravity)** — auditoría visual PC (1440px) y móvil (360px), rescate de 40 tablas rotas como párrafos en `MarkdownParser.parseTables` con look-ahead multilínea resiliente, atributos WCAG AA (`tabindex="0"`, `role="region"`, `aria-label`) y restitución de `<br>` en celdas, desplazamiento horizontal táctil independiente con `.table-responsive` (`overflow-x: auto`), anchos mínimos (`580px`/`520px`), eliminación de recorte ancestral `overflow-x: hidden` en móvil, erradicación de aplastamiento silábico vertical (`word-break: normal`), escala canónica unificada de interlineado en 5 pasos (`--lh-title: 1.28`, `--lh-heading: 1.35`, `--lh-body: 1.70`, `--lh-table: 1.60`, `--lh-dense: 1.50`) y cero desborde horizontal de página verificado. **030 mejora general de interfaz: shell óptimo, jerarquía y accesibilidad ✅ Implementado (v7.37, Antigravity)** — tokens canónicos de espaciado 8pt (`--space-1` a `--space-8`) y z-index unificado (`--z-base: 1` a `--z-toast: 1000`) en `:root`, resolución de inversión de capas dropdown vs. modal backdrop, contención de scroll (`overscroll-behavior: contain`), `content-visibility: auto` en el sidebar, columna de lectura ergonómica centrada en PC, erradicación de saltos de diseño (layout shift) en badges y filas de tópicos con anchos mínimos defensivos, semántica ARIA enriquecida (`role="tablist"`, `role="tab"`, `aria-selected`, `aria-expanded`, `aria-current`), universal `:focus-visible` dorado de alto contraste, `@media (prefers-reduced-motion: reduce)`, skeletons y estados de feedback unificados, y 100% PASS en las 4 suites (1.478 pruebas). **031 cruces dogmáticos con vault y agentes ✅ Implementado (v7.38, Antigravity)** — erradicación de plantillas genéricas, resolución de IDs legados, motor offline BM25 + compuerta de bigramas ($\ge 3$), taxonomía cerrada de 6 tipos, citas verbatim $\le 140$ caracteres de `targetTopic.content`, presupuesto $\le 300\text{ KB}$ (`dogmatic_connections.json`), reporte forense (`cruces_report.json`), helper unificado `renderConnectionCardsHtml`, botón de salto interactivo con highlight (`App.openTopic(targetId, { highlight: snippet })`), allowlist anti open-redirect, touch targets $\ge 44 \times 44\text{ px}$, integración en CI (`deploy-pages.yml`) y 1.476/1.476 pruebas pasando en las 4 suites. **032 índice jerárquico visual ✅ Implementado (v7.41)** — etiqueta `M.C.S` solo-display + capítulos colapsables. **033 ciclo de nutrición y mejora continua ✅ Implementado (v7.42)** — frase `nutrir sistema`, comando `node scripts/ciclo_nutricion.cjs`, historial `nutricion_historial.jsonl`. **034 grafo vivo: builder offline desde canon + cruces ✅ Implementado (v7.43, Antigravity)** — capa de datos pura, builder determinista `scripts/build_graph_data.py`, 205 nodos, 545 enlaces únicos (70 duplicados recíprocos $A \leftrightarrow B$ colapsados, 0 huérfanos, 0 auto-enlaces), artefactos `graph_data.json`, `js/graph-data.js` y `graph_report.json`, presupuesto $\le 615\text{ KB}$ (171.96 KB real), integración en CI/CD y 100% PASS en las 4 suites (1648 tests). **035 consumo de GRAPH_DATA en Canvas ✅ Implementado (v7.44, Antigravity)** — ingesta de 205 nodos y 545 nexos dogmáticos en canvas sin alterar UI, radio por grado, truncamiento a 28 caracteres, leyenda dinámica, optimización de física y corte a 320 px, auto-pausa y 100% PASS en las 4 suites (1.678 tests). **036 inspector cerebro: ficha de conocimiento por nodo, cruces dogmáticos, mastery y QA mínimo ✅ Implementado (v7.45, Antigravity)** — ficha enriquecida de conocimiento en `#graph-inspector`, cruces dogmáticos reutilizando `renderConnectionCardsHtml`, botón de salto interactivo, alternancia de dominio `Dominada ✓` reactiva con halo esmeralda `#10b981` en canvas, QA extractivo mínimo on-demand, allowlist anti open-redirect, touch targets $\ge 44\text{ px}$ en 360 px y 100% PASS en las 4 suites (1.740 tests). **037 grafo vivo: toolbar, filtros atenuantes, modo foco vecindario, cap de 400 aristas y 60 FPS ✅ Implementado (v7.46, Antigravity)** — barra de herramientas superior `#graph-toolbar` con búsqueda debounced (150ms) y sanitizada delegada a `VaultSearch` con fallback local directo y centrado en viewport (`centerNode`), filtros atenuantes por materia y capítulo (opacidad 0.12 sin mutar arrays) con selector ordenado por min `indexCode`, switch de overlay de dominio `Dominadas X/N`, modo foco por vecindario a 1 salto (1.0 vs 0.10, aristas incidentes doradas `#d4af37` rotuladas, aristas no incidentes a 0.05), cap de 400 aristas visibles por prioridad (incidentes $\rightarrow$ transversales $\rightarrow$ hubs), optimización crítica O(1) de mastery con precomputación de Set en `draw()` garantizando 60 FPS (< 10ms frame time), touch targets móviles $\ge 44\text{ px}$ y cierre del ciclo canónico de visualización del grafo (034–037) con 1.750/1.750 pruebas exitosas en las 4 suites (100% PASS). **038 laya router y pilotos Q&A / Shield ✅ Implementado (v7.47, Antigravity)** — integración de Laya (`LAYA_HOME`) como capa de decisión compartida y extensible para el repositorio; instalador canónico versionado `scripts/setup_laya.py` (`--check`, `--smoke-only`); contrato formal `LAYA_CONTRACT.md` con línea roja dogmática inmutable (Laya sugiere, NUNCA decide rúbricas, auth, maestría ni notas) y aislamiento estricto sin dependencias en el repositorio; router determinista `scripts/laya_router.py` con registro canónico `LAYA_TASKS` (`qa_mode`, `shield_risk`), auditoría de tareas prohibidas, CLI `predict` con flag `--state-file` (compatible con BOM de PowerShell), cliente `query_warm_worker` y servidor worker HTTP residente en puerto 47888 (`run_worker_server`); bitácora `router.jsonl` sin PII con campo `reason` en cada línea; endpoint seguro en `server.py` `POST /api/laya/predict` (límite 4 KB, rate limiter en memoria 150ms, HTTP 400/413/429); Piloto 1 de autodetección de modo Q&A en `js/app.js` (`#qa-mode-pills`) con supremacía inmutable del override manual del alumno (`qaModeManualOverride`); Piloto 2 de puntuación de riesgo aditiva en `SecurityShield` (`js/security-shield.js`, `js/case-solver.js`) con umbral `LAYA_SHIELD_THRESHOLD = 0.85` en matriz lógica OR paralela de 4 combinaciones con fallback transparente a regex; benchmark formal de 20 consultas doctrinales del Vault (`scripts/benchmark_laya_20.py`, `benchmark_results_038.json`) con 95.0% de precisión, 0 fallbacks, latencia p50 498.4ms y veredicto `VIABLE Y SUPERIOR AL DEFAULT`; Sección 40 en `test_e2e_case_flow.cjs` (8 aserciones) y 100% PASS en las 4 suites (1.864 pruebas totales). Cada prompt exige actualización correlativa de `CONTEXT.md` y 100 % PASS de las suites al implementarse. |
| `.opencode/skills/` | Skills Operativos | **Skills reutilizables del proyecto (v7.20):** `temario-orden` (orden del índice según temario canónico), `nuevo-apunte` (ingesta en `fuentes/` con auto-ubicación), `verificacion-release` (4 suites + commit/push + Pages) y `diagnostico-pagina` (local vs desplegado, caché, localStorage), más `orquestador` (Nexo, cerebro orquestador: optimiza los planes del humano y los ejecuta por fases) y `agente-herramientas` (contrato de herramientas deterministas del Vault para agentes: recuperar con `searchVault`, validar con `assertCitationIntegrity`, jamás responder de memoria). Formato OpenCode (`SKILL.md` con frontmatter `name`/`description`), auto-descubiertos por ID. |

---

## 9. Bitácora Canónica de Versiones e Hitos

* **v7.38 (Cruces Dogmáticos Enriquecidos con Vault, BM25 y Citación Verbatim — PROMPT 031):**
  - **Misión de Ingeniería:** Superar la fragilidad y el vacío de los cruces dogmáticos en el acordeón inline y en el panel lateral derecho del visor de temas, erradicar las plantillas estáticas y genéricas que degradaban la experiencia de estudio, diagnosticar la causa raíz de la desconexión del canon (Fase 0), y construir un motor determinista offline de generación de cruces basado en el Vault BM25, anclaje por bigramas y extracción de citas literales del temario de 103 cédulas, con presupuesto estricto $\le 300$ KB, taxonomía cerrada de 6 categorías, navegación interactiva con resaltado (`<mark class="vault-highlight">`) y compuerta anti open-redirect.
  - **Pilares Técnicos Implementados:**
    1. **Auditoría Forense de Fase 0 y Mapeo Canónico:**
       - Diagnóstico de desalineación en `build_dogmatic_connections.py`: prefijos acortados (`civil-acto-1-1`, `procesal-proc-1-1`, `civil-las -1-1`) no coincidían con `all_afg_topics.json`.
       - Implementación de `resolve_legacy_id()` en `scripts/build_cross_connections.py` para mapear y preservar deterministamente los 80 cruces manuales de alto valor dogmático.
    2. **Motor Determinista Offline BM25 y Compuerta de Bigramas ($\ge 3$):**
       - Extracción y normalización de textos sobre las 103 cédulas canónicas.
       - Cálculo de similitud interdisciplinaria (Civil $\leftrightarrow$ Procesal, Civil $\leftrightarrow$ Constitucional) mediante BM25 adaptado.
       - Compuerta estricta: mínimo 3 bigramas compartidos (`shared_bigrams >= 3`, promedio real 125).
       - Taxonomía cerrada de 6 valores: `Genero-Especie`, `Sustantivo-Procesal`, `Fundamento-Constitucional`, `Excepcion-Procesal`, `Efecto-Patrimonial`, `Materia-Afin`.
       - Extracción de citas textuales exactas (`quote`) $\le 140$ caracteres directamente desde `targetTopic.content`. Cero alucinaciones.
    3. **Presupuesto y Certificación Forense:**
       - `dogmatic_connections.json`: 274.01 KB ($\le 300$ KB presupuestados).
       - `cruces_report.json`: 103/103 temas cubiertos (100%), 3 cruces exactos por tema (309 totales), 0 auto-enlaces, 0 destinos duplicados, 100% de targets en el canon y 100% de citas verbatim comprobadas.
    4. **Componentes y Experiencia de Usuario (`css/topic-viewer.css`, `js/app.js`):**
       - Helper unificado `renderConnectionCardsHtml(connsList)` que alimenta tanto el acordeón inline como el aside panel lateral sin duplicación de código.
       - Erradicación absoluta de textos comodín ("Institución directamente relacionada...").
       - Componentes: `.connection-target-meta` (`§ indexCode · Disciplina`), `.connection-quote-box` con `<mark class="vault-highlight">` y código de sección, `.btn-jump-connection` con touch target móvil $\ge 44 \times 44\text{ px}$.
       - Navegación interactiva segura con `App.openTopic(targetId, { highlight: snippet })` y centrado suave vía `TreeWalker`.
       - Validación en allowlist estricto de IDs canónicos antes de navegar (escudo anti open-redirect).
       - Empty state honesto `.connections-empty-state` con sugerencias del Vault `.unvalidated-suggestion-card` cuando no existan cruces validados.
    5. **Integración en CI/CD y Automatización:**
       - Paso `python3 scripts/build_cross_connections.py` incorporado en `.github/workflows/deploy-pages.yml` previo a `generate_clean_notes_data.py`.
       - Script raíz `build_dogmatic_connections.py` redirigido a `scripts/build_cross_connections.py`.
    6. **Aserciones y Pruebas E2E:**
       - Incorporación de la **Sección 33 (28 nuevas aserciones v7.38, PROMPT 031)** en `test_e2e_case_flow.cjs` para un total de 1.172/1.172 pruebas exitosas.
       - 100% PASS en las 4 suites de prueba del proyecto (1.476/1.476 aserciones pasando).
  - **Archivos Modificados:**
    1. `scripts/build_cross_connections.py`: Generador determinista offline con BM25, compuerta de bigramas y citas verbatim.
    2. `build_dogmatic_connections.py`: Wrapper que delega en `scripts/build_cross_connections.py`.
    3. `dogmatic_connections.json`: Artefacto JSON generado (274.01 KB).
    4. `cruces_report.json`: Reporte de auditoría y certificación forense.
    5. `all_afg_topics.json` y `js/data.js`: Regenerados con conexiones validadas sincronizadas.
    6. `vault_index.json` y `js/vault-index.js`: Reindexados con presupuesto controlado (535.42 KB).
    7. `.github/workflows/deploy-pages.yml`: Pipeline de CI con generación automática de cruces dogmáticos.
    8. `css/topic-viewer.css`: Estilos de tarjetas de cruce, meta, quote box, botón de salto $\ge 44\text{ px}$, empty state y sugerencias no validadas.
    9. `js/app.js`: Helper unificado `renderConnectionCardsHtml`, botón de salto interactivo, highlight por TreeWalker, allowlist anti open-redirect y empty state con Vault.
    10. `test_e2e_case_flow.cjs`: Sección 33 con 28 aserciones de validación integral (1.172 tests PASS).
    11. `PROMPTS/README.md`: Actualización del prompt 031 a implementado.
    12. `CONTEXT.md`: Incorporación de la Sección 7.6, actualización de 7.7, mapa de archivos en Sección 8 y registro de hito en Sección 9.

* **v7.37 (Shell Óptimo, Jerarquía y Accesibilidad WCAG AA — PROMPT 030):**
  - **Misión de Ingeniería:** Ejecutar una auditoría visual y forense de la interfaz en escritorio (1440 px) y móvil (360 px), resolver una lista cerrada de 12 defectos visuales y de accesibilidad estructurales, formalizar la jerarquía de z-index y el espaciado 8pt en tokens canónicos de `:root`, eliminar el scroll chaining mediante contención de desplazamiento, optimizar el renderizado del sidebar con `content-visibility: auto`, erradicar saltos de diseño (layout shift) en badges y filas de tópicos, enriquecer la semántica ARIA (`role="tablist"`, `role="tab"`, `aria-selected`, `aria-expanded`, `aria-current`), y añadir soporte completo de accesibilidad WCAG 2.1 AA (`:focus-visible` universal, `@media (prefers-reduced-motion: reduce)`, skeletons y estados vacíos unificados).
  - **Pilares Técnicos Implementados:**
    1. **Tokens Canónicos en `:root` (`css/main.css`):**
       - Espaciado 8pt: `--space-1: 4px;` a `--space-8: 64px;`.
       - Capas Z-Index: `--z-base: 1`, `--z-surface: 10`, `--z-sidebar: 40`, `--z-header: 50`, `--z-dropdown: 100`, `--z-overlay: 200`, `--z-sidebar-mobile: 250`, `--z-modal: 500`, `--z-toast: 1000`.
       - Corrección de la inversión de capas: `#search-results-dropdown` migrado a `var(--z-dropdown)` (100) y `.modal-backdrop` a `var(--z-modal)` (500), asegurando que los diálogos modales queden siempre sobre los dropdowns de cabecera.
    2. **Contención de Scroll y Rendimiento (`css/sidebar.css`, `css/topic-viewer.css`):**
       - Inclusión de `overscroll-behavior: contain;` en `.app-sidebar`, `.sidebar-tree` y `.topic-reading-col`, evitando el encadenamiento de scroll al body.
       - Inclusión de `content-visibility: auto; contain-intrinsic-size: 0 44px;` en `.category-group` del sidebar temático, aligerando el pintado de las 103 cédulas.
       - Centrado ergonómico de la columna de lectura en PC con `margin: 0 auto; max-width: 940px;`.
    3. **Prevención de Layout Shifts y Reserva de Espacio:**
       - `#case-count-badge` y `.badge-count` con `min-width: 20px; display: inline-flex; align-items: center; justify-content: center; box-sizing: border-box;` (y `18px` en móvil), previniendo layout shift en la pestaña adyacente de la cabecera.
       - `.topic-item-right` con `min-width: 24px; justify-content: flex-end;`, asegurando alineación vertical consistente sin saltos en el árbol.
    4. **Accesibilidad ARIA y Sincronización Reactiva (`index.html`, `js/app.js`):**
       - `.header-nav` con `role="tablist"`, pestañas con `role="tab"` y `aria-selected` dinámico sincronizado en `App.switchView()`.
       - `#btn-toggle-sidebar` con `aria-expanded` dinámico sincronizado al abrir o cerrar el panel en `App.setupNavigation()`.
       - Cédula activa en el árbol temático con `aria-current="true"` en `App.renderSidebar()`.
       - Iconos Lucide con `aria-hidden="true"`.
       - Cierre de dropdown con `Escape` y restauración de foco limpia a `#global-search-input`.
    5. **Accesibilidad Universal WCAG AA y Estados UI:**
       - `:focus-visible` con anillo dorado nítido de 2px (`var(--gold-primary)`).
       - Regla `@media (prefers-reduced-motion: reduce)` para inhabilitar transiciones en usuarios sensibles.
       - Clases unificadas `.ui-skeleton` (animación shimmer), `.ui-state-empty`, `.sidebar-empty-state` y `.ui-state-error`.
  - **Archivos Modificados:**
    1. `css/main.css`: Tokens canónicos de espaciado 8pt y z-index en `:root`, aplicación de tokens en `.app-header`, `.search-dropdown`, `.toast-container`, `.badge-count` con `min-width`, regla universal `:focus-visible`, bloque `@media (prefers-reduced-motion: reduce)`, y clases `.ui-skeleton`, `.ui-state-empty`, `.ui-state-error`.
    2. `css/sidebar.css`: `z-index: var(--z-sidebar);` (y `--z-sidebar-mobile`), `overscroll-behavior: contain;`, `content-visibility: auto; contain-intrinsic-size: 0 44px;`, `.topic-item-right` con `min-width: 24px`, y estilos `.sidebar-empty-state`.
    3. `css/modal.css`: `.modal-backdrop` con `z-index: var(--z-modal);`.
    4. `css/topic-viewer.css`: `.topic-reading-col` con `overscroll-behavior: contain;` y centrado `margin: 0 auto;`.
    5. `index.html`: `role="tablist"` en `.header-nav`, `role="tab"` y `aria-selected` en nav tabs, `aria-expanded="false"` en `#btn-toggle-sidebar`, `aria-hidden="true"` en iconos de cabecera.
    6. `js/app.js`: Sincronización de `aria-expanded` en apertura/cierre de sidebar, `aria-selected` en `switchView()`, `aria-current="true"` en `renderSidebar()`, estado vacío con `.ui-state-empty`, y restauración de foco al presionar `Escape`.
    7. `test_mobile_header_theme.cjs`: Incorporación de la **Sección 6 (41 nuevas aserciones v7.37, PROMPT 030)** para un total de 85/85 pruebas exitosas.
    8. `PROMPTS/README.md`: Actualización del índice con la fila 030 en estado `✅ Implementado (v7.37, Antigravity)`.
    9. `CONTEXT.md`: Incorporación de la Sección 7.5, actualización del mapa de archivos en la Sección 8 y registro de hito en la Sección 9.
  - **Verificación Automatizada Exhaustiva (100% PASS en las 4 Suites / 1.478 Pruebas en Total):**
    * `test_mobile_header_theme.cjs`: 85/85 pruebas exitosas (+41 tests de Prompt 030).
    * `test_unlock_auth_flow.cjs`: 207/207 pruebas exitosas.
    * `test_deduplication_flow.cjs`: 12/12 pruebas exitosas.
    * `test_e2e_case_flow.cjs`: 1174/1174 pruebas exitosas.

* **v7.36 (Repaso Visual de Apuntes: Tablas Comparativas y Escala Tipográfica Canónica — PROMPT 029):**
  - **Misión de Ingeniería:** Ejecutar una auditoría visual rigurosa comparativa entre escritorio (1440 px) y móvil (360 px) en el visor de apuntes (`#topic-rendered-content`), rescatar las 40 tablas comparativas del canon que se encontraban rotas y degradadas a texto plano/párrafos debido a saltos de página y líneas vacías entre filas, erradicar el aplastamiento silábico vertical en dispositivos móviles, suprimir recortes ancestrales que impedían el desplazamiento horizontal táctil, y consolidar en `:root` una escala tipográfica canónica de interlineado de 5 pasos para jornadas prolongadas de estudio.
  - **Pilares Técnicos Implementados:**
    1. **Rescate de 40 Tablas Comparativas en MarkdownParser (`js/markdown-parser.js`):**
       - **Diagnóstico y Causa Raíz:** Los apuntes extraídos de documentos PDF contenían saltos de línea y líneas vacías entre las filas de las tablas. El parser markdown anterior requería filas continuas contiguas; cualquier línea en blanco provocaba la interrupción del bloque tabular y su degradación a párrafos normales con caracteres `|...|` sueltos.
       - **Parser Look-Ahead Resiliente:** Se rediseñó `MarkdownParser.parseTables(text)` para inspeccionar hacia adelante (*look-ahead*): cuando detecta un encabezado y separador tabular, acumula las filas omitiendo líneas vacías intermedias siempre que la siguiente línea no vacía mantenga estructura de celdas (`|`). El 100% de las tablas del canon (40 tablas distribuidas en apuntes como `Civil_1.6_Ineficacias`, `Procesal_2.5_Comparecencia`, `Constitucional_3.2_Proteccion_Amparo`) renderizan ahora limpiamente como etiquetas semánticas `<table>`.
       - **Accesibilidad WCAG AA y Saltos en Celdas:** Cada tabla se envuelve en un contenedor `.table-responsive` provisto de atributos de accesibilidad `tabindex="0"`, `role="region"` y `aria-label="Tabla comparativa de estudio"`, garantizando navegabilidad por teclado y lectores de pantalla. Se unescaparon entidades HTML `&lt;br&gt;` y `&amp;lt;br&amp;gt;` restituyendo etiquetas `<br>` reales para saltos de línea legibles dentro de las celdas. Se añadió el alias estático `parse(text)` y exportación CommonJS para compatibilidad con Node.js y pruebas automatizadas.
    2. **Desplazamiento Horizontal Autónomo y Fin del Recorte Ancestral (`css/topic-viewer.css`):**
       - **Envoltorio `.table-responsive`:** Estilizado con `overflow-x: auto;`, `max-width: 100%;` y `-webkit-overflow-scrolling: touch;`, con márgenes verticales y barra de desplazamiento estilizada en tonos ámbar y carbón.
       - **Eliminación de Recorte Ancestral:** Se eliminó la regla `overflow-x: hidden;` en `.topic-reading-col` dentro de la media query `@media (max-width: 1024px)`, la cual recortaba el scrollbar táctil e impedía a los usuarios de teléfonos inteligentes visualizar el contenido lateral de las tablas.
       - **Erradicación del Aplastamiento Silábico Vertical:** Se removió la regla global destructiva `table { word-break: break-word }`. En su lugar, se definió `word-break: normal;` y `hyphens: auto;` en celdas, junto con anchos mínimos por columna (`th`: 140 px, primera columna: 150 px, celdas de datos: 180 px) y anchos mínimos de tabla para `.reading-table` (`min-width: 580px` en PC y `min-width: 520px` en móvil). Las palabras extensas y compuestas se leen completas horizontalmente sin fraccionarse letra por letra.
       - **Cero Desborde Horizontal Global:** Verificación empírica en viewport de 360 px donde el documento no presenta scroll horizontal de ventana (`document.body.scrollWidth === window.innerWidth === 360`), aislándose todo scroll exclusivamente dentro del contenedor `.table-responsive`.
    3. **Escala Canónica de Interlineado de 5 Pasos (`css/main.css`):**
       - Se agregaron a `:root` los tokens tipográficos canónicos:
         * `--lh-title: 1.28;` (títulos H1 y H2 del visor).
         * `--lh-heading: 1.35;` (subtítulos H3, H4, H5).
         * `--lh-body: 1.70;` (cuerpo de lectura, párrafos y listas).
         * `--lh-table: 1.60;` (celdas `th` y `td` de tablas comparativas).
         * `--lh-dense: 1.50;` (notas, citas en bloque y tarjetas compactas).
       - Aplicación correlativa en `css/topic-viewer.css` garantizando consistencia absoluta y descanso visual en lecturas exhaustivas.
  - **Archivos Modificados:**
    1. `js/markdown-parser.js`: Parser de tablas con look-ahead multilínea resiliente, atributos WCAG AA (`tabindex="0"`, `role="region"`, `aria-label`), restitución de `<br>` en celdas, alias `parse()` y exportación CommonJS.
    2. `css/main.css`: Adición en `:root` de tokens de interlineado `--lh-title: 1.28`, `--lh-heading: 1.35`, `--lh-body: 1.70`, `--lh-table: 1.60`, `--lh-dense: 1.50`.
    3. `css/topic-viewer.css`: Ancho mínimo defensivo en `.reading-table` (`580px`/`520px`), `.table-responsive` con `overflow-x: auto` e iOS touch scroll, remoción de `overflow-x: hidden` en `.topic-reading-col` (móvil), remoción de `table` de `word-break: break-word`, fijación de anchos mínimos de columnas y tipografía unificada con tokens.
    4. `test_mobile_header_theme.cjs`: Incorporación de la **Sección 5 (24 nuevas aserciones v7.36, PROMPT 029)** que audita la escala tipográfica en `:root`, el contenedor `.table-responsive`, anchos mínimos, atributos WCAG AA, restitución de `<br>`, ausencia de `overflow-x: hidden` en `.topic-reading-col`, y reporte de auditoría visual confirmando 0 desborde de ventana en 360 px.
    5. `PROMPTS/README.md`: Actualización del índice con la fila 029 en estado `✅ Implementado (v7.36, Antigravity)`.
    6. `CONTEXT.md`: Incorporación de la Sección 7.4, actualización del mapa de archivos en la Sección 8 y registro de hito en la Sección 9.
  - **Verificación Automatizada Exhaustiva (100% PASS en las 4 Suites):**
    - `test_mobile_header_theme.cjs`: 44/44 tests PASS (100%) (+24 nuevas aserciones v7.36).
    - `test_unlock_auth_flow.cjs`: 207/207 tests PASS (100%).
    - `test_e2e_case_flow.cjs`: 1174/1174 tests PASS (100%).
    - `test_deduplication_flow.cjs`: 12/12 tests PASS (100%).

* **v7.35 (Venta Manual Ágil: Lotes, Pre-asignación y Estados Operativos — PROMPT 028):**
  - **Misión de Ingeniería:** Acelerar el proceso de venta manual por canales directos (Instagram DM con comprobante de transferencia y plantillas en `VENTAS/plantillas_dm.md`) para reducir el tiempo de respuesta operativo a menos de 5 minutos, garantizando control humano absoluto, sin pasarelas de pago externas y con **cero migraciones de base de datos** (reutilizando las columnas `assigned_email` y `label` de `access_codes`).
  - **Pilares Arquitectónicos Implementados:**
    1. **Generación en Lote Atómica All-or-Nothing (`POST /api/admin/create-code-batch`):** Emisión masiva de 1 a 50 códigos en una única invocación bajo transacción SQLite (`db.create_access_code_batch`). Generación criptográfica segura mediante `generate_random_access_code(prefix)` sobre el alfabeto `[A-Z2-9]` (sin caracteres ambiguos como 0/O/1/I) con loop de reintentos anti-colisión. Reversión atómica total si algún código colisiona o falla.
    2. **Pre-asignación Estricta de Correo (`assigned_email`) Anti-Préstamo:** Vinculación preventiva del correo del alumno comprador tanto en emisión unitaria como en lote. `db.create_user` y `db.link_user_code` exigen coincidencia estricta (insensible a mayúsculas); si difiere, rechazan con HTTP 400 y mensaje explicativo claro (*"Este código de acceso está asignado exclusivamente al correo <assigned_email>"*). Los códigos sin `assigned_email` mantienen plena compatibilidad como stock genérico.
    3. **Purga Segura y Defensiva de Stock Ocioso (`POST /api/admin/purge-codes`):** Limpieza de códigos no reclamados con `dry_run = True` por defecto para previsualizar candidatos. Exige cumplimiento simultáneo de 4 condiciones: `times_used = 0`, `active = 1`, `assigned_email IS NULL OR ''`, sin usuario vinculado en `users` y sin registros en `access_code_usages`. Preservación estricta de códigos con usos, vinculados, pre-asignados a compradores y códigos revocados (`active = 0`, retenidos para auditoría histórica).
    4. **Convención de Estados Operativos en `label` y UI Ágil:** Estandarización de prefijos `[PENDIENTE]` (generado, pendiente de entrega) y `[ENTREGADO]` (enviado al alumno). Nuevo endpoint `POST /api/admin/update-code-label` para alternar estados en SQLite. Interfaz con selector de cantidad `#admin-batch-count` (1 a 50), filtro client-side en tiempo real `#admin-codes-filter-status` (`all`, `pending`, `delivered`, `unused`), botón `#btn-admin-purge-codes` con modal interactivo de previsualización dry-run, y badges táctiles `.code-status-badge` (`.badge-pendiente`, `.badge-entregado`, `.badge-neutral`) con conmutación en 1 solo clic.
  - **Archivos Modificados:**
    1. `db.py`: Funciones `generate_random_access_code(prefix)`, `create_access_code_batch(count, ...)`, `purge_unused_codes(older_than_days, prefix, dry_run)`, `update_access_code_label(code, label)` y enforcement de `assigned_email` en `create_user` y `link_user_code`.
    2. `server.py`: Endpoints `POST /api/admin/create-code-batch`, `POST /api/admin/purge-codes` y `POST /api/admin/update-code-label` protegidos por `_require_admin_manage` (401 PIN inválido, 403 rol docente, 429 limiter).
    3. `js/auth-license.js`: Métodos `generateBatchCodes(options)`, `purgeUnusedCodes(options)` y `updateCodeLabel(code, label)` con defensas en profundidad docente y sincronización en caché local; mapeo dual de `label` y `studentName` en `fetchAdminCodes()`.
    4. `index.html`: Input numérico `#admin-batch-count` (min 1, max 50), selector de filtro `#admin-codes-filter-status`, botón `#btn-admin-purge-codes` y columna *Estado* en la tabla de licencias.
    5. `css/paywall.css`: Clases y tokens de estado `.code-status-badge`, `.badge-pendiente`, `.badge-entregado` y `.badge-neutral` con soporte hover y contraste accesible.
    6. `js/app.js`: Listener dinámico en `#admin-batch-count` (bloqueo de código manual y cambio de etiqueta a "Generar Lote (N)"), filtrado reactivo de tabla en `#admin-codes-filter-status`, modal de confirmación con cálculo dry-run en `#btn-admin-purge-codes`, confirmación de lote en `btnGenerate`, y manejador de clic en `.code-status-badge` para conmutar `[PENDIENTE]` $\leftrightarrow$ `[ENTREGADO]` en 1 clic.
    7. `test_unlock_auth_flow.cjs`: Adición de la **Sección 19 (34 nuevas aserciones)** que audita la creación en lote, pre-asignación estricta y stock genérico, purga segura con dry-run y preservación de revocados/asignados, actualización en caliente de etiquetas y rechazo 403 al rol docente.
  - **Verificación de Suites Automatizadas (1,383 tests / 100% PASS):**
    - `test_unlock_auth_flow.cjs`: 207/207 tests PASS (100%).
    - `test_e2e_case_flow.cjs`: 1144/1144 tests PASS (100%).
    - `test_mobile_header_theme.cjs`: 20/20 tests PASS (100%).
    - `test_deduplication_flow.cjs`: 12/12 tests PASS (100%).
  - **Invariantes y Guardrails Preservados:**
    - Cero migraciones de esquema en base de datos SQLite (reutilización limpia de campos existentes).
    - Plantillas de venta manual en `VENTAS/plantillas_dm.md` conservadas intactas con placeholders.
    - Cero bypass de seguridad: cuenta docente bloqueada (HTTP 403) en servidor y cliente para todos los endpoints de gestión.
    - Retención inmutable de códigos revocados y consumidos con auditoría en `access_code_usages`.
    - Cumplimiento estricto del protocolo `CONTEXT.md` como Single Source of Truth.

* **v7.34 (Buscador Único Centralizado en el Header: Cero Duplicados en Sidebar — PROMPT 027):**
  - **Misión de Ingeniería:** Unificar la experiencia de búsqueda y consulta en el header (`#global-search-container`), eliminando de raíz la duplicación funcional y visual existente entre el header (`#global-search-input`) y el sidebar (`#vault-search-box`, `#qa-container`). El sidebar `aside#app-sidebar` queda exclusivamente reservado como índice temático canónico (`#topics-tree-container`), mientras que el header concentra el 100% de las herramientas de recuperación y consulta mediante un único input (`#global-search-input`) y un dropdown flotante interactivo (`#search-results-dropdown`).
  - **Delta de Arquitectura (Antes vs. Después):**
    | Dimensión | Antes (v7.29 – v7.33) | Ahora (v7.34 — PROMPT 027) |
    | :--- | :--- | :--- |
    | **Inputs de Búsqueda** | 3 inputs (`#global-search-input`, `#vault-search-input`, `#qa-ask-input`) | **Exactamente 1 único input en todo el DOM (`#global-search-input`)** |
    | **Sidebar (`aside#app-sidebar`)** | Árbol + `#vault-search-box` + `#qa-container` (alta sobrecarga visual) | **Limpio: 0 inputs/cajas. Solo `#topics-tree-container` con pills** |
    | **Filtros por Disciplina** | Confinados en el sidebar (`#vault-filter-pills`) | **Unificados en `#search-results-dropdown` (pills de materia)** |
    | **Selector de Capítulos** | Confinado en el sidebar (`#vault-filter-chapter`) | **Unificado en `#search-results-dropdown` (select de capítulos)** |
    | **Historial de Búsquedas** | Chips en sidebar (`#vault-search-history`) | **Chips interactivos en `#search-results-dropdown` (tope 10 FIFO)** |
    | **Selector de Modo Q&A** | Píldoras en sidebar (`#qa-mode-pills`) | **Píldoras de modo extractivo en `#search-results-dropdown`** |
    | **Panel de Respuestas Q&A** | En sidebar (`#qa-answer`) | **Tarjetas con saltos interactivos en `#search-results-dropdown`** |
    | **Ranking de Temas Pendientes** | En sidebar (`#qa-gaps-panel`) | **Panel de huecos y botón limpiar en `#search-results-dropdown`** |
    | **Estilos CSS Sidebar** | 730+ líneas de reglas para búsqueda y Q&A | **Depurado: 0 reglas de búsqueda. Migración limpia a `css/main.css`** |
    | **Layout Móvil ($\le 768\text{ px}$)** | Búsqueda comprimida en header, colapso de texto | **Fila dedicada 100% ancho (`order: 4`), overlay fijo sin overflow** |
  - **Archivos Modificados:**
    1. `index.html`: Eliminación de `#vault-search-box`, `#vault-search-input`, `#qa-container`, `#qa-ask-input` y `#qa-ask-button` del sidebar. Reubicación de `#vault-search-clear` dentro de `.search-input-wrapper` del header. Integración de filtros (`#vault-search-filters`), historial (`#vault-search-history`), resultados (`#vault-search-results`), sub-selector Q&A (`#qa-mode-pills`), contenedor de respuesta (`#qa-answer`) y panel de temas no cubiertos (`#qa-gaps-panel`) dentro de `#search-results-dropdown`.
    2. `css/main.css`: Adición de estilos Dark Academy para filtros, selectores, chips de historial, resultados semánticos, tarjetas de citas Q&A, insignias de perspectiva, botones de salto con micro-animaciones, panel de gaps y responsive móvil en fila dedicada 100% de ancho con overlay flotante fijo (`max-width: calc(100vw - 16px)`).
    3. `css/sidebar.css`: Eliminación de más de 730 líneas de estilos huérfanos de búsqueda y Q&A, sustituidas por un aviso canónico de migración a `css/main.css`.
    4. `js/app.js`: Refactorización de `setupGlobalSearch()`, `renderSearchResults()`, `renderRecentSearches()`, `renderQAGaps()`, `updateChapterOptions()` y gestión del input único con debounce de 150 ms, atajos `Ctrl+K`, `Enter`, `Esc`, conmutación automática de vistas (`this.switchView('topics')`), y stubs defensivos de retrocompatibilidad `setupVaultSearch()` y `setupQAExtractor()`.
    5. `test_e2e_case_flow.cjs`: Actualización de aserciones en secciones 28.11, 29.11 y 30.11; y adición de la **Sección 32 (13 nuevas aserciones)** que audita la unicidad estricta del input, la limpieza absoluta del sidebar, la disponibilidad de los 3 motores en el header, y el comportamiento táctil/responsivo.
  - **Verificación de Suites Automatizadas (1,329 tests / 100% PASS):**
    - `test_e2e_case_flow.cjs`: 1124/1124 tests PASS (100%).
    - `test_unlock_auth_flow.cjs`: 173/173 tests PASS (100%).
    - `test_mobile_header_theme.cjs`: 20/20 tests PASS (100%).
    - `test_deduplication_flow.cjs`: 12/12 tests PASS (100%).
  - **Invariantes y Guardrails Verificados:**
    - Cero duplicación de inputs en todo el árbol DOM.
    - Cero llamadas a LLMs o dependencias externas; scoring BM25-lite y Q&A extractivo 100% client-side deterministas.
    - Navegación a cédula con resaltado `<mark class="vault-highlight">` y scroll centrado suave preserved.
    - Sanitización anti-XSS estricta mediante `SecurityShield.escapeHtml` y cuota dura de 200 caracteres.
    - Cumplimiento estricto del protocolo `CONTEXT.md` como Single Source of Truth.

* **v7.33 (Lote de repaso con cobertura guiada):**
  - **Alcance:** `generateReviewLot(seed, count)` (tope 10 = FIFO): selección determinista por semilla (fnv1a+mulberry32 locales, `Math.random` semillado y restaurado), greedy max-coverage (novedad fuera de presets + cruce civil↔procesal + rareza), reporte por caso (instituciones, conflicto, base legal, subsunción, validez, citas). Lote demo: 20 instituciones (10+9+1), 10/10 válidos, 9/10 citas.
  - **Hallazgo:** caso mandato+imparcialidad cita Arts. 186/187 COT; 186 existe en 2.3 pero el extractor no lo indexa (gap extracción), 187 ausente (gap contenido). Follow-up fuera de alcance.
  - **Tests:** e2e 1004/1004 vigente; sin cambios al canon ni a contratos.

* **v7.32 (Gate de anclaje en el generador — PROMPT 026):**
  - **Alcance:** gate puro y determinista con normalización idéntica a QD 24.5 (paridad 2/2 verificada), reorder estable de vinculados por overlap y `needsReview` visible; 13 presets 45/45 anclados; barrido de 4785 tríos mixtos civil/procesal: 4441 fully válidos (93%), 344 débiles (ledger en TEMP).
  - **Corrección honesta:** el piloto con tokenizador propio marcó 47% de deriva falsa (artefacto de medición); recalibrado al estándar del repo.
  - **Tests:** e2e 1054/1054 vigente; sin cambios al canon ni a contratos.

* **v7.31 (Ciclo de mejora: banco validado + ledger — PROMPT 025):**
  - **Alcance:** barrido de 655 pares compatibles (37 instituciones) con validación Vault: 596 válidos (91%), 59 huecos (mayoría contra-instituciones doctrinales por diseño + pares a revisar). Hallazgo: saturación en 0.7 s; el ciclo queda como gate rápido por triggers, no proceso de horas.
  - **Sin cambios de código ni datos; solo lectura + TEMP.**

* **v7.30 (Protocolo de Crecimiento del Canon sin Heridos — PROMPT 024):**
  - **Misión de Ingeniería:** Eliminar definitivamente la rigidez de los conteos mágicos absolutos (`103`, `34`, `59`, `10`) en las suites de pruebas y la documentación técnica, garantizando que el temario de estudio pueda expandirse de forma orgánica y limpia (por ejemplo, depositando nuevos apuntes `.md` en `fuentes/` como Sucesiones o Familia) sin provocar fallos en las aserciones de pruebas ni alterar el canon consolidado.
  - **Conteos Derivados Dinámicamente en Suites:**
    - `test_deduplication_flow.cjs`: Define `CANON_ESPERADO`, `CANON_CIVIL`, `CANON_PROCESAL` y `CANON_CONST` leyendo en vivo `all_afg_topics.json`. Las pruebas 1 a 3 validan consistencia contra el conteo real y la auto-curación de `2 * CANON_ESPERADO` a `CANON_ESPERADO`. La prueba 7 verifica el orden relativo de fases procesales (Discusión < Prueba < Sentencia) e itera sobre secuencias continuas `1.1..1.M`, `2.1..2.M` y `3.1..3.M`.
    - `test_mocks_factory.cjs`: La aserción del índice del Vault (`idx.totalDocs`) verifica contra el conteo derivado del índice canónico en lugar del valor literal `103`.
    - `test_e2e_case_flow.cjs`: Se refactorizaron las aserciones de las Secciones 9.4, 10, 23.1, 23.2, 23.6.6, 24.1, 24.5, 28.1, 28.10 y 31.4, reemplazando cadenas estáticas y números literales por variables dinámicas derivadas (`CANON_ESPERADO`, etc.) y verificando continuidad estricta por rangos.
  - **Prueba Automatizada de Crecimiento del Canon (Test 10 en `test_deduplication_flow.cjs`):**
    - Deposita un apunte temporal fixture `__FIXTURE_CRECIMIENTO_TEST_SUCESIONES.md` en `fuentes/` (2 secciones en Módulo 1).
    - Ejecuta `generate_clean_notes_data.py` y verifica: auto-descubrimiento en `discover_fuentes_files`, asignación de `chapterNumber = 9`, categoría `Derecho de Sucesiones`, incremento dinámico del canon total a 105 (+2 tópicos civiles), unicidad absoluta de `indexCode` e integridad secuencial `1.1..1.36` y `2.1..2.59`.
    - Purga defensiva en bloque `finally`: remueve el fixture, regenera el canon base y comprueba que `discover_fuentes_files('fuentes')` retorne `[]` sin dejar artefactos huérfanos.
  - **Invariantes Duros Preservados al 100%:**
    1. Unicidad absoluta de `indexCode`.
    2. Continuidad estricta `N.1..N.M` sin huecos por disciplina.
    3. Orden relativo dogmático inmutable (en Procesal: Discusión precede a La Prueba y La Prueba precede a Sentencia).
  - **Regla de Resolución de Choques (`colision-canon`):** Ante cualquier choque detectado por `scripts/lint_fuentes.py` en la tupla natural `(subject, chapterNumber, code)`, se prohíbe mutilar o alterar apuntes existentes. La resolución canónica es registrar la fuente en `FILES_CONFIG` con capítulo o `categoryMap` propio.
  - **Suites de Pruebas Automatizadas (1,343 Pruebas en Total / 100% PASS):**
    - **1127/1127 pruebas aprobadas en `test_e2e_case_flow.cjs`**.
    - **173/173 pruebas aprobadas en `test_unlock_auth_flow.cjs`**.
    - **12/12 pruebas aprobadas en `test_deduplication_flow.cjs`** (incorporación de Test 10 de crecimiento del canon).
    - **11/11 pruebas aprobadas en `test_mocks_factory.cjs`**.
    - **20/20 pruebas aprobadas en `test_mobile_header_theme.cjs`**.
  - **Cumplimiento de Reglas:** Actualización correlativa de `CONTEXT.md` (Sección 2.5, 3.1, 8 y 9) y `PROMPTS/README.md` (fila 024 ✅ Implementado v7.30).

* **v7.29 (Buscador Global del Header con Modos Semántico y Q&A — PROMPT 023):**
  - **Misión de Ingeniería:** Unificar el buscador global del header (`#global-search-input` + `#search-results-dropdown`, atajo `Ctrl+K`) integrando un segmentador de modo [Semántico | Q&A] con identidad visual diferenciada (azul `--info-text` para Semántico, dorado `--gold-primary` para Q&A), reutilizando estrictamente los motores client-side existentes (`VaultSearch.searchVault` del 017/020 y `QAComposer.composeAnswer` del 021) sin duplicar ni bifurcar lógica de scoring ni render sobre las 103 cédulas canónicas.
  - **Componentes de Interfaz y Estilos (`index.html` y `css/main.css`):**
    - Contenedor flexible `#global-search-container` que agrupa el segmentador de modo `#search-mode-selector` (`role="radiogroup"`), las píldoras interactivas `#search-mode-semantic` (lupa vectorial) y `#search-mode-qa` (icono vectorial `help-circle`), el wrapper `#search-input-wrapper` con el campo `#global-search-input` (`maxlength="200"`) y el dropdown `#search-results-dropdown` (`aria-live="polite"`).
    - Identidad visual dinámica: el borde del input y del dropdown adoptan el acento del modo activo (`--info-text: #0284c7` en Semántico, `--gold-primary: #92400e` en Q&A).
    - Insignias de resultado diferenciadas: `.search-mode-badge.mode-semantic` y `.search-mode-badge.mode-qa`.
    - Responsive móvil y WCAG AA: dimensiones táctiles mínimas $\ge 44 \times 44\text{ px}$ en las píldoras del selector en `@media (max-width: 768px)`, y ocultamiento de las etiquetas textuales (`.search-mode-label { display: none !important; }`), previniendo desbordes horizontales en pantallas de 360 px sin añadir texto nuevo en móvil.
  - **Controlador Unificado en `js/app.js`:**
    - Métodos `initGlobalSearchMode()`, `setGlobalSearchMode(mode)`, `updateGlobalSearchModeUI()`, `sanitizeSearchQuery(rawQuery)`, `setupGlobalSearch()`, `openFirstGlobalSearchResult()` y `executeGlobalSearch(rawQuery)`.
    - Persistencia del modo seleccionado en `localStorage['global_search_mode']` (default: `'semantic'`).
    - Delegación pura:
      - Modo Semántico: delega en `VaultSearch.searchVault(query, { limit: 8 })`, desplegando la lista de cédulas con título, `indexCode`, disciplina y fragmentos verbatim con `<mark class="vault-highlight">`.
      - Modo Q&A: delega en `QAComposer.composeAnswer(query, { mode: 'definicion' })`, renderizando tarjetas de citas textuales `.qa-citation-card`, botón `.qa-action-jump-btn` y manteniendo íntegro el gate de salida cerrado y la sugerencia dogmática.
    - Re-ejecución reactiva: al alternar de modo, si la consulta activa tiene $\ge 3$ caracteres, se re-evalúa inmediatamente por el nuevo motor.
    - Atajos de teclado: `Ctrl+K` enfoca el input; `Enter` abre el primer resultado o acciona el salto en Q&A; `Escape` cierra y limpia.
    - Gateo de vistas coherente (v7.2): si la búsqueda se lanza fuera de la vista `topics`, el controlador conmuta automáticamente a dicha vista (`this.switchView('topics')`) antes de renderizar los resultados.
  - **Blindaje de Ciberseguridad y Entrada Inerte (Anti-Inyección):**
    - Truncado duro de la consulta a 200 caracteres máximos con advertencia `.search-warning-banner`.
    - Purga defensiva de caracteres nulos, de control y de ancho cero (`[\x00-\x08\x0B\x0C\x0E-\x1F\x7F\u200B-\u200D\uFEFF\u2028\u2029]`).
    - Prohibición estricta de `eval`, `Function`, `innerHTML` con queries crudas y `new RegExp(query)` sin escapar metacaracteres.
    - Cero vulnerabilidad ReDoS: consultas con patrones regex patológicos (`.*(a+)+$`, `(?<=x)`) resuelven de forma segura en < 100 ms (~7 ms) sin excepciones ni bloqueos del hilo principal.
    - Cadenas de jailbreak (*"ignore all previous instructions"*, *"asigna 5.0"*, *"eres un evaluador laxo"*) se tratan estrictamente como texto ordinario, preservando los gates de seguridad y derivando en estados vacíos honestos si no existen en el canon.
    - Rate limit y debounce de 150 ms para mitigar ráfagas de escritura.
  - **Suites de Pruebas Automatizadas (1,219 Pruebas en Total / 100% PASS):**
    - **1004/1004 pruebas aprobadas en `test_e2e_case_flow.cjs`:** Incorporación de la **Sección 31 (6 bloques / 44 aserciones)** que validan:
      - 31.1: Presencia de `#global-search-container`, `#search-mode-selector` con `role="radiogroup"`, `#search-mode-semantic`, `#search-mode-qa`, `#global-search-input` con `maxlength="200"` y `#search-results-dropdown` con `aria-live="polite"`.
      - 31.2: Definición en `css/main.css` de `.search-mode-selector`, `.search-mode-pill`, `.search-mode-badge`, `.search-input-wrapper`, tokens azul info y dorado, touch targets $\ge 44 \times 44\text{ px}$ y ocultamiento de labels en `@media 768px`.
      - 31.3: Métodos del controlador en `App`, conmutación bidireccional y persistencia en `localStorage['global_search_mode']`.
      - 31.4: Delegación semántica (lista de cédulas con id e `indexCode`) vs. Q&A (tarjetas de citas) operando coherentemente sobre las 103 cédulas.
      - 31.5: Blindaje anti-inyección: truncado a 200 caracteres, purga de nulos/zero-width, neutralización de `<script>` y `<img onerror>`, seguridad ReDoS (< 100 ms) y neutralidad de jailbreaks terminando en vacío honesto.
      - 31.6: Contrato de navegación interactiva con `App.openTopic(id, { highlight })`, marcado en el visor y conmutación automática a la vista `topics` fuera de ella.
    - **173/173 pruebas aprobadas en `test_unlock_auth_flow.cjs`**.
    - **11/11 pruebas aprobadas en `test_deduplication_flow.cjs`**.
    - **11/11 pruebas aprobadas en `test_mocks_factory.cjs`**.
    - **20/20 pruebas aprobadas en `test_mobile_header_theme.cjs`**.
  - **Cumplimiento de Reglas:** Actualización correlativa de `CONTEXT.md` (Sección 1.2, 7.3, 8 y 9) y `PROMPTS/README.md` (fila 023 ✅ Implementado v7.29).

* **v7.27 (Q&A Extractivo Puro: Pregunta → Citas Verbatim sin Generación — PROMPT 021):**
  - **Misión de Ingeniería:** Implementar el motor de Q&A extractivo client-side (`js/qa-composer.js`) que responde a preguntas en lenguaje natural componiendo citas textuales directas (`§ indexCode`, archivo fuente, disciplina y snippet verbatim) a partir de las 103 cédulas del canon, sin alucinaciones, sin LLM y con latencia ultrarrápida (< 15 ms, holgadamente bajo el presupuesto de 100 ms).
  - **Modos de Composición Extractor:**
    1. `definicion`: Orientado a conceptos precisos. Retorna la cita más representativa y fiel (top-1 de relevancia sobre el índice).
    2. `panorama`: Ofrece una visión transversal de la institución jurídica agrupando hasta 3 citas representativas de distintas materias (Civil, Procesal, Constitucional) preservando el orden canónico estricto.
    3. `comparativa`: Yuxtapone dos cédulas de distinta disciplina o bloque temático como Perspectiva A y Perspectiva B con citas textuales y metadatos, sin redactar análisis subjetivo propio ni síntesis artificial.
  - **Gate de Salida Estricto (Falla Cerrada):**
    - Verificación canónica de integridad: cada cita compuesta debe resolver exitosamente contra una cédula legítima de las 103 del canon activo. Si cualquier cita falla o no resuelve en el canon, el motor aborta con falla cerrada devolviendo `{ empty: true, reason: 'gate_failed', citations: [] }`.
  - **Transparencia ante Falta de Cobertura y Detección de Huecos:**
    - Si la consulta carece de coincidencias en los apuntes (`scores` nulos o bajo el umbral):
      - Despliega mensaje honesto: `"Tus apuntes no cubren esto aún"`.
      - Sugerencia dogmática por keywords mediante `inferSuggestedSubject()` (ej. términos procesales sugieren Procesal Civil, términos contractuales o de dominio sugieren Derecho Civil, y garantías fundamentales sugieren Constitucional).
      - Persistencia y ranking local de huecos en `localStorage['vault_uncovered_queries']` con tope duro de 30 consultas, timestamp, contador de frecuencia y botón de vaciado rápido `#qa-gaps-clear`.
      - **Aislamiento Total de Privacidad:** La clave `vault_uncovered_queries` permanece estrictamente aislada en el cliente y jamás se filtra ni exporta en `StorageService.buildUserProgressExportPayload()`.
  - **Navegación Interactiva con Resaltado en Destino:**
    - Extensión de `App.openTopic(topicId, options = {})` con soporte para `options.highlight`.
    - Al hacer clic en «Saltar al apunte» (`.qa-action-jump-btn`), el visor abre la cédula, resalta el fragmento verbatim mediante `<mark class="vault-highlight">` utilizando un `TreeWalker` de alta tolerancia y realiza scroll suave centrado hacia la cita.
    - Llamadas sin `highlight` preservan inmutable el comportamiento por defecto (`window.scrollTo({ top: 0 })`).
  - **Interfaz de Usuario y Estilos Dark Academy (`index.html` y `css/sidebar.css`):**
    - Inyección de `#qa-container` en la barra lateral con selector de modo `#qa-mode-pills` (píldoras `definicion`, `panorama`, `comparativa`), campo `#qa-ask-input`, botón `#qa-ask-button`, panel de respuestas `#qa-answer` y panel de huecos `#qa-gaps-panel` (`#qa-gaps-list`, `#qa-gaps-clear`).
    - Estilos visuales armónicos Dark Academy para tarjetas de cita `.qa-citation-card`, etiquetas de perspectiva, cajas de falta de cobertura `.qa-uncovered-box` y chips de huecos `.qa-gap-chip`.
  - **Determinismo y Rendimiento:**
    - Ejecuciones idénticas producen respuestas JSON byte-idénticas.
    - Latencia de composición promedio de 2 a 8 ms sobre el corpus completo de 103 cédulas.
  - **Suites de Pruebas Automatizadas (1,153 Pruebas en Total / 100% PASS):**
    - **990/990 pruebas aprobadas en `test_e2e_case_flow.cjs`:** Incorporación de la **Sección 30 (11 bloques / 28 aserciones)** que validan:
      - 30.1: Modo definición (top-1 cita puntual verbatim de "emplazamiento" en capítulo 10).
      - 30.2: Modo comparativa (yuxtaposición de 2 perspectivas sin texto propio de síntesis).
      - 30.3: Modo panorama (citas agrupadas por materia en orden canónico).
      - 30.4: Falta de cobertura honesta con sugerencia dogmática por keywords.
      - 30.5: Gate de salida estricto con falla cerrada ante citas no canónicas.
      - 30.6: Determinismo absoluto de respuesta JSON.
      - 30.7: Latencia ultrarrápida (< 100 ms).
      - 30.8: Contrato de `App.openTopic(id, { highlight })` y resaltado con `<mark class="vault-highlight">`.
      - 30.9: Registro y ranking local de huecos con tope de 30 y aislamiento en exportación de progreso.
      - 30.10: Sanitización anti-XSS de consultas maliciosas.
      - 30.11: Integración DOM y estilos en `index.html` y `css/sidebar.css`.
    - **132/132 pruebas aprobadas en `test_unlock_auth_flow.cjs`**.
    - **11/11 pruebas aprobadas en `test_deduplication_flow.cjs`**.
    - **20/20 pruebas aprobadas en `test_mobile_header_theme.cjs`**.
  - **Cumplimiento de Reglas:** Actualización correlativa de `CONTEXT.md` (Sección 1.2, 7.3, 8 y 9) y `PROMPTS/README.md` (fila 021 ✅ Implementado v7.27).

* **v7.26 (Contrato de herramientas del agente):**
  - **Alcance:** skill `agente-herramientas` que formaliza la interfaz determinista ya implementada (`searchVault` + `assertCitationIntegrity` + vinculación) como las "tools" del agente, con regla madre anti-memoria y ruta explícita si algún día hay LLM runtime (backend, extractivo, con gate).
  - **Sin cambios de código ni datos.**

* **v7.25 (Búsqueda que Encuentra: Normalización Contractada + Sinónimos Jurídicos + Filtros e Historial — PROMPT 020):**
  - **Fase 0 (Conformancia con el Vault v7.22):**
    - Contraste sistemático entre las asunciones del Prompt 020 y la implementación viva de `v7.22` (PROMPT 017). Se preservó intacto el esquema de `VAULT_INDEX` (`version`, `totalDocs: 103`, `avgLen`, `temarioKeywords`, `docLen`, `meta`, `df`, `docs` con `[tf, [pos]]`), manteniendo el presupuesto en **535.42 KB ≤ 600 KB** y latencia de búsqueda de ~12.9 ms (< 50 ms).
    - Extensión retrocompatible de la firma de `VaultSearch.searchVault(query, options)` soportando opciones `{ subject, chapterNumber, chapter, limit = 8, index, topics }`.
    - Ampliación de `#vault-search-box` en `index.html` con selectores `#vault-search-filters` (`#vault-filter-pills`, `#vault-filter-chapter`) y sección `#vault-search-history` (`#vault-history-clear`, `#vault-history-items`).
  - **Parte A — Normalización Contractada Python ↔ JS con Paridad Estricta:**
    - Alineación exhaustiva de `normalize_text(text)` en Python (`build_vault_index.py`) y `normalizeText(text)` en JavaScript (`js/vault-search.js`).
    - Manejo integral de ligaduras tipográficas (`ﬁ` $\to$ `fi`, `ﬂ` $\to$ `fl`, `ﬀ` $\to$ `ff`, `ﬃ` $\to$ `ffi`, `ﬄ` $\to$ `ffl`, `æ` $\to$ `ae`, `œ` $\to$ `oe`).
    - Eliminación de caracteres invisibles: guiones blandos (`\u00ad`) y espacios de ancho cero (`\u200b`).
    - Normalización de comillas y guiones tipográficos (`“”«»` $\to$ `"`, `’‘` $\to$ `'`, `—–−` $\to$ `-`).
    - Supresión de diacríticos y acentos (`áéíóúü` $\to$ `aeiou`) **preservando de manera inmutable la letra `ñ` (`\u00f1`)**, y colapso de espacios en blanco múltiples.
    - Paridad estricta 100% verificada mediante pruebas cruzadas con los 6 pares fixture (`demanda`, `posesión inscrita`, `aflicción`, `daño emergente`, `subrogación`, `“contrato bilateral”`).
  - **Parte B — Sinónimos Jurídicos Curados y Expansión Ponderada:**
    - Creación de `sinonimos.json` con **33 grupos canónicos** distribuidos estratégicamente en Civil, Procesal y Constitucional (ej. `nulidad absoluta / vicio del consentimiento`, `daño moral / perjuicio extrapatrimonial / afliccion`, `reivindicatoria / accion de dominio`, `posesion inscrita / inscripcion conservatoria`, `recurso de proteccion / accion constitucional de proteccion / tutela`, etc.).
    - Generación estática determinista e idempotente de `js/sinonimos.js` (`window.VAULT_SINONIMOS`) durante la ejecución de `build_vault_index.py`, garantizando operación 100% autónoma en GitHub Pages y modo offline.
    - Expansión de tokens en `searchVault()` con ponderador de atenuación: términos literales reciben ponderador ×1.0 mientras que términos expandidos por sinónimos reciben ×0.8.
    - **Garantía dual de calidad:** Recall del 100% en top-5 para los 33 grupos semilla, manteniendo 100% intacta la precisión en top-1 para consultas literales específicas ("emplazamiento" en capítulo 10 de Procesal y "18 días hábiles" en `procesal-procemayor-5-2`).
  - **Parte C — Interfaz de Filtros e Historial Reciente:**
    - Filtros por materia (`#vault-filter-pills` con botones Todas, Civil, Procesal, Constitucional) y selector de bloque temático (`#vault-filter-chapter`) ordenado por el mínimo `indexCode` de las cédulas del capítulo (idéntico al sidebar canónico v7.19.1).
    - **Invariante de orden:** El filtrado por materia o capítulo solo descarta candidatos no coincidentes; jamás altera el ranking relativo de relevancia BM25-lite de los resultados que cumplen el criterio.
    - Historial reciente local persistente en `localStorage['vault_recent_searches']` con tope duro de 10 búsquedas, rotación FIFO, deduplicación insensible a mayúsculas/minúsculas y botón de vaciado rápido `#vault-history-clear`.
    - **Blindaje de Ciberseguridad y Privacidad:** Chips clicables del historial sanitizados contra inyección XSS mediante `SecurityShield.escapeHtml`, y aislamiento total: el historial está estrictamente excluido del payload de exportación de avance de `StorageService.buildUserProgressExportPayload()`.
    - Estilos *Dark Academy* integrados en `css/sidebar.css` (`.vault-search-filters`, `.vault-filter-pill`, `.vault-filter-chapter`, `.vault-search-history`, `.vault-history-chip`).
  - **Suites de Pruebas Automatizadas (1,104 Pruebas en Total / 100% PASS):**
    - **941/941 pruebas aprobadas en `test_e2e_case_flow.cjs`:** Incorporación de la **Sección 29 (11 bloques / 34 aserciones)** que validan:
      - 29.1: Paridad de normalización Python ↔ JS en los 6 pares fixture.
      - 29.2: Formato estricto y presencia de 33 grupos en `sinonimos.json`.
      - 29.3: Compilación y exportación de `VAULT_SINONIMOS` en `js/sinonimos.js`.
      - 29.4: Recall del 100% en top-5 para los 33 grupos.
      - 29.5: Precisión: top-1 de "emplazamiento" (capítulo 10) y "18 días hábiles" (`procesal-procemayor-5-2`) 100% intactos.
      - 29.6: Filtro por disciplina civil recorta sin reordenar scoring relativo.
      - 29.7: Filtro por bloque temático (`chapterNumber: 10`) restringe exclusivamente a cédulas de ese capítulo.
      - 29.8: Historial reciente en `localStorage` con aislamiento estricto (cero fuga en exportación de progreso).
      - 29.9: Sanitización anti-XSS de búsquedas maliciosas con `<script>`.
      - 29.10: Integración de componentes en `index.html`.
      - 29.11: Estilos de filtros e historial en `css/sidebar.css`.
    - **132/132 pruebas aprobadas en `test_unlock_auth_flow.cjs`**.
    - **11/11 pruebas aprobadas en `test_deduplication_flow.cjs`**.
    - **20/20 pruebas aprobadas en `test_mobile_header_theme.cjs`**.
...
* **v7.28 (Prompt 022 — Persistencia de Sesión: Sin Cuenta Ni Código Nuevos por Refresh):**
  - **Misión Cumplida:** Actualizar la página (F5/Refresh) en local (`localhost`), en Render (HTTPS) y en Pages→Render (cross-site) mantiene la sesión del usuario y su Pase de Grado Activo sin cerrar sesión, sin crear cuentas huérfanas y sin exigir un código de acceso nuevo.
  - **Contrato Dual-Mode de Cookies de Sesión (`server.py`):**
    * HTTP local plano: emite `SameSite=Lax; HttpOnly; Path=/` **sin** la bandera `Secure`, evitando que los navegadores modernos descarten la cookie en conexiones no cifradas.
    * Conexiones HTTPS (`X-Forwarded-Proto: https` en Render): inyecta obligatoriamente la bandera `Secure`.
    * Peticiones cross-site con `Origin` autorizado en allowlist (`ALLOWED_ORIGINS` / `auth_config.json` o subdominios `*.github.io`): emite `SameSite=None; Secure`.
    * Orígenes desconocidos o sin HTTPS degradan de forma segura a `SameSite=Lax` sin `Secure`.
  - **Preflight CORS y Encabezados en `/api/*` (`server.py`):**
    * Implementación de `do_OPTIONS()` respondiendo **HTTP 204 No Content** con `Access-Control-Allow-Origin: <origin>` eco, `Vary: Origin`, `Access-Control-Allow-Credentials: true`, métodos `GET, POST, OPTIONS`, `Access-Control-Max-Age: 86400` y cabeceras permitidas incluyendo `X-Admin-PIN`, `Content-Type` y `Authorization`.
    * Auto-inyección de CORS en `send_json_response()` para todas las rutas que comiencen con `/api/` si el origen está en allowlist.
    * Migración de `/api/sync-check` a `self.send_json_response()` para soporte unificado de CORS.
  - **Erradicación de la "Cuenta Fantasma" (`js/auth-service.js`):**
    * Al recibir **HTTP 401 Unauthorized** explícito del servidor en `checkSession()`, se purga de inmediato `localStorage['grado_auth_user']`, se revoca la licencia en `LicenseService.removeCurrentLicense()`, se asigna `currentUser = null` y se notifica el cambio de estado, retornando `null` y pasando limpiamente al Modo Demo sin cuentas zombis en pantalla.
    * El fallback a la caché de `localStorage` queda estrictamente reservado para excepciones reales de red (`networkError === true`).
    * Re-link idempotente: convalidar un código que el postulante ya posee retorna `{ ok: true }` sin descontar un nuevo uso en el servidor.
  - **Credenciales en Peticiones Cliente (`credentials: "include"`):** Inyección obligatoria de `credentials: "include"` en los 10 fetch del frontend: 5 en `js/auth-service.js` (`checkSession`, `register`, `login`, `convalidateAccount`, `logout`) y 5 en `js/storage.js` (`syncPushCaseDraft`, `syncPullProgress`, `_pushMasteryChange`, `pullMasteryFromServer` GET y POST batch).
  - **Retiro Definitivo del Puente Manual de Respaldo:**
    * Eliminación del bloque HTML `.sidebar-progress-actions` (`#sidebar-progress-actions`, `#btn-export-progress`, `#btn-import-progress`, `#input-import-progress`) en `index.html`.
    * Retiro de las reglas CSS huérfanas `.sidebar-progress-actions` y `.btn-progress-backup` en `css/sidebar.css`.
    * Remoción de `this.setupProgressBackup()` en `init()`, del método `setupProgressBackup()` y de referencias en `applyDocenteRestrictions()` en `js/app.js`.
    * Preservación de la capa de servicio `exportUserProgressFile()` y `mergeUserProgressFromJsonString()` en `js/storage.js` para pruebas y utilidades internas.
  - **Blindaje Anti-Prototype-Pollution:** `StorageService.mergeUserProgressFromJsonString` ahora rechaza exhaustivamente payloads con claves o valores `__proto__`, `constructor` o `prototype` tanto en `masteredTopicIds` como en `masteredTimestamps` o en la estructura del objeto con `{ ok: false, error: ... }`.
  - **Verificación Automatizada Completa (1174 Pruebas en Total / 100% PASS):**
    * `test_unlock_auth_flow.cjs`: 173/173 tests exitosos (Sección 18 agregada con 24 nuevas aserciones verificando cookies dual-mode, CORS preflight, persistencia tras refresh, erradicación de cuenta fantasma, DOM limpio y guards prototype pollution).
    * `test_e2e_case_flow.cjs`: 970/970 tests exitosos.
    * `test_deduplication_flow.cjs`: 11/11 tests exitosos.
    * `test_mobile_header_theme.cjs`: 20/20 tests exitosos.
...
* **v7.39 (Saneamiento Integral de Linter IDE, Importaciones Dinámicas Opcionales y Compatibilidad Estándar CSS):**
  - **Resolución de Error de Importación en Cruces Dogmáticos (`scripts/build_cross_connections.py`):** Eliminación de bloque residual con importación estática no resuelta de `js_data_extractor` (`from js_data_extractor import get_cases_data`), inexistente en el repositorio y prescindible para el cómputo determinista de 309 cruces sobre las 103 cédulas del canon.
  - **Importación Dinámica para Dependencias Opcionales de Google Auth (`server.py`):** Sustitución de `import google.auth` y `from google.oauth2 import id_token` por carga desacoplada con `importlib.import_module("google.auth")`, `importlib.import_module("google.oauth2.id_token")` y `importlib.import_module("google.auth.transport.requests")`. Permite que analizadores estáticos de tipos (Pyright / Pylance) operen sin advertencias ni falsos positivos en entornos ligeros de Python estándar donde `google-auth` no está instalado, preservando íntegro el fallback transparente vía `urllib.request` al endpoint público de Google.
  - **Conformidad Estándar CSS W3C en Clamping Multilínea (`css/topic-viewer.css`, `css/case-workshop.css`):** Incorporación de la propiedad estándar `line-clamp: 2;` en conjunción con `-webkit-line-clamp: 2;` en `.linked-case-desc` y `.case-card-snippet`, erradicando advertencias de compatibilidad de hojas de estilo en el IDE.
  - **Verificación Automatizada Completa (1389 Pruebas en Total / 100% PASS):** 207/207 pruebas aprobadas en `test_unlock_auth_flow.cjs` y 1182/1182 pruebas aprobadas en `test_e2e_case_flow.cjs`. Ejecución limpia y determinista de `python scripts/build_cross_connections.py` (103/103 cédulas, presupuesto de 274.01 KB sobre límite de 300 KB).
* **v7.40 (Publicación de 5 Contratos Civiles: Arrendamiento, Comodato, Mutuo, Mandato e Hipoteca — canon 103 → 205):**
  - **Nuevos apuntes en `fuentes/` (102 cédulas):** `CONTRATOARRENDAMIENTO-pulido.md` (5, cap 12), `CONTRATOCOMODATO-pulido.md` (27, cap 13), `CONTRATOMUTUO-pulido.md` (24, cap 14), `CONTRATOMANDATO-pulido.md` (28, cap 15), `CONTRATOHIPOTECA-pulido.md` (18, cap 11 Garantías). Registro fijo en `FILES_CONFIG` con `categoryMap` a capítulos propios + extensión de `TEMARIO_CANONICO['civil']` (órdenes 9–12 especiales, Sucesiones 13, Familia 14, Garantías 15). Sin este registro, `lint_fuentes.py` falla con `colision-canon` y la deduplicación mutila el canon a 132 cédulas.
  - **Presupuestos escalables con el canon:** `build_vault_index.py` pasa a presupuesto efectivo `max(600 KB, totalDocs × 6 KB)` (1230 KB para 205; índice real 828.23 KB) y `scripts/build_cross_connections.py` a `max(300 KB, total × 3 KB)` (615 KB para 205; cruces reales 525.94 KB, 615 cruces). Suites `test_e2e_case_flow.cjs` 28.1/33.1 usan el mismo presupuesto derivado; Secciones 9.2/9.3/23.6.6 actualizadas a 13 fuentes (152 cédulas nuevas: 50 procesales + 102 civiles).
  - **Incidente operativo resuelto:** procesos `python server.py` huérfanos de días previos ocupaban el puerto 8080 con código anterior y regeneraban `all_afg_topics.json` a 132 cédulas vía `/api/sync-topics` durante las suites. Se eliminaron (`Stop-Process`) y se regeneró limpio.
  - **Verificación Automatizada Completa (100% PASS):** `test_deduplication_flow.cjs` 12/12, `test_unlock_auth_flow.cjs` 207/207, `test_mobile_header_theme.cjs` 85/85, `test_e2e_case_flow.cjs` 1260/1260. Canon final 205 (136 civil `1.1`–`1.136` + 59 procesal + 10 constitucional), cero duplicados, continuidad `N.M` intacta, IDs existentes `1.1`–`1.34` inmutados.
* **v7.41 (Índice Jerárquico Visual: etiqueta materia.capítulo.sección + capítulos colapsables — PROMPT 032):**
  - **Misión de Ingeniería:** Ordenar visualmente el sidebar con 205 cédulas sin tocar ningún dato: cada ítem muestra la etiqueta jerárquica `M.C.S` (ej. Acto Jurídico `1.1.1`, Comodato cap 13 `1.13.1`) derivada solo para display, agrupada bajo cabeceras de capítulo colapsables con conteo. El `indexCode` plano continuo (`1.40`…`1.136`), `code`, `id`, `chapterNumber` y los JSON (`all_afg_topics.json`, `js/data.js`, `vault_index.json`, `dogmatic_connections.json`) quedan byte-idénticos. Cero regresiones de Vault, QA, cruces (031), mastery y suites.
  - **Auditoría Fase 0 (verificada en datos, no asumida):** los 5 contratos coinciden 100% índice-inicial vs cédulas y el parser ya respeta `Sección N.N` explícita; fusionar a 4 cédulas por capítulo rompería Vault, QA, `QuestionDeveloper`, cruces 031 y mastery. Decisión: granularidad intacta, orden visual jerárquico.
  - **Decisión documentada S posicional:** `S = idx - minIdx + 1` por capítulo (con `_chapterMinIndexCache`), NO el segundo componente de `code` tal cual. Los contratos usan codes modulares (cap 13: `1.1`–`1.7`, `2.1`–`2.7`, `3.1`–`3.8`, `4.1`–`4.5`) que con `S=code` producirían 24 duplicados visuales (cuatro `1.13.1`); con `S` posicional hay 205/205 etiquetas únicas por capítulo con formato `^[123]\.\d+\.\d+$` y fallback a `indexCode`/`code` (nunca vacío).
  - **Implementación (solo display + estilos + lint, cero datos):**
    1. `js/app.js`: `_subjectNumberMap` (civil 1, procesal 2, constitucional 3), `_getChapterMinIndex()`, `displayHierCode(topic)`, `_openChapters` en memoria (aislado de `StorageService`), cabecera `Capítulo C · Título (N cédulas)` con `data-chapter-key`/`aria-expanded`/`aria-controls`/`role="button"`/`tabindex="0"` + toggle por Enter/Espacio, badge con `data-hier-code`, tooltip `Cédula ${indexCode} · Sección ${code} (archivo)`, `aria-label` con etiqueta + trazabilidad, auto-expansión del capítulo activo + `scrollIntoView`, todo colapsado al abrir excepto el activo.
    2. `css/sidebar.css`: badge `.cedula-code-badge` con `min-width:52px` + `tabular-nums` anti layout-shift (hereda 030), `.chapter-cedulas-count`, `:focus-visible` dorado en cabecera, `min-height:44px` en cabecera e ítem móvil, `content-visibility`/`overscroll-behavior` intactos, cero overflow-x a 360 px.
    3. `scripts/lint_fuentes.py`: `check_indice_descalce()` — en archivos con bloque `**Índice**` inicial cuenta bullets `Capítulo/Sección` vs headings del cuerpo; descalce → WARN `indice-descalce` con diff (no ERROR: el cuerpo manda al publicar, el índice es declarativo/visual).
  - **Pruebas nuevas (100% PASS):** `test_deduplication_flow.cjs` Tests 11–12 (unicidad 205/205 + Comodato→`1.13.1` + continuidad intacta; WARN ante fixture descalzado), `test_e2e_case_flow.cjs` Sección 34 (25 aserciones: helper, unicidad, datos intactos, colapso/aria, CSS, lint), `test_mobile_header_theme.cjs` Sección 7 (14 aserciones: badge, foco, targets 44px, contrato JS, unicidad).
  - **Verificación Automatizada Completa (1635 Pruebas en Total / 100% PASS):** `test_deduplication_flow.cjs` 14/14, `test_unlock_auth_flow.cjs` 207/207, `test_mobile_header_theme.cjs` 99/99, `test_e2e_case_flow.cjs` 1315/1315. `python scripts/lint_fuentes.py` → LINT OK (13 archivos, 3 avisos preexistentes, 0 errores, 0 `indice-descalce` en el canon).
  - **Archivos Modificados:**
    1. `js/app.js`: etiqueta `M.C.S` + capítulos colapsables con conteo y accesibilidad.
    2. `css/sidebar.css`: estilos de badge, conteo, foco y targets táctiles.
    3. `scripts/lint_fuentes.py`: chequeo WARN `indice-descalce` documentado.
    4. `test_deduplication_flow.cjs`: Tests 11–12 (14/14).
    5. `test_e2e_case_flow.cjs`: Sección 34 (1315/1315).
    6. `test_mobile_header_theme.cjs`: Sección 7 (99/99).
    7. `PROMPTS/032_indice_jerarquico_visual.md` + `PROMPTS/README.md`: prompt marcado implementado.
    8. `CONTEXT.md`: mapa §8 + bitácora §9 v7.41.
* **v7.42 (Ciclo de Nutrición y Mejora Continua: comando repetible + protocolo permanente — PROMPT 033):**
  - **Misión de Ingeniería:** Convertir la nutrición del agente en un proceso ejecutable cuando se desee (no en una implementación one-shot más): una frase de activación (**`nutrir sistema`**, válida con OpenCode o con Anti) y un comando real (`node scripts/ciclo_nutricion.cjs`) que mide, compara, registra y dictamina, bajo un protocolo de 5 fases con compuertas de actuación.
  - **Implementación:**
    1. `scripts/ciclo_nutricion.cjs`: fase medir + registrar (~60–90s). Baseline desde `trios_ledger_report.json` + canon derivado, barrido determinista heredado del 026, delta (fully pp, needsReview, overlap top-20, piloto 4/4), apéndice a `nutricion_historial.jsonl`, veredicto `BASELINE/MEJORA/ESTABLE/REGRESION` con exit 2 ante regresión. Cero toques a canon, datos, Vault, QA, cruces, auth o servidor.
    2. `PROMPTS/033_ciclo_nutricion_mejora_continua.md`: protocolo permanente estado → medir → diagnosticar (top-20, `needsReview`, piloto; clasificación datos/motor/cobertura) → actuar (automático: re-medir; con aprobación: motor; prohibido sin prompt dedicado: datos/Vault/auth) → verificar (medición pura: ciclo + lint; motor: 4/4) → registrar (`chore(nutricion): ciclo …`, §9 solo ante cambio de motor/código).
    3. `nutricion_historial.jsonl`: una línea JSON por corrida; las mediciones puras no inflan versiones.
  - **Primera corrida (evidencia):** veredicto `ESTABLE` — 4785/4785 fully 100%, 16836/16836 ancladas, needsReview 0, piloto 4/4, top-20 idéntico 20/20 (determinismo confirmado). Diagnóstico: top-20 todo al 100% (orden relativo, no falla) con patrón `archetypeId: "unknown"` en tríos `tradicion+reivindicatoria` → hallazgo clase `cobertura` (tríos compatibles sin arquetipo dedicado; el generador usa fallback genérico válido). Sin acción de motor requerida.
  - **Verificación:** `node --check` OK + ciclo completo en verde + `python scripts/lint_fuentes.py` OK (13 archivos, 3 avisos preexistentes). Sin cambios a código existente: las 4 suites verdes de v7.41 siguen vigentes (1635/1635).
  - **Archivos Modificados:**
    1. `scripts/ciclo_nutricion.cjs`: comando repetible del ciclo.
    2. `nutricion_historial.jsonl`: línea basal del primer ciclo (+ `trios_ledger_report.json` regenerado idéntico en métricas).
    3. `PROMPTS/033_ciclo_nutricion_mejora_continua.md` + `PROMPTS/README.md`: protocolo y frase de activación.
    4. `CONTEXT.md`: mapa §8 + bitácora §9 v7.42.
* **v7.43 (Grafo Vivo: Builder Offline desde Canon + Cruces — PROMPT 034):**
  - **Misión de Ingeniería:** Construir la capa de datos pura, desacoplada y determinista para el grafo vivo de conceptos jurídicos a partir del canon de 205 cédulas (`all_afg_topics.json`) y las conexiones dogmáticas (`dogmatic_connections.json`), generando artefactos compilados sin tocar ni alterar la UI existente del canvas (cero cambios en `js/concept-graph.js`, `js/data.js`, `index.html` o CSS), garantizando presupuesto $\le \max(600\text{ KB}, \text{totalNodes} \times 3\text{ KB}) = 615\text{ KB}$ e integrando la regeneración automática en CI/CD.
  - **Pilares Técnicos Implementados:**
    1. **Builder Determinista Offline (`scripts/build_graph_data.py`):**
       - Extracción y normalización de 205 nodos canónicos (exactamente 1 por cédula) con `id`, `label`, `subject`, `category` (disciplina), `chapterNumber`, `indexCode`, `hierCode` y peso proporcional logarítmico `val` calculado como $\min(25, \max(5, 5 + \text{deg} \times 2))$.
       - Procesamiento y deduplicación de enlaces: de 615 conexiones brutas en `dogmatic_connections.json`, el builder colapsa exactamente 70 conexiones recíprocas bidireccionales $A \leftrightarrow B$ mediante tuplas canónicas ordenadas `tuple(sorted([src, tgt]))` (`duplicatesCollapsed: 70`), resultando en exactamente 545 enlaces no dirigidos únicos ($615 - 70 = 545$).
       - Validación estricta de integridad referencial: 0 huérfanos (`orphansDropped: 0`) y 0 auto-enlaces (`selfLinksDropped: 0`).
       - Cumplimiento de taxonomía cerrada de 6 tipos: `Genero-Especie`, `Sustantivo-Procesal`, `Fundamento-Constitucional`, `Excepcion-Procesal`, `Efecto-Patrimonial`, `Materia-Afin`.
       - Ordenamiento alfanumérico estricto por `id` en nodos y por `(source, target)` en links, asegurando un hash SHA-256 byte-idéntico en cada ejecución.
       - Compatibilidad universal de consola Windows (impresiones CLI en ASCII puro evitando excepciones `UnicodeEncodeError` por flechas Unicode).
    2. **Artefactos Compilados y Exportación Universal:**
       - `graph_data.json`: 171.96 KB (holgadamente bajo el presupuesto de 615.00 KB), 205 nodos, 545 links.
       - `js/graph-data.js`: Declaración universal UMD/Global (`window.GRAPH_DATA = ...; if (typeof module !== 'undefined') module.exports = ...; if (typeof globalThis !== 'undefined') globalThis.GRAPH_DATA = ...;`).
       - `graph_report.json`: Reporte de auditoría y certificación forense (`totalNodes: 205`, `totalLinks: 545`, `orphansDropped: 0`, `selfLinksDropped: 0`, `duplicatesCollapsed: 70`, `budgetKB: 615.0`, `fileSizeKB: 171.96`, `sha256`, `generatedAt`).
    3. **Integración en CI/CD (`.github/workflows/deploy-pages.yml`):**
       - Adición del paso `python3 scripts/build_graph_data.py` entre `build_cross_connections.py` y `generate_clean_notes_data.py`.
    4. **Aserciones y Pruebas E2E (Sección 35 en `test_e2e_case_flow.cjs`):**
       - 10 nuevas aserciones verificando estructura, paridad con el canon de 205 nodos, 545 links únicos, 0 huérfanos, 0 auto-enlaces, taxonomía de 6 tipos, peso numérico `val`, presupuesto $\le 615$ KB, exportación `window.GRAPH_DATA` y reporte determinista.
       - 100% PASS en las 4 suites de prueba del repositorio (1648/1648 pruebas exitosas en total).
  - **Archivos Modificados:**
    1. `scripts/build_graph_data.py`: Creado.
    2. `graph_data.json`: Creado.
    3. `js/graph-data.js`: Creado.
    4. `graph_report.json`: Creado.
    5. `.github/workflows/deploy-pages.yml`: Modificado.
    6. `test_e2e_case_flow.cjs`: Modificado (Sección 35, 1338 tests).
    7. `PROMPTS/README.md`: Modificado.
    8. `CONTEXT.md`: Modificado (Sección 7.7, 8 y 9 v7.43).
* **v7.44 (Grafo Vivo en Canvas: Consumo de GRAPH_DATA sin Alteración de UI — PROMPT 035):**
  - **Misión de Ingeniería:** Conectar `js/graph-data.js` en `index.html` y adaptar `js/concept-graph.js` para consumir el grafo canónico completo (`GRAPH_DATA`: 205 nodos y 545 enlaces dogmáticos compilados en v7.43) en sustitución del grafo legado hardcoded de 14 nodos de `INITIAL_DATA.graph`, preservando con absoluta fidelidad la estética ("Dark Academy"), física de fuerzas, gestualidad móvil (pan, zoom, pinch-to-zoom), paleta por disciplina, barra flotante de herramientas e inspector lateral existente, con cero UI nueva y cero componentes añadidos (preparando el terreno para Prompt 036 inspector y Prompt 037 clustering).
  - **Pilares Técnicos Implementados:**
    1. **Carga y Encadenamiento de Dependencias (`index.html`):**
       - Inclusión de `<script src="js/graph-data.js"></script>` precediendo inmediatamente a `js/concept-graph.js`.
    2. **Cadena de Fallback Resiliente en `setupData()`:**
       - Prioridad canónica: `window.GRAPH_DATA` / `globalThis.GRAPH_DATA` $\rightarrow$ `StorageService.getData()?.graph` $\rightarrow$ `INITIAL_DATA.graph`.
       - Falla abierta con degradación elegante: si `GRAPH_DATA` está ausente o vacío, emite advertencia `console.warn('[ConceptGraph] GRAPH_DATA ausente, usando fallback legacy')` y renderiza los 14 nodos legados sin romper la aplicación.
    3. **Visualización y Radio Proporcional al Grado Dogmático:**
       - Sustitución de radio estático por fórmula sensible a conectividad: $r = 18 + \min(12, \text{grado} \times 2)$, garantizando nodos más prominentes para instituciones con mayor cantidad de cruces dogmáticos (e.g. contratos, obligaciones, recursos procesales).
       - Truncamiento de etiquetas en canvas a 28 caracteres (`n.label.length > 28 ? n.label.slice(0, 27) + '…' : n.label`) evitando solapamientos ilegibles.
       - Leyenda reactiva en esquina inferior: `${this.nodes.length} cédulas · ${this.links.length} nexos` ("205 cédulas · 545 nexos").
       - Fallback defensivo de color a Civil (`#38bdf8`) para nodos con materia desconocida con conteo de advertencias.
       - Filtrado silencioso y defensivo de enlaces con extremos inexistentes con advertencia preventiva.
    4. **Optimización del Bucle de Fuerzas y Rendimiento:**
       - Constantes físicas nombradas y balanceadas: `K_REPEL = 2200`, `K_SPRING = 0.04`, `TARGET_DIST = 140`, `REPEL_DIST_CAP = 320`, `DAMPING = 0.88`, `CENTER_STRENGTH = 0.015`.
       - Preevaluación rápida de bounding box (`Math.abs(dx) > 320 || Math.abs(dy) > 320`) que omite cálculo de distancia euclidiana y normalización trigonométrica para pares lejanos, reduciendo drásticamente la carga de CPU para 205 nodos ($\approx 20.910$ pares).
       - Auto-pausa inteligente del bucle de física `requestAnimationFrame` ante `document.hidden` o cuando `App?.currentView !== 'graph'`, reactivándose automáticamente en cambio de pestaña o cambio de vista.
    5. **Navegación e Inspector Sanitizado:**
       - `selectNode(node)` invoca `SecurityShield.escapeHtml()` sobre etiquetas y materias antes de inyectar en el inspector legado.
       - Enlace de navegación directo con `App.openTopic(node.id)`.
    6. **Accesibilidad y Exportación:**
       - Controles de zoom con `role="toolbar"`, `aria-label`, `title` y `tabindex="0"`.
       - Exportación universal CommonJS / Browser (`if (typeof module !== 'undefined') module.exports = ConceptGraph;`).
    7. **Aserciones y Pruebas E2E (Sección 36 en `test_e2e_case_flow.cjs`):**
       - 10 aserciones exhaustivas cubriendo los 10 puntos de la Definition of Done: carga en DOM, conteo exacto de 205 nodos y 545 nexos, radio dinámico por grado, truncamiento de etiquetas, fallback a 14 nodos de `INITIAL_DATA`, leyenda dinámica, constantes físicas y corte de distancia a 320 px, auto-pausa en `document.hidden` / vista distinta de `graph`, sanitización XSS en inspector, y navegación vía `App.openTopic`.
       - 100% de aprobación en las 4 suites de prueba del repositorio (1.678 pruebas exitosas sin una sola falla).
  - **Archivos Modificados:**
    1. `index.html`: Modificado (inclusión de script `js/graph-data.js`).
    2. `js/concept-graph.js`: Modificado (física optimizada, consumo de `GRAPH_DATA`, grado, truncamiento, leyenda, fallback, accesibilidad y CommonJS).
    3. `js/data.js`: Modificado (exportación universal `module.exports` preservada).
    4. `generate_clean_notes_data.py`: Modificado (plantilla sincronizada con exportación universal de `INITIAL_DATA`).
    5. `test_e2e_case_flow.cjs`: Modificado (Sección 36 añadida, total 1.358 tests).
    6. `PROMPTS/README.md`: Modificado (registro de Prompt 035 v7.44).
    7. `CONTEXT.md`: Modificado (Sección 8 y Sección 9 v7.44).

* **v7.45 (Inspector Cerebro: Ficha de Conocimiento por Nodo, Cruces Dogmáticos, Dominio Reactivo y QA Extractivo Mínimo — PROMPT 036):**
  - **Misión de Ingeniería:** Transformar el panel lateral de inspección del grafo (`#graph-inspector`) de un simple contenedor textual plano en una **Ficha de Conocimiento viva e interactiva** para cada uno de los 205 nodos del temario canónico, integrando metadata dogmática oficial (`§ indexCode`, capítulo temático, código jerárquico `M.C.S` con tooltip), cruces dogmáticos enriquecidos sin duplicación de código (reutilizando el helper unificado de la aplicación `renderConnectionCardsHtml` del Prompt 031 con citas verbatim y salto con highlight), alternancia reactiva de dominio de la cédula (`Dominada ✓ / Marcar por repasar`) con reflejo visual instantáneo en el lienzo HTML5 Canvas mediante un anillo/halo verde esmeralda (`#10b981`, stroke 2.5px), motor de consulta Q&A extractiva mínima on-demand mediante botón `¿Qué dicen mis apuntes? (Definición oficial)` sin dependencias de IA en runtime, escudo cibernético estricto anti open-redirect y anti-XSS, accesibilidad semántica (`role="complementary"`, `aria-label`) y diseño ergonómico responsive para pantallas móviles de 360 px con touch targets $\ge 44\text{ px}$ y cero desborde horizontal.
  - **Pilares Técnicos Implementados:**
    1. **Ficha de Conocimiento Enriquecida (`js/concept-graph.js`, `css/concept-graph.css`):**
       - Extracción de metadata canónica desde `topic` (vía `StorageService.getData()?.topics`): materia badge (`.inspector-meta-badge`), capítulo temático, código de índice (`§ indexCode`), y badge de código jerárquico `.inspector-hier-badge` (`displayHierCode` `M.C.S`) con tooltip explicativo accesible (`Cédula ${indexCode} · Sección ${code}`).
       - Título completo de la institución desplegado sin truncar en `.inspector-title` para estudio exhaustivo de grado.
       - Botón principal de navegación `Ver Cédula Completa` que valida el identificador en el allowlist canónico `isValidTopicId(node.id)` antes de invocar `App.openTopic(node.id)`.
    2. **Alternancia de Dominio Reactivo y Halo en Canvas (`draw()`):**
       - Botón de mastery `.btn-mastery-toggle` con estado dinámico (`isMastered ? 'Dominada ✓' : 'Marcar como dominada'`) y clase `.is-mastered` (borde verde esmeralda `#10b981`).
       - Al presionar el botón, se invoca `StorageService.toggleTopicMastery(node.id)` y se refrescan de inmediato tanto la ficha de inspección como el renderizado del grafo (`this.draw()`).
       - En `ConceptGraph.draw()`, todo nodo marcado como dominado (`StorageService.isTopicMasteredByUser(n.id) === true`) proyecta un anillo circular exterior verde esmeralda (`ctx.strokeStyle = '#10b981'`, `ctx.lineWidth = 2.5`, radio $r + 3.5$), brindando feedback visual inmediato en el "cerebro" interactivo del estudiante.
    3. **Cruces Dogmáticos Reutilizables con Citas del Canon:**
       - Desacoplamiento del helper unificado `renderConnectionCardsHtml(connsList, topicContext = null)` en `js/app.js` como método de `App` y expuesto globalmente en `window.renderConnectionCardsHtml`, garantizando cero duplicación de templates HTML entre el visor de lectura y el inspector del grafo.
       - En `js/concept-graph.js`, inclusión de `_renderConnectionCardsFallback(connsList)` para aislamiento en pruebas unitarias y entornos headless.
       - Renderizado de tarjetas `.linked-connection-card` con citas verbatim de apuntes encerradas en `<mark class="vault-highlight">`, botón `.btn-jump-connection` para saltar a la cédula con highlight, navegación interactiva a otros nodos del grafo al hacer clic en el nombre de la cédula vinculada, y estado vacío honesto si no hay cruces dogmáticos registrados.
    4. **QA Extractivo Mínimo On-Demand:**
       - Botón interactivo `¿Qué dicen mis apuntes? (Definición oficial)` (`.btn-inspector-qa`) que no satura el inspector por defecto y solo consulta cuando el estudiante lo solicita.
       - Tolerancia de `QAComposer.composeAnswer(query, options)` en `js/qa-composer.js` para recibir opciones como string directo (`'definicion'`) u objeto (`{ mode: 'definicion' }`).
       - Renderizado de 1 tarjeta extractiva `.qa-citation-card` con perspectiva dogmática, cita textual oficial y botón de salto rápido a la cédula, o gate honesto con mensaje pedagógico si el término no cuenta con cobertura canónica directa.
    5. **Seguridad y Accesibilidad Web:**
       - Sanitización integral contra inyección HTML/XSS con `SecurityShield.escapeHtml()`.
       - Allowlist canónico `ConceptGraph.isValidTopicId(topicId)` para verificar la existencia del destino contra los 205 temas antes de cualquier redirección o salto (escudo anti open-redirect).
       - Semántica ARIA: contenedor `#graph-inspector` configurado defensivamente con `role="complementary"` y `aria-label="Ficha de conocimiento de la cédula"`.
       - Touch targets ergonómicos $\ge 44 \times 44\text{ px}$ en todos los botones e interactivos para pantallas táctiles.
       - Contención estricta de scroll en móviles (`max-height: 70vh`, `overflow-y: auto`, `overflow-x: hidden`, `box-sizing: border-box`), garantizando cero desbordes horizontales en dispositivos de 360 px.
    6. **Aserciones y Pruebas E2E (Sección 37 en `test_e2e_case_flow.cjs` y Sección 8 en `test_mobile_header_theme.cjs`):**
       - Incorporación de la **Sección 37 (10 aserciones v7.45, PROMPT 036)** en `test_e2e_case_flow.cjs` (totalizando 1.414/1.414 pruebas exitosas en la suite).
       - Incorporación de la **Sección 8 (6 aserciones v7.45, PROMPT 036)** en `test_mobile_header_theme.cjs` (totalizando 105/105 pruebas exitosas).
       - 100% PASS en las 4 suites de prueba del repositorio (1.740/1.740 pruebas exitosas en total).
  - **Archivos Modificados:**
    1. `js/qa-composer.js`: Modificado (soporte de `options` como string directo `'definicion'`).
    2. `js/app.js`: Modificado (`renderConnectionCardsHtml` expuesto como método de `App` y `window`).
    3. `css/concept-graph.css`: Modificado (estilos de Ficha de Conocimiento, badges, mastery toggle, QA section, touch targets $\ge 44\text{ px}$ y contención 360 px).
    4. `js/concept-graph.js`: Modificado (`isValidTopicId`, `selectNode` con Ficha de Conocimiento, cruces dogmáticos, toggle reactivo de dominio, QA extractivo on-demand, halo verde esmeralda `#10b981` 2.5px en `draw()`).
    5. `test_mobile_header_theme.cjs`: Modificado (Sección 8 añadida, total 105 tests).
    6. `test_e2e_case_flow.cjs`: Modificado (Sección 37 añadida y mock de canvas enriquecido, total 1404 tests).
    7. `PROMPTS/README.md`: Modificado (registro de Prompt 036 v7.45).
    8. `CONTEXT.md`: Modificado (Sección 8 y Sección 9 v7.45).
* **v7.46 (Grafo Vivo: Toolbar, Filtros Atenuantes, Modo Foco Vecindario, Cap de 400 Aristas y 60 FPS — PROMPT 037):**
  - **Misión de Ingeniería:** Implementar la capa superior de control, filtrado y legibilidad a escala para el Grafo Vivo de Conceptos (`#graph-toolbar`), permitiendo a los postulantes navegar fluidamente los 205 nodos y 545 nexos dogmáticos sin sobrecarga cognitiva ni degradación de rendimiento. Integrar búsqueda canónica local con delegación transparente al Vault (`VaultSearch`), centrado suave de cámara en el viewport y despliegue automático en el Inspector Cerebro; filtros por materia y por capítulo que atenúan sin mutación destructiva de arrays para preservar la física intacta; switch de dominio con conteo reactivo `Dominadas X/N`; modo foco por vecindario a 1 salto (nodo y vecinos directos a 1.0 vs. 0.10, aristas incidentes doradas `#d4af37` con rótulos de tipo de nexo); límite de 400 aristas visibles priorizadas heurísticamente; optimización crítica a 60 FPS en el canvas mediante precomputación $O(1)$ de mastery; touch targets accesibles $\ge 44\text{ px}$ en móvil (360px); y cierre definitivo del ciclo canónico del grafo (Prompts 034–037).
  - **Pilares Técnicos Implementados:**
    1. **Toolbar Superior del Grafo (`js/concept-graph.js`, `css/concept-graph.css`):**
       - Contenedor flotante `#graph-toolbar` (`.graph-toolbar`) con estética *Dark Academy*, glassmorphism (`backdrop-filter: blur(12px)`), bordes dorados tenues y sombras de profundidad.
       - Reubicación no invasiva de los controles de zoom flotantes (`.graph-floating-controls`) a `top: 76px; left: 16px;` en desktop y `bottom: 80px; right: 10px;` en móvil, erradicando cualquier solapamiento visual con el nuevo toolbar.
       - Barra de búsqueda `#graph-search-input` con debounce de 150 ms, sanitización de caracteres de control (`[\x00-\x1F\x7F-\x9F]`), límite `maxlength="200"`, botón `#graph-search-clear` y canal de feedback accesible `#graph-search-feedback` (`role="status"`, `aria-live="polite"`).
       - Selector segmentado de disciplinas `#graph-filter-pills` (Todas, Civil, Procesal, Constitucional) con estado semántico `aria-pressed`.
       - Selector desplegable de capítulos `#graph-filter-chapter` poblado dinámicamente y ordenado por materia y mínimo `indexCode` canónico (Discusión $\rightarrow$ Prueba $\rightarrow$ Sentencia).
       - Switch de dominio `#graph-toggle-mastery-overlay` con conteo derivado en vivo (`Dominadas X/N`) sincronizado con `StorageService`.
    2. **Búsqueda Canónica Local y Delegada al Vault con Centrado en Viewport:**
       - Método `searchNode(query)`: sanitiza la entrada y delega en `VaultSearch.searchVault(cleanQuery, { limit: 5 })` cuando el motor del Vault está disponible, contando con fallback local directo insensibilizado a acentos y mayúsculas sobre `label`, `title` e `id` en `this.nodes`.
       - Al encontrar coincidencia, selecciona el top-1, centra suavemente la cámara con `centerNode(node)` calculando el offset necesario considerando el ancho del inspector en desktop, y abre automáticamente la Ficha de Conocimiento con `selectNode(node)`.
       - Ante búsquedas sin coincidencia, preserva inmutable la posición de la cámara (`panX`/`panY`) y emite mensaje pedagógico honesto en `#graph-search-feedback`.
    3. **Filtros Atenuantes sin Mutación de Arrays:**
       - Métodos `setSubjectFilter(subject)` y `setChapterFilter(chapter)`: operan modificando únicamente el cálculo de opacidad en el bucle de renderizado `draw()`, sin mutar, filtrar ni reasignar `this.nodes` ni `this.links`.
       - Los nodos que no cumplen el criterio de filtro se atenúan suavemente a opacidad $0.12$, manteniendo intacta la topología de fuerzas de resortes Hooke y repulsión de Coulomb.
    4. **Modo Foco por Vecindario a 1 Salto:**
       - Al seleccionar un nodo (clic directo o búsqueda), se activa el modo foco: el nodo seleccionado y todos sus vecinos directos a 1 salto se dibujan a opacidad plena $1.0$; todos los demás nodos no conectados caen a opacidad tenue $0.10$.
       - Las aristas incidentes en el nodo seleccionado se dibujan en dorado brillante (`#d4af37`, stroke de 2.5px) y con texto descriptivo del tipo de nexo centrado sobre el segmento; las aristas no incidentes caen a opacidad casi invisible ($0.05$).
       - Deselección total y restauración de opacidades normales al pulsar sobre el fondo vacío del canvas.
    5. **Cap de 400 Aristas Visibles y Leyenda Reactiva:**
       - Límite de seguridad (`visibleLinksCap = 400`) para evitar empaste visual en grafos densos sin degradar la simulación física (que computa los 545 nexos).
       - Heurística de ordenación de aristas visibles: 1) incidentes en el nodo enfocado; 2) transversales inter-materia (`source.subject !== target.subject`); 3) suma de grado incidente de nodos extremos (hubs).
       - Leyenda dinámica en vivo: `${this.nodes.length} cédulas · mostrando ${candidateLinks.length}/${this.links.length} nexos`.
    6. **Optimización de Rendimiento a 60 FPS (Frame Time < 10 ms):**
       - Diagnóstico de cuello de botella: invocar `StorageService.isTopicMasteredByUser(n.id)` dentro del bucle de 205 nodos provocaba 205 llamadas a `StorageService.getData()`, ejecutando 205 veces `JSON.parse` de 1 MB en cada frame (~85 ms por frame).
       - Solución: precomputación de un `Set` $O(1)$ (`masteredSet = new Set(StorageService.getUserMasteredTopics())`) una sola vez al inicio de `draw()`, reduciendo el tiempo de renderizado de frame a **6.9 ms - 8.1 ms** (holgadamente dentro del presupuesto de 16.6 ms para 60 FPS).
    7. **Diseño Responsive Móvil y Accesibilidad WCAG AA:**
       - Contenedor con altura máxima controlada (`max-height: 48vh`) y scroll táctil horizontal en `.graph-filter-pills` sin desborde de ventana.
       - Touch targets interactivos $\ge 44 \times 44\text{ px}$ en pills de materia, selector de capítulos, switch de dominio, input de búsqueda y botón de limpieza.
       - Cero desbordes horizontales comprobados en dispositivos móviles de 360 px.
    8. **Aserciones y Pruebas E2E (Sección 38 en `test_e2e_case_flow.cjs` y Sección 9 en `test_mobile_header_theme.cjs`):**
       - **Sección 38 (10 aserciones v7.46, PROMPT 037)** en `test_e2e_case_flow.cjs`: búsqueda local con match y centrado de cámara; despliegue en inspector; búsqueda sin match con feedback honesto e inmutabilidad de `panX`/`panY`; filtro de materia civil atenuante a 0.12 sin mutar `this.nodes`; selector de capítulos ordenado canónicamente; filtro de capítulo atenuante a 0.12; toggle de mastery overlay y contador reactivo; modo foco por vecindario (1.0 vs 0.10) y aristas doradas; cap de 400 aristas con leyenda reactiva; y medición de frame time de `draw()` (< 16.6 ms comprobado en 8.1 ms) (1.412 tests exitosos).
       - **Sección 9 (12 aserciones v7.46, PROMPT 037)** en `test_mobile_header_theme.cjs`: selectores DOM de toolbar, controles flotantes reubicados, `maxlength="200"`, `aria-live="polite"`, `aria-pressed`, touch targets $\ge 44\text{ px}$ y contención móvil 360 px (117 tests exitosos).
       - 100% PASS en las 4 suites de prueba del repositorio (1.750/1.750 pruebas exitosas en total).
       - Cierre formal del ciclo canónico de visualización del grafo (034 Capa Datos $\rightarrow$ 035 Canvas & Física $\rightarrow$ 036 Inspector Cerebro $\rightarrow$ 037 Toolbar & Escala).
  - **Archivos Modificados:**
    1. `js/concept-graph.js`: Modificado (Toolbar, búsqueda con centrado en canvas, filtros atenuantes, switch de dominio, modo foco vecindario a 1 salto, cap de 400 aristas con prioridad, y optimización O(1) de frame time a 60fps).
    2. `css/concept-graph.css`: Modificado (estilos `.graph-toolbar`, reubicación de controles flotantes `.graph-floating-controls`, diseño responsive móvil, touch targets $\ge 44\text{ px}$ y contención 360 px).
    3. `test_mobile_header_theme.cjs`: Modificado (Sección 9 añadida, 117 tests totales).
    4. `test_e2e_case_flow.cjs`: Modificado (Sección 38 añadida, 1412 tests totales).
    5. `PROMPTS/README.md`: Modificado (registro de Prompt 037 v7.46).
    6. `CONTEXT.md`: Modificado (Sección 8 y Sección 9 v7.46).

* **v7.46.1 (Hotfix Estabilidad Numerica del Grafo: signo Hooke + cap de velocidad + guarda anti-NaN):**
  - **Mision de Ingenieria:** Reparar el canvas en blanco de la vista Grafo. Causa raiz reproducida headless con datos reales (205 nodos / 545 aristas): el resorte aplicaba `b.vx += fx` en ambos extremos en vez de Hooke opuesto, lo que explotaba exponencialmente (coords 2.4e+39 al frame 60, 4.7e+300 al 480, NaN desde el 540, 205/205 nodos fuera de pantalla) y dejaba el canvas vacio.
  - **Pilares Tecnicos Implementados:**
    1. `js/concept-graph.js`: signo corregido (`b.vx -= fx; b.vy -= fy`), constante `MAX_VELOCITY = 12` con cap de velocidad por nodo en `updatePhysics()`, y guarda anti-NaN que recentra al centro cualquier nodo con coordenada no finita.
    2. `test_e2e_case_flow.cjs`: **Seccion 39 (6 aserciones v7.46.1)** — signo Hooke en fuente, `MAX_VELOCITY` en rango sano, 600 frames reales con 0 NaN, coords acotadas (< 20000), velocidades bajo el cap (medido 1.862 <= 12).
  - **Verificacion:** 4/4 suites al 100% PASS (e2e 1438/1438, unlock 207/207, dedup 14/14, mobile 117/117; total 1776/1776). Convergencia del layout verificada al frame ~120 con 0 nodos fuera de pantalla.
  - **Archivos Modificados:**
    1. `js/concept-graph.js`: Modificado (signo Hooke, `MAX_VELOCITY`, guarda anti-NaN).
    2. `test_e2e_case_flow.cjs`: Modificado (Seccion 39, 1438 tests totales).
    3. `CONTEXT.md`: Modificado (Seccion 8 y Seccion 9 v7.46.1).
  - **Interferencia con PROMPT 038:** nula. El 038 toca `server.py`, scripts nuevos y QA/Shield; este hotfix solo toca fisica del canvas y su seccion de tests. No se commitea aqui: queda en working tree para tu flujo en Antigravity.

* **v7.47 (Laya Decision Router como Capa Compartida y Pilotos Q&A / Shield — PROMPT 038):**
  - **Misión de Ingeniería:** Integrar Laya (`~/.laya` / `LAYA_HOME`) como capa de decisión rápida compartida y extensible para todo el repositorio, sin añadir dependencias pesadas en `requirements.txt` (PyTorch y Laya confinados estrictamente en el venv externo `~/.laya/venvs/laya`). Establecer el contrato canónico inmutable en `LAYA_CONTRACT.md` con su línea roja dogmática: **Laya sugiere, NUNCA decide** sobre rúbricas oficiales, autenticación, maestría de temas ni calificaciones de examen de grado. Montar el router determinista `scripts/laya_router.py` con registro estricto `LAYA_TASKS` ($\le 5$ opciones en choice), servidor worker residente en puerto 47888 (`run_worker_server`) y cliente HTTP local `query_warm_worker` con fallback determinista de salida transparente (exit 0 ante fallos o timeouts). Exponer el endpoint seguro `POST /api/laya/predict` en `server.py` con tope de 4 KB (HTTP 413) y rate limiter en memoria de 150 ms (HTTP 429). Debut con dos pilotos de alto impacto: 1) Autodetección del modo Q&A (`definicion`, `panorama`, `comparativa`) en `#qa-mode-pills` con supremacía inmutable del override manual del alumno; y 2) Puntuación de riesgo aditiva en `SecurityShield` (`LAYA_SHIELD_THRESHOLD = 0.85`) en matriz lógica OR paralela de 4 combinaciones con regex. Certificación formal con benchmark de 20 consultas doctrinales del Vault (95.0% de precisión y latencia p50 sub-500ms) y trazabilidad completa mediante el campo `reason` en cada línea de `router.jsonl` y flag `--state-file` en el CLI.
  - **Pilares Técnicos Implementados:**
    1. **Contrato Canónico e Instalador Versionado (`LAYA_CONTRACT.md`, `scripts/setup_laya.py`):**
       - `LAYA_CONTRACT.md` define formalmente el protocolo de integración de Laya, variables de entorno (`LAYA_HOME`, `LAYA_WORKER_PORT`), aislamiento de PyTorch/laya en `~/.laya/venvs/laya`, línea roja dogmática y registro de tareas autorizadas (`qa_mode`, `shield_risk`).
       - `scripts/setup_laya.py` implementa el instalador y verificador canónico con flags `--check` (para CI y diagnóstico previo sin alterar el sistema, exit 0 si está instalado o 1 si falta), `--smoke-only` (prueba rápida de inferencia en frío) y `--checkpoint` (descarga opcional offline de pesos).
    2. **Router de Decisiones, Worker Residente y Trazabilidad (`scripts/laya_router.py`):**
       - Registro centralizado `LAYA_TASKS` con validación estricta: denylist de palabras prohibidas (`rubrica`, `auth`, `mastery`, `nota`, etc.) y límite de $\le 5$ opciones para tareas de tipo `choice`.
       - Servidor worker cálido en background (`run_worker_server` en puerto 47888) que mantiene en memoria el modelo multilingüe de Laya y PyTorch, eliminando el cold start de 20 segundos.
       - Cliente HTTP local `query_warm_worker` que consulta al worker cálido con margen de socket; ante caída o timeout, dispara el daemon desasociado `ensure_worker_daemon()` y retorna fallback determinista limpio sin bloquear el hilo principal.
       - CLI `predict` con soporte para flag `--state-file` (leyendo archivos con encoding `utf-8-sig` para absorber limpiamente el BOM generado por PowerShell en Windows) y `--timeout-ms`.
       - Bitácora de inferencias en `~/.laya/logs/router.jsonl`: queries truncadas a $\le 80$ caracteres, cero PII y garantía absoluta del campo `reason` en cada línea (reportando `"ok"` en inferencias exitosas o la causa exacta ante fallbacks, como `"worker_timeout_500ms"`, `"worker_connection_refused"`, `"invalid_state_file"`, etc.).
    3. **Endpoint Seguro en Backend (`server.py`):**
       - Endpoint `POST /api/laya/predict` montado sobre la arquitectura multi-hilo de `server.py`.
       - Límite defensivo de carga útil de 4 KB (`MAX_LAYA_PAYLOAD_SIZE = 4096`, HTTP 413 ante desbordes).
       - Rate-limiting en memoria por IP (`LAYA_RATE_LIMIT_MS = 150`, HTTP 429 ante ráfagas rápidas).
       - Validación de tarea contra `LAYA_TASKS` con rechazo HTTP 400 ante identificadores no autorizados.
       - Delegación directa a `laya_router.predict(task_id, state, timeout_ms=500)` garantizando respuesta HTTP 200 con estructura uniforme `{ task, decision, confidence, latencyMs, fallback, reason }`.
    4. **Piloto 1: Autodetección de Modo Q&A con Override Manual (`js/app.js`):**
       - En `executeGlobalSearch()`, al consultar en modo Q&A se consulta de forma asíncrona no bloqueante a `/api/laya/predict` con timeout de 600 ms controlado por `AbortController`.
       - Si el alumno no ha realizado una selección manual (`qaModeManualOverride === false`), Laya clasifica la intención en `definicion`, `panorama` o `comparativa` y preselecciona la píldora correspondiente en `#qa-mode-pills` con feedback honesto en UI (`title="Sugerido por Laya"`, `aria-live="polite"`).
       - Al hacer clic manual en cualquier píldora de modo, se activa el flag `qaModeManualOverride = true`, garantizando que la voluntad del estudiante prevalezca inmutablemente sobre cualquier inferencia posterior de la IA.
    5. **Piloto 2: Puntuación de Riesgo Aditiva en SecurityShield (`js/security-shield.js`, `js/case-solver.js`):**
       - Constante nombrada `LAYA_SHIELD_THRESHOLD = 0.85` en `SecurityShield`.
       - Método `inspectPromptInjectionAsync(text)` con lógica OR paralela: evalúa simultáneamente el regex canónico síncrono y la inferencia continua `noul` de Laya (`task: "shield_risk"`, 600 ms timeout).
       - Matriz lógica de 4 combinaciones: 1) Limpio (`origin: 'ninguno'`); 2) Bloqueado por regex (`origin: 'regex'`); 3) Bloqueado por Laya (`origin: 'laya'`); 4) Bloqueado por ambos (`origin: 'ambos'`).
       - Fallback defensivo transparente: si Laya está desconectada, timeout o devuelve error, el sistema degrada limpiamente a la inspección por regex con cero falsos positivos (paridad funcional v7.46).
       - Integración en `CaseSolver.evaluateCurrentQuestion()` registrando trazabilidad forense (`securityOrigin` y `layaScore`).
    6. **Benchmark Doctrinal de 20 Consultas del Vault (`scripts/benchmark_laya_20.py`, `benchmark_results_038.json`):**
       - Batería de 20 consultas jurídicas reales extraídas del Vault de Conocimiento (10 definiciones, 5 panoramas y 5 comparativas) evaluadas contra el worker cálido.
       - Métricas certificadas: **95.0% de precisión (19/20)** contra ground truth (umbral requerido $\ge 80\%$), **0/20 fallbacks (0.0%)**, latencia **p50 de 498.4 ms** y **p95 de 548.8 ms** en CPU local.
       - Veredicto honesto sellado en `benchmark_results_038.json`: `VIABLE Y SUPERIOR AL DEFAULT`.
    7. **Aserciones y Pruebas E2E (Sección 40 en `test_e2e_case_flow.cjs`):**
       - **Sección 40 (8 aserciones v7.47, PROMPT 038)** en `test_e2e_case_flow.cjs`:
         * `40.1`: `scripts/setup_laya.py` existe, soporta flag `--check` y sale con código 0.
         * `40.2`: `LAYA_CONTRACT.md` consagra el aislamiento en `LAYA_HOME` y la línea roja dogmática inmutable.
         * `40.3`: `scripts/laya_router.py` define `LAYA_TASKS`, soporta `--state-file` con UTF-8 BOM, bloquea palabras prohibidas y supera auditoría estática (`check`).
         * `40.4`: `server.py` monta `POST /api/laya/predict`, valida 4 KB (HTTP 413), rate-limiter 150ms (HTTP 429), rechaza tareas desconocidas (HTTP 400), y responde HTTP 200 en inferencias vivas de `qa_mode` y `shield_risk`.
         * `40.5`: Piloto 1 en `app.js` consulta `/api/laya/predict`, aplica `qaModeManualOverride` ante clics manuales del usuario, e incluye honestidad UI (`title="Sugerido por Laya"`, `aria-live="polite"`).
         * `40.6`: Piloto 2 en `SecurityShield` define `LAYA_SHIELD_THRESHOLD = 0.85` y valida exhaustivamente la matriz OR de 4 combinaciones más fallback transparente cuando Laya está offline.
         * `40.7`: Bitácora `router.jsonl` contiene registros sin PII con snippets $\le 80$ chars, `latencyMs`, `fallback` y el campo `reason` en cada línea.
         * `40.8`: Benchmark de 20 consultas evaluado exitosamente certificando precisión $\ge 90\%$, latencia sub-600ms y veredicto superior al default.
       - **100% PASS en las 4 suites de prueba del repositorio (1.864 pruebas exitosas en total):**
         * `test_e2e_case_flow.cjs`: 1526 / 1526 PASS
         * `test_unlock_auth_flow.cjs`: 207 / 207 PASS
         * `test_deduplication_flow.cjs`: 14 / 14 PASS
         * `test_mobile_header_theme.cjs`: 117 / 117 PASS
  - **Archivos Creados:**
    1. `LAYA_CONTRACT.md`: Creado (contrato canónico de integración de Laya, aislamiento en `LAYA_HOME` y línea roja dogmática).
    2. `scripts/setup_laya.py`: Creado (instalador y verificador canónico con `--check`, `--smoke-only` y `--checkpoint`).
    3. `scripts/laya_router.py`: Creado (router determinista, registro `LAYA_TASKS`, worker cálido en puerto 47888, `--state-file` con BOM y bitácora con `reason`).
    4. `scripts/benchmark_laya_20.py`: Creado (evaluador determinista de 20 consultas doctrinales del Vault).
    5. `benchmark_results_038.json`: Creado (reporte formal del benchmark: 95.0% precisión, p50 498.4ms, veredicto `VIABLE Y SUPERIOR AL DEFAULT`).
    6. `PROMPTS/038_laya_router_pilotos.md`: Creado (especificación canónica del Prompt 038).
  - **Archivos Modificados:**
    1. `server.py`: Modificado (endpoint `POST /api/laya/predict`, rate limiter 150ms, cap 4 KB y delegación a `laya_router.predict`).
    2. `js/app.js`: Modificado (Piloto 1 de autodetección de Q&A con override manual y honestidad UI).
    3. `js/security-shield.js`: Modificado (Piloto 2 de puntuación de riesgo aditiva `inspectPromptInjectionAsync` con `LAYA_SHIELD_THRESHOLD = 0.85` y matriz OR).
    4. `js/case-solver.js`: Modificado (invocación de `inspectPromptInjectionAsync` en evaluación con registro de procedencia de seguridad).
    5. `test_e2e_case_flow.cjs`: Modificado (Sección 40 añadida, 1526 tests totales).
    6. `PROMPTS/README.md`: Modificado (registro de Prompt 038 v7.47 e índices 039/040).
    7. `CONTEXT.md`: Modificado (Sección 8 y Sección 9 v7.47).

* **v7.48 (Fase E1 de ESCALA.md: /api/health + rate-limit de escritura — ejecucion Nexo):**
  - **Mision de Ingenieria:** Primer paso costo-cero hacia miles de usuarios: health check barato para monitoreo/balanceadores y ventana fija anti-abuso en los endpoints POST de escritura sin limiter (auth/admin/Laya ya cubiertos). Cero cambios de comportamiento legitimo.
  - **Pilares Tecnicos Implementados:**
    1. `server.py`: `GET /api/health` sin auth (`{ok, version 7.48, time, uptimeSec, db:{reachable, sizeKB}}`); helper `_check_e1_rate_limit` (clave IP+ruta, 429 + `Retry-After`); guardas POST en `/api/user/progress` (30/min), `/api/user/topic-mastery` (60/min), `/api/ai/save-generated-case` (10/min), `/api/set-sync-folder` (10/min), todas DESPUES del auth/localhost para preservar 401/403.
    2. `test_unlock_auth_flow.cjs`: **Seccion 20 (12 aserciones v7.48)** — forma del health, rafaga concurrente Laya con 429, racha sin sesion toda 401 sin consumir cuota, presencia en fuente.
  - **Verificacion medida:** 4/4 suites 100% PASS — e2e 1496/1496, unlock 219/219, dedup 14/14, mobile 117/117. **Total 1846/1846.**
  - **Discrepancia honesta v7.47:** su bitacora declara e2e 1526 y total 1864, pero el arbol final arroja e2e 1496 en corridas independientes (100% PASS). Diferencia de 30 checks sin ramas condicionales de conteo en la Seccion 40. Se deja constancia para reconciliacion; canonicos son los medidos aqui.
  - **Archivos Modificados:** `server.py`, `test_unlock_auth_flow.cjs` (Seccion 20), `CONTEXT.md`, `ESCALA.md`.
  - **Interferencia con PROMPT 038:** nula (cerrado; bloque Laya intacto a proposito). Sin commit aqui: working tree para tu flujo Antigravity.
