...
* **Deduplicación Canónica Multicapa y Resiliencia de Almacenamiento (v7.7):**
  ...
  * **Regla de Prevalencia en Backend:** Si una sección o capítulo coincide simultáneamente entre un apunte fijo (`FILES_CONFIG`) y uno administrado (`apuntes_registry.json`), prevalece el apunte administrado más reciente (`source == "admin-upload"`). La función `deduplicate_sections()` en `generate_clean_notes_data.py` y `server.py` garantiza unicidad absoluta por tupla natural `(subject, chapterNumber, code)`.
...
* **Subida de Apuntes desde el Panel de Administrador (v6.8):** El panel de control (`#admin-dashboard-screen`) incluye la tarjeta `.admin-notes-upload-card` para subir archivos `.md` o `.txt` con drag-and-drop. El servidor valida PIN admin, verifica extensión, previene path traversal, persiste el archivo tanto en `apuntes/` como en `fuentes/`, actualiza `apuntes_registry.json`, regenera los tópicos con `generate_clean_notes_data.py` y responde con **HTTP 201 Created**. El cliente ejecuta de inmediato `await this.syncWithServer()` y `this.renderSidebar()`, reflejando la nueva sección y su cobertura temática en tiempo real sin recargar la página.
* **Parser Dinámico y Tolerancia a Archivos Monolíticos:** `generate_clean_notes_data.py` integra dinámicamente los apuntes preconfigurados en `FILES_CONFIG` con los registrados en `apuntes_registry.json`. Si un archivo no contiene subdivisiones formales `Sección N.N`, el parser genera automáticamente una sección monolítica con código `"1.1"`, título inferido del primer `#` o del nombre del archivo y estado de acceso protegido (`isFree: false`).
...
* **Límite Especial para Subida de Apuntes (`MAX_NOTES_UPLOAD_SIZE`):** El endpoint `/api/admin/upload-notes` cuenta con un umbral específico de **8 MB** (`8388608` bytes) para admitir apuntes y cédulas extensas en formato Markdown, texto o conversión documental sin comprometer la protección DoS en el resto de los endpoints de la API.
...
| `/api/admin/upload-notes` | `POST` | PIN Admin Requerido (`X-Admin-PIN` o body `pin`) | `201`, `400`, `401`, `413`, `500` | **Subir Apunte y Sincronizar (Admin):** Valida PIN admin y payload de hasta 8 MB (`MAX_NOTES_UPLOAD_SIZE`). Verifica extensión (`.md`, `.txt`, `.docx`, `.pdf`) y bloquea path traversal. Persiste el contenido en `apuntes/` y `fuentes/`, actualiza atómicamente `apuntes_registry.json`, regenera dinámicamente los tópicos y cédulas vivas con `generate_clean_notes_data.py`, actualiza `all_afg_topics.json` y `js/data.js`, y responde **HTTP 201 Created** con el número de secciones creadas. |
| `/api/admin/delete-notes` | `POST` | PIN Admin Requerido (`X-Admin-PIN` o body `pin`) | `200`, `400`, `401`, `404`, `500` | **Eliminar Apunte Administrado (Admin):** Valida PIN admin, previene path traversal, elimina el archivo físico de `apuntes/` y `fuentes/`, lo retira de `apuntes_registry.json`, regenera el índice temático y responde **HTTP 200 OK**. |
...
#### Componentes e Identificadores del Panel de Administrador y Subida de Apuntes (`#admin-modal`):
| Selector / ID | Tipo | Rol en el Flujo |
| :--- | :--- | :--- |
| `#admin-modal` | Contenedor Modal | Diálogo modal protegido por PIN maestro para gestión de licencias y subida de apuntes. |
| `#admin-dashboard-screen` | Pantalla Admin | Panel de control desplegado tras la autenticación exitosa del administrador con PIN SHA-256. |
| `.admin-notes-upload-card` | Tarjeta de Subida | Contenedor interactivo para la carga de nuevos apuntes y sincronización directa con el índice temático. |
| `#drop-zone-admin-notes` | Zona Drag & Drop | Área visual para arrastrar y soltar o seleccionar archivos `.md`, `.txt`, `.docx` o `.pdf`. |
| `#input-admin-notes-file` | Input File | Selector nativo de archivos oculto activado por clic en la zona drop o botón de explorar. |
| `#admin-notes-subject` | Selector Materia | Menú desplegable para asignar la materia jurídica del apunte (`civil`, `procesal`, `constitucional`). |
| `#admin-notes-chapter` | Input Capítulo | Código o identificador del capítulo del temario (ej: `cap-civ-1`). |
| `#admin-notes-chapter-title` | Input Título | Nombre canónico del capítulo para su agrupación en el índice lateral. |
| `#admin-notes-category` | Selector Categoría | Clasificación pedagógica del apunte (`apunte`, `cedula`, `resumen`, `esquema`). |
| `#btn-upload-admin-notes` | Botón Submit | Disparador de envío `POST /api/admin/upload-notes` con feedback de carga y spinner. |
| `#admin-notes-result` | Contenedor Feedback | Alerta dinámica para mostrar confirmación de éxito con número de secciones creadas o errores sanitizados. |
| `#btn-admin-open-advanced-notes` | Botón Acceso Rápido | Conmuta inmediatamente a la vista de apuntes (`topics`) para visualizar el nuevo contenido en vivo. |
...
| `js/app.js` | Orquestador | Controlador principal de GRADOMANIACOS, navegación, gateo estricto del sidebar `#app-sidebar` y `#btn-toggle-sidebar` a la vista `topics`, sincronización autoritativa con `/api/sync-topics` sin acumulación zombi (`syncWithServer`), renderizado deduplicado y debounced con `replaceChildren` (`renderSidebar`), tooltips y breadcrumbs sanitizados y auto-sincronizador de fuentes. |
...
| `server.py` | Backend | Servidor HTTP Python multi-hilo, resolución dinámica de `DB_PATH`, deduplicación canónica en `get_all_synced_topics()` y upsert estricto en `/api/admin/upload-notes`, libro mayor de usos auditables en `/api/admin/codes`, daemon de purga con doble inmunidad (`run_auto_purge_daemon`), ordenamiento canónico de `indexCode` en `/api/sync-topics`, Zero Exposure y rate limiting. |
| `generate_clean_notes_data.py` | Parser / Sync | Parser dinámico de apuntes y cédulas jurídicas. Deduplicación canónica con `deduplicate_sections()`, prevalencia explícita de apuntes administrados sobre fijos, numeración continua ordinal única por disciplina mediante `assign_index_codes()` (`indexCode`: Civil `1.1..1.34`, Procesal `2.1..2.9`, Constitucional `3.1..3.10`). Combina `FILES_CONFIG` con `apuntes_registry.json`, provee fallback monolítico y regenera `all_afg_topics.json` y `js/data.js`. |
...
| `test_deduplication_flow.cjs` | Test Integración | Suite de 6 pruebas (100% PASS) que verifica deduplicación canónica en `all_afg_topics.json`, unicidad de `indexCode`, auto-curación de 106 a 53 tópicos en `StorageService.getData()`, preservación de `mastered` y prevalencia de apuntes administrados. |
